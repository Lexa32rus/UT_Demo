...
Функция ВнешниеНаборыДанных()
	...
	ПодставитьТекстыЗапросаОстатков(МакетДляОтборов);
...
Процедура ПодставитьТекстыЗапросаОстатков(МакетДляОтборов)
	
	ТекстЗапроса = МакетДляОтборов.НаборыДанных[0].Запрос;
	...
	Иначе
		...
		Текст =
			"ВЫБРАТЬ
			|	ВЫБОР
			|		КОГДА ИнформацияОДоступности.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ПоступлениеНеПодтвержденное)
			|			ТОГДА 2
			|		КОГДА ИнформацияОДоступности.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление)
			|			ТОГДА
			|				ВЫБОР
			|					КОГДА ИнформацияОДоступности.ДатаПоступления < &ТекущаяДата
			|						ТОГДА 0
			|					ИНАЧЕ 1
			|				КОНЕЦ
			|	КОНЕЦ КАК ГруппаПериодов,
			...
			|	ИнформацияОДоступности.Запас КАК Количество
			|ИЗ
			...
			|		ПО ЗаказыРеестра.Ссылка = ИнформацияОДоступности.ЗаказНаПоступление
			...
			|		КОГДА ИнформацияОДоступности.ДатаПоступления < &ТекущаяДата
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ИнформацияОДоступности.ДатаПоступления <= КОНЕЦПЕРИОДА(&ДатаОкончания, ДЕНЬ)
			|	КОНЕЦ}
			|{ГДЕ
			|	ИнформацияОДоступности.Номенклатура.* КАК Номенклатура,
			|	ИнформацияОДоступности.Характеристика.* КАК Характеристика,
	...
	СтрокаДляПоиска = "ПОМЕСТИТЬ ЗапасыНаСкладе"; //@Query-part
	НачальнаяПозиция = СтрНайти(ТекстЗапроса, СтрокаДляПоиска);
	...
	Иначе
		...
		Текст =
			...
			|	ИнформацияОДоступности.Назначение КАК Назначение,
			|	ИнформацияОДоступности.Запас КАК Запас
			|ИЗ
			|	РегистрСведений.РаспределениеЗапасов КАК ИнформацияОДоступности
			|		
			|		{ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтборПоСегментуНоменклатуры КАК ТаблицаОтбора
			...
			|		 И ТаблицаОтбора.Характеристика = ИнформацияОДоступности.Характеристика}
			|ГДЕ
			|	ИнформацияОДоступности.Склад ССЫЛКА Справочник.Склады
	...
	МакетДляОтборов.НаборыДанных[0].Запрос = ТекстЗапроса;
