...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Создает служебный документ ""Регистратор графика движения товаров"", если соглашения используются при приемке товаров'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ИЗ
	|			Документ.РегистраторЗапасыИПотребности КАК РегистраторыГрафика
	|		ГДЕ
	|			РегистраторыГрафика.Распоряжение = СоглашенияСПоставщиками.Ссылка)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ЗапросРегистраторовГрафика = Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ИСТИНА КАК ЕстьЗаписи
		|ИЗ
		|	Документ.РегистраторЗапасыИПотребности КАК РегистраторыГрафика
		|ГДЕ
		|	РегистраторыГрафика.Распоряжение = &Распоряжение");
	...
	Для Каждого ЭлементСправочника Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			ЗапросРегистраторовГрафика.УстановитьПараметр("Распоряжение", ЭлементСправочника.Ссылка);
			Если ЗапросРегистраторовГрафика.Выполнить().Пустой() Тогда
				ДокументОбъект = Документы.РегистраторЗапасыИПотребности.СоздатьДокумент();
				ДокументОбъект.Дата = ТекущаяДатаСеанса();
				ДокументОбъект.Распоряжение = ЭлементСправочника.Ссылка;
