...
Процедура ЗаполнитьДеревоОбъектовЗакупки()
	...
	Для каждого ОбъектЗакупки Из ДеревоОбъектов Цикл
		...
		ОбъектЗакупки.Наименование = ОбъектЗакупки.Номенклатура;
	КонецЦикла;
	...
	Элементы.ДеревоОбъектовНоменклатура.ОграничениеТипа = ТипНоменклатура;
	
КонецПроцедуры

&НаСервере
Процедура СформироватьНаименованиеКонтракта()
	
	Шаблон = НСтр("ru='РК %1 №%2 от %3'");
	ДатаСтрока = Формат(Объект.ДатаЗаключенияКонтракта, "ДЛФ=D");
	Представление = СтрШаблон(Шаблон, Объект.НомерГОЗ, Объект.Номер, ДатаСтрока);
...
Процедура ПолучитьИзЕИСНаСервере()
	...
	Если РезультатПолучения.Контракты.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Контракт не найден.'"));
		Возврат;
	КонецЕсли;
	...
	Если СодержимоеКонтракта.ОшибкаПриСкачивании Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не удалось скачать данные контракта с сервера ЕИС.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьИзДанныхКонтракта(ДанныеКонтрактаИлиАдрес)
	...
	ЭлектронноеАктированиеЕИСПолучениеВходящих.
		ЗаполнитьИзДанныхКонтракта(ДанныеКонтракта, Объект);
