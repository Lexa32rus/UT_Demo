...
Функция СписокДоступныхОрганизаций(ТолькоСДоговором = Неопределено, ТолькоДействительные = Истина) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	                      ...
	                      |				ТОГДА НастройкиОнлайнОплат.СДоговором = &ТолькоСДоговором
	                      |			ИНАЧЕ ИСТИНА
	                      |		КОНЕЦ");
...
//
// Возвращаемое значение:
//   СправочникСсылка.НастройкиОнлайнОплат - Ссылка на найденную настройку, если ничего не найдено будет возвращена
//                                           пустая ссылка.
//
Функция НайтиНастройку(КлючиПоиска, ТолькоДействительные = Истина) Экспорт 
	...
	Если Не ТипЗнч(КлючиПоиска) = Тип("Структура") 
		...
		Возврат Справочники.НастройкиОнлайнОплат.ПустаяСсылка();
	КонецЕсли;		
...
// Параметры:
//  Настройка - СправочникОбъект.НастройкиОнлайнОплат, ДанныеФормыСтруктура - элемент, для которого нужно отработать
...
Функция ПолучитьТабличныйДокументКарточкиНастройки(Настройка) Экспорт
	...
	Объект = Неопределено;
	Если ТипЗнч(Настройка) = Тип("СправочникСсылка.НастройкиОнлайнОплат") Тогда
		Объект = Настройка.ПолучитьОбъект();
	ИначеЕсли ТипЗнч(Настройка) = Тип("СправочникОбъект.НастройкиОнлайнОплат")
