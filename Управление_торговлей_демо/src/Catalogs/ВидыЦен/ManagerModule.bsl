...
//   * ИспользуетсяЦенообразование25 - Булево, Неопределено - флажок использования нового ценообразования.
//
Функция ИнициализироватьСтруктуруПарметровДляПолученияОписанияСхемКомпоновкиПоИмени() Экспорт
	...
	Параметры.Вставить("ИспользуетсяЦенообразование25", Неопределено);
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВыводитьПроизвольныйВидЦен", ЗначениеПараметра) И ЗначениеПараметра = Истина Тогда
		...
		Если ЗначениеЗаполнено(ВладелецИндивидуальногоВидаЦен) Тогда
			...
			|		И (Партнеры.ИндивидуальныйВидЦены = ВидыЦен.Ссылка)
			...
			|		И ВидыЦен.Ссылка = СоглашенияСКлиентами.ИндивидуальныйВидЦены
			...
			|		И (ФорматыМагазинов.ИндивидуальныйВидЦены = ВидыЦен.Ссылка)
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
			|		ПО (ВидыЦен.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияВидовЦен.ИндивидуальныйДляСклада))
			|		И (Склады.ИндивидуальныйВидЦены = ВидыЦен.Ссылка)
			|ГДЕ
