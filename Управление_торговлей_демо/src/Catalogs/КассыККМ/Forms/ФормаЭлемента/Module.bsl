...
Процедура ТестовоеПодключениеНСПКЛокализация()
	...
	ОчиститьСообщения();
	ТестовоеПодключениеНСПКНаСервере();
...
&НаСервере
Процедура ТестовоеПодключениеНСПКНаСервере()
	
	ПараметрыОперации = ЭлектронныеСертификатыНСПКУТ.ПараметрыОперацииНСПКПоКассеККМ(Объект.Ссылка);
