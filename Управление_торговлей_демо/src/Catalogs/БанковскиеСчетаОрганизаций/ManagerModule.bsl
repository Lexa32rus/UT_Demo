...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение формата обмена в банковских счетах'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Справочник.БанковскиеСчетаОрганизаций КАК БанковскиеСчетаОрганизаций
	|ГДЕ
	|	БанковскиеСчетаОрганизаций.ФорматОбмена = ЗНАЧЕНИЕ(Перечисление.ФорматОбменаСБанком.ПустаяСсылка)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	СчетаКОбработке = Новый Массив;
	
	Пока Выборка.Следующий() Цикл
		Если Выборка.ФорматОбмена = Перечисления.ФорматОбменаСБанком.ПустаяСсылка() Тогда
...
Процедура ЗаполнитьФорматОбмена(ОбъектОбработки, Записать)
	...
	Если ОбъектОбработки.ФорматОбмена = Перечисления.ФорматОбменаСБанком.ПустаяСсылка() Тогда
		Если ОбъектОбработки.УдалитьФорматОбмена = "" Тогда
			ОбъектОбработки.ФорматОбмена = Перечисления.ФорматОбменаСБанком.ISO20022;
			ОбъектОбработки.ВерсияФорматаВыгрузки = "pain.001.001.03";
			//++ Локализация
			ОбъектОбработки.ФорматОбмена = Перечисления.ФорматОбменаСБанком.ClientBankExchange1c;
			ОбъектОбработки.ВерсияФорматаВыгрузки = "";
			//-- Локализация
		ИначеЕсли ОбъектОбработки.УдалитьФорматОбмена = "1CClientBankExchange" Тогда
			ОбъектОбработки.ФорматОбмена = Перечисления.ФорматОбменаСБанком.ClientBankExchange1c;
			ОбъектОбработки.ВерсияФорматаВыгрузки = "";
		ИначеЕсли ОбъектОбработки.УдалитьФорматОбмена = "ISO20022" Тогда
			ОбъектОбработки.ФорматОбмена = Перечисления.ФорматОбменаСБанком.ISO20022;
			ОбъектОбработки.ВерсияФорматаВыгрузки = "pain.001.001.03"
		КонецЕсли;
		Записать = Истина;
