...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Данные.ТипОбъектаНазначения) Тогда
		Представление = Данные.Наименование + " (" + Строка(Данные.ТипОбъектаНазначения)+")";
	Иначе
...
Функция ТекстЗапросаВсехНазначений(ПараметрыФормированияЗапроса) Экспорт
	...
	Если Не РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрНакопления.ЗапасыИПотребности", "РегистрНакопления.РаспределениеЗапасовДвижения");
	КонецЕсли;
...
Процедура ПриЗаписиДокумента(Назначение, ШаблонНазначения, ЭтотОбъект, Получатель, ВидДеятельностиНДС) Экспорт
	...
	Иначе // Это новое назначение
		...
		Если ЗначениеЗаполнено(ШаблонНазначения.Заказ) Тогда
			НазначениеОбъект.ПометкаУдаления    = РеквизитыДокумента.ПометкаУдаления;
			НазначениеОбъект.ВидДеятельностиНДС = РеквизитыДокумента.ВидДеятельностиНДС;
			НазначениеОбъект.ТипОбъектаНазначения = ТипОбъектаНазначенияПоДокументу(ШаблонНазначения.Заказ);
		КонецЕсли;
		НазначениеОбъект.Записать();
...
Функция ДобавитьОписаниеОбеспечениеЗаказовРаботамиПоВалюте(МакетФормы, ДобавитьСторно, Префикс)
	...
	ТекстЗапроса = 
		...
		|					0
		|			КОНЕЦ КАК ПрефиксВНаличии,
		|	РаспределениеЗапасов.КОбеспечениюБезРезерва - РаспределениеЗапасов.Запас КАК ПрефиксПотребность,
		|	РаспределениеЗапасов.КОбеспечениюБезРезерва + РаспределениеЗапасов.НеОбеспечивать КАК ПрефиксАрхивное
		|	,&ПоляВыбораПоследующие
		|ИЗ
		|	РегистрСведений.РаспределениеЗапасов КАК РаспределениеЗапасов
		|ГДЕ
...
Процедура ОпределитьТипОбъектаНазначения(РеквизитыНазначения)
	...
	ИначеЕсли ЗначениеЗаполнено(РеквизитыНазначения.Договор) Тогда
		...
		Если ВыбранТипНазначенияОтличныйОтПоставкиПодПринципала Тогда
			Если РеквизитыНазначения.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеМатериалы22 Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("97a57bf7-16bc-494b-85bd-b61c25c9eb49");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Контролировать превышение обособленного обеспечения"" справочника ""Направления деятельности""'");
	...
	Изменяемые.Добавить(Метаданные.Справочники.Назначения.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Справочники.Назначения.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ТаблицаОбъекта.Назначение КАК Ссылка
		|ИЗ
		...
		|			ИЛИ ТаблицаОбъекта.ОбновлениеПризнакаДопускаетсяОбособлениеСверхПотребностиНеТребуется
		|				И ТаблицаОбъекта.ДопускаетсяОбособлениеСверхПотребности <> ТаблицаОбъекта.Назначение.КонтролироватьТолькоНаличие)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДополнительныеПараметры.ДополнительныеИсточникиДанных.Вставить("НаправлениеДеятельности");
	
	Выборка = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, ПолноеИмяОбъекта, ДополнительныеПараметры);
	Пока Выборка.Следующий() Цикл
		
		НаправлениеДеятельности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Ссылка, "НаправлениеДеятельности");
		НачатьТранзакцию();
		...
		Попытка
			...
			Если ЗначениеЗаполнено(НаправлениеДеятельности) Тогда
				...
				ЭлементБлокировки.УстановитьЗначение("Ссылка", НаправлениеДеятельности);
				
			КонецЕсли;
			Блокировка.Заблокировать();
			
			Если НаправлениеДеятельности
					= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Ссылка, "НаправлениеДеятельности") Тогда
			
				ДанныеОбъекта = Выборка.Ссылка.ПолучитьОбъект();
				ДопускаетсяОбособлениеСверхПотребности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НаправлениеДеятельности,
					"ДопускаетсяОбособлениеСверхПотребности");
				
				Если ДанныеОбъекта = Неопределено Тогда
					ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(Выборка.Ссылка);
				ИначеЕсли ДопускаетсяОбособлениеСверхПотребности = Ложь
								И ДанныеОбъекта.КонтролироватьТолькоНаличие
							Или ДопускаетсяОбособлениеСверхПотребности = Истина
								И Не ДанныеОбъекта.КонтролироватьТолькоНаличие Тогда
					ДанныеОбъекта.КонтролироватьТолькоНаличие = ДопускаетсяОбособлениеСверхПотребности;
