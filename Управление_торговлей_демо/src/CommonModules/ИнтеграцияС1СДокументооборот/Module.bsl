...
Процедура ЗаполнитьПечатныеФормы(ВнешнийОбъект, ИмяКоманды, КоллекцияПечатныхФорм) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьПереопределяемый.ПриЗаполненииПечатныхФорм(
		ВнешнийОбъект,
		ИмяКоманды,
...
Функция ПолучитьОбъектXDTOПроцесса(ТипПроцессаXDTO, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры.ID) И ЗначениеЗаполнено(Параметры.type) Тогда
		ДанныеБП = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси, Параметры.type, Параметры.ID);
...
Функция ПринятьЗадачуКИсполнению(Прокси, Задачи) Экспорт
	...
	Для Каждого ID Из Задачи Цикл
		...
		ОбъектИД = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
		ОбъектИД.ID = ID;
...
Функция СоздатьОбъектДОПоПравилу(Прокси, ОбъектИС, Правило) Экспорт
	...
	ДоступенФункционалПакетныеЗапросы = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.
	...
	Если СтруктураРеквизитов.Свойство("Файлы") И СтруктураРеквизитов.Файлы.Количество() > 0 Тогда
		...
		Если ДоступенФункционалПакетныеЗапросы Тогда
			Пакет = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMBatchRequest");
		...
		Для Каждого СтрокаФайла Из СтруктураРеквизитов.Файлы Цикл
			...
			ОбъектВладелец.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси,
		...
		Если ДоступенФункционалПакетныеЗапросы Тогда
			Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапрос(Прокси, Пакет);
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, Ответ, "DMError") Тогда
				Возврат Ответ.description;
			КонецЕсли;
		КонецЕсли;
...
Функция СоздатьВидДокументаВДО(Свойства) Экспорт
	...
	ВидДокумента.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
...
Функция СоздатьПапкуВнутреннихДокументовВДО(Свойства) Экспорт
	...
	Папка.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
	Папка.objectID.ID = "";
...
Функция СоздатьXDTOФайл(Прокси, МассивПодписей, Файл) Экспорт
	...
	Объект = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси,"DMFile");
	Подписи = Объект.signatures; // СписокXDTO
	
	Объект.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, Файл.ID, "DMFile");
...
Процедура ЗаполнитьXDTOПодпись(Прокси, ПодписьXDTO, ДанныеПодписи) Экспорт
	...
	Если ДанныеПодписи.Свойство("УстановившийПодпись") Тогда
		...
		ПодписьXDTO.signer =  ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMUser");
		ПодписьXDTO.signer.name = ДанныеПодписи.УстановившийПодпись;
		
		ПодписьXDTO.signer.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси,
...
Процедура СогласованиеВозможно(Параметры, АдресРезультата) Экспорт
	...
	СвязанныйОбъектДО = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.
...
Процедура ДобавитьКомандуПереключитьХронометраж(Форма, Знач МестоРазмещенияКоманд)
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Попытка
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ПолучитьПрокси() <> Неопределено Тогда
...
Процедура СкопироватьКонтрагентаXDTO(Прокси, Источник, Приемник)
	...
	Приемник.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Источник, "legalPrivatePerson") Тогда
		Приемник.legalPrivatePerson = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"DMLegalPrivatePerson");
		Приемник.legalPrivatePerson.name = Источник.legalPrivatePerson.name;
		Приемник.legalPrivatePerson.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"DMObjectID");
