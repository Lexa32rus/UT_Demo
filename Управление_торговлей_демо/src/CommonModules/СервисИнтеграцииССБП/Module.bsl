...
Функция НастройкиТорговойТочки(ТорговаяТочка, ПлатежнаяСистема) Экспорт
	...
	Настройки.Используется = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ТорговаяТочка,
		"Используется");
	Если ПлатежнаяСистема = Перечисления.ПлатежныеСистемы.СБП_PayMaster Тогда
...
Функция ВызовЗагрузитьФайлОтчетаПоСверкеОпераций(
	...
	// Временное решение, связанное с проблемой использования Accept-Ranges
	// в сервисе PayMaster.
	Если ПараметрыИнтеграции.ПлатежнаяСистема = Перечисления.ПлатежныеСистемы.СБП_PayMaster
		И РезультатОперации.КодСостояния = 403 Тогда
