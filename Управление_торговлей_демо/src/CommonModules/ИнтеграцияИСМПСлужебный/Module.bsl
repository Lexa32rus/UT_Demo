...
Функция ВремяОжиданияОбработкиКвитанции() Экспорт
	...
	// Квитанция формируется в течение 40 минут.
	ВремяОжидания = 60*40;
...
Процедура ПодготовитьКПередачеСообщениеЗапросаСтатусаКодов(ЭлементОчереди, ПараметрыОбмена)
	...
	ПараметрыЗапроса = Новый Структура(
		"КодыМаркировкиДляКонтроляСтатусов,
		|ДопустимыеНачальныеСтатусыКодовОтчетОНанесении,
...
Функция ПодготовитьКПередачеСообщенияДляАгрегацииКодовМаркировкиИСМП(
	...
	Если (СтрокаДереваВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
		И СтрокаДереваВерхнегоУровня.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен)
		Или ((СтрокаДереваВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
			Или СтрокаДереваВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая)
			И (СтрокаДереваВерхнегоУровня.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Нанесен
				Или СтрокаДереваВерхнегоУровня.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Эмитирован)) Тогда
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.АгрегацияИзменение Тогда
		...
		Если ТелоЗапросаДобавление <> Неопределено И ТелоЗапросаДобавление.uit_uitu_list.Количество() > 0 Тогда
			...
			СообщениеJSON.Описание = СтрШаблон(
				...
				ИнтеграцияИСМПСлужебный.ОписаниеОперацииПередачиДанных(Операция),
				СтрокаДереваВерхнегоУровня.Штрихкод);
...
Процедура ДополнитьКолонкиДереваДаннымиОперацииЕАЭС(ДеревоУпаковок)
	...
	ДеревоУпаковок.Колонки.Добавить("ВидДокументаСертификации",   Новый ОписаниеТипов("ПеречислениеСсылка.ВидыДокументовОбязательнойСертификацииИСМП"));
...
Процедура ЗагрузитьСодержаниеРезультатаОбработкиДокумента(ЭлементОчереди, ПараметрыОбмена)
	...
	Если ЭлементОчереди.ВидПродукции = Перечисления.ВидыПродукцииИС.Духи Тогда
		ПараметрыОбработки.ИмяПоляСотировки = "name";
	Иначе
		ПараметрыОбработки.ИмяПоляСотировки = "key";
	КонецЕсли;
...
Процедура ОбработкаЗагрузкиПереданногоДокумента(ЭлементОчереди, ПараметрыОбмена, РезультатОбработкиЗаявки, ПараметрыОбновленияСтатуса)
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса
		...
		ИспользуетсяРегистрДляХраненияДанныхОтчета = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ИспользуетсяРегистрДляХраненияДанныхОтчета;
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.АгрегацияРасчетСтатуса Тогда
		...
		ПараметрыЗапроса = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса;
		ИспользуетсяРегистрДляХраненияДанныхОтчета = ПараметрыЗапроса.ИспользуетсяРегистрДляХраненияДанныхОтчета;
	...
	ИначеЕсли (ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.СписаниеПроверкаСтатусаОбработкиДокумента
			Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.СписаниеЭмитированныхКодовМаркировки
				И Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
					ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса, "КонтролироватьСтатусДокументаСписаниеИСМП", Ложь))
		...
		Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена
			...
			Если РезультатОбработкиЗаявки.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена Тогда
				...
				Если ПараметрыЗапроса <> Неопределено Тогда
					...
					Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОВерификацииНанесенныхКМ Тогда
...
Функция ЗапроситьСтатусПулаКодовМаркировкиПоСтрокеЗаказаИПолучитьКоды(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	Иначе
		...
		КодыМожноЗагружать = (ДанныеПула <> Неопределено);
...
//                  Не реализовано со стороны ЦРПТ
//					РезультатОбработкиПула.ОбработкаПулаЗавершена = (РезультатЗапросаКодовМаркировки.ДанныеБлока.ИдентификаторПоследнегоБлокаКодов = 0);
					РезультатОбработкиПула.ОбработкаПулаЗавершена = Истина;
...
Функция ЗагрузитьДокументы(ЭлементОчереди, ПараметрыОбмена, ПараметрыЗагрузки)
	...
	Для Каждого ИдентификаторДокумента Из ДокументыДляЗагрузки Цикл
		...
		Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ЭлементОчереди.ВидПродукции) Тогда
			ПараметрыОбработки = ИнтерфейсИСМП.НовыеПараметрыОбработкиСоставаДокумента();
			ПараметрыОбработки.ИмяПоляСотировки = "gtin";
		КонецЕсли;
		...
		Если СоставДокумента.Количество() > 0 Тогда
			...
			ИначеЕсли СтруктураОбработкиДокумента.ДанныеДокумента.Свойство("type") Тогда
				Если СтруктураОбработкиДокумента.ДанныеДокумента.type = "LP_SHIP_GOODS_CROSSBORDER" Тогда
					ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ПриемкаТрансграничнаяТорговля;
					ЭтоТрансграничнаяОперация = Истина;
				ИначеЕсли СтруктураОбработкиДокумента.ДанныеДокумента.type = "EAS_CROSSBORDER" Тогда
					ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ПриемкаИзЕАЭССПризнаниемКМ;
					ЭтоТрансграничнаяОперация = Истина;
				ИначеЕсли СтруктураОбработкиДокумента.ДанныеДокумента.type = "EAS_GTIN_CROSSBORDER_IMPORT" Тогда
...
Процедура ОбработкаДокументаОбъемноСортовогоУчета(ДокументПриемка, ДанныеШтрихкодов)
	...
	Для Каждого СтрокаДанных Из ДанныеШтрихкодов Цикл
		...
		Если ВидПродукцииСтроки = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС
			...
			ИдентификаторПроисхожденияВЕТИССтрокой = ИнтеграцияИСМПКлиентСервер.ПреобразоватьИдентификаторВСД(
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Иначе
		...
		РезультатПолученияДанных = ШтрихкодированиеИСМП.ВложенныеШтрихкодыУпаковокПоДокументу(ЭлементОчереди.Документ, ПараметрыСканирования);
	...
	Если Не НайденыСформированныеАгрегатыПоДаннымИСМП Тогда
...
Функция ЗаполнитьДанныеСтрокиВводВОборотИмпортФТС(СообщениеJSON, ОбщийЭлементОчереди, СтрокаИсточникаДанных, ИсточникДанныхТовары, Настройки)
	...
	Если СтрокаИсточникаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.АгрегированныйТаможенныйКод Тогда
		СтрокаТЧ.Вставить("packType", "ATK");
	ИначеЕсли СтрокаИсточникаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
		СтрокаТЧ.Вставить("packType", "UNIT");
	ИначеЕсли СтрокаИсточникаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
...
Процедура ПодготовитьСообщенияПоАбстрактнойАгрегации(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если Не ДанныеШапки.ОтчетПроизводственнойЛинии Тогда
		...
		Если ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ЭлементОчереди.ВидПродукции) Тогда
			РезультатПолученияДанных = ШтрихкодированиеМОТП.ВложенныеШтрихкодыУпаковокПоДокументу(ЭлементОчереди.Документ, ПараметрыСканирования);
		Иначе
			РезультатПолученияДанных = ШтрихкодированиеИСМП.ВложенныеШтрихкодыУпаковокПоДокументу(ЭлементОчереди.Документ, ПараметрыСканирования);
		КонецЕсли;
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииПриемкаИСМП(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если ОперацияПодтверждениеПриемки Тогда
		...
		Иначе
			...
			Если Не ЕстьОшибки
				...
				И ЭтоПриемкаИзЕАЭССПризнаниемКМ Тогда
	...
	Иначе
		...
		Если ИнтеграцияИСМПСлужебныйКлиентСервер.ЭтоПриемкаИзСтранЕАЭС(Шапка.Операция) Тогда
			Если ЭтоПриемкаИзЕАЭССПризнаниемКМ Тогда
...
Функция ТребуетсяОжиданиеОбработкиКвитанции(ЭлементОчереди) Экспорт
...
Процедура ВыполнитьЗаписьВРегистрПакетКодов(ЭлементОчереди, ДанныеСообщения = Неопределено)
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииОтчетОбИспользовании(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если МассивСтрок.Количество() Тогда
		...
		// Ошибка валидации. Один или несколько запрошенных ID заказов не найдены.
		// Способ обхода: повтор запроса.
		Если РезультатПолученияДанных.РезультатОтправкиЗапроса.КодСостояния = 404 Тогда
			ТребуетсяПовторОперации = Истина;
		КонецЕсли;
...
Процедура ЗагрузитьРезультатПолученияКвитанцииДокументаСУЗ(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	Если ТребуетсяПовторОперации Тогда
		...
		Если Не ЗначениеЗаполнено(ЭлементОчереди.ДатаМодификацииУниверсальная) Тогда
			ЭлементОчереди.ДатаМодификацииУниверсальная = ТекущаяУниверсальнаяДата();
		КонецЕсли;
		Если Не ТребуетсяОжиданиеОбработкиКвитанции(ЭлементОчереди) Тогда
...
Процедура ЗавершитьОбработкуКвитанцииСтатусНеопределен(ЭлементОчереди, ПараметрыОбмена, РезультатОбработкиЗаявки, ОписаниеЗамера)
	...
	ЭлементОчередиОснование = ЭлементОчередиСообщенияОснования(ЭлементОчереди, ПараметрыОбмена);
	ЗапрашиватьСтатусыКодовПослеОжиданияКвитанции = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ЗапрашиватьСтатусыКодовПослеОжиданияКвитанции;
	...
	Попытка
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru = 'Квитанцию получить не удалось.'") + 
				?(ЗначениеЗаполнено(РезультатОбработкиЗаявки.ТекстОшибки), Символы.ПС + РезультатОбработкиЗаявки.ТекстОшибки, "");
	...
	Если ТранзакцияЗафиксирована Тогда
		...
		УдалитьСообщениеВиртуальнойОчереди(ЭлементОчереди, ПараметрыОбмена);
		Если Не ЗапрашиватьСтатусыКодовПослеОжиданияКвитанции Тогда
			УдалитьСообщениеВиртуальнойОчереди(ЭлементОчередиОснование, ПараметрыОбмена);
		КонецЕсли;
...
Процедура ЗаполнитьОшибкиПриОбработкеВГИСМТ(ДанныеОтчета, КодыОбработанныеСОшибками, ИспользуетсяРегистрДляХраненияДанныхОтчета)
	...
	ОписаниеОшибки = НСтр("ru = 'Ошибка при обработке в ГИС МТ:'");
	...
	Если ИспользуетсяРегистрДляХраненияДанныхОтчета Тогда
		...
		Для Каждого НормализованныйКод Из КодыОбработанныеСОшибками Цикл
			...
			ТекстОшибки = ОписаниеОшибки + " " + НормализованныйКод;
	...
	Иначе
		...
		Для Каждого СтрокаОтчета Из ДанныеОтчета Цикл
			...
			Если КодыОбработанныеСОшибками.Найти(СтрокаОтчета.НормализованноеЗначениеШтрихкода) <> Неопределено Тогда
				СтрокаОтчета.ТекстОшибкиЗначениеШтрихкода = ОписаниеОшибки + " " + СтрокаОтчета.НормализованноеЗначениеШтрихкода;
			КонецЕсли;
			Если КодыОбработанныеСОшибками.Найти(СтрокаОтчета.НормализованноеЗначениеШтрихкодаУпаковки) <> Неопределено Тогда
				СтрокаОтчета.ТекстОшибкиЗначениеШтрихкодаУпаковки = ОписаниеОшибки + " " + СтрокаОтчета.НормализованноеЗначениеШтрихкодаУпаковки;
			КонецЕсли;
