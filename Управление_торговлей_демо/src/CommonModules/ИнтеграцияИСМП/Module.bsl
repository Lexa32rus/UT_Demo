...
Функция ОперацииСообщенияСовпадают(Выборка, ПредыдущаяВыборка)
	...
	Если Выборка.Операция = ПредыдущаяВыборка.Операция Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция ДанныеДляУточненияКоэффициентовУпаковок(ВходящиеДанные, ИспользоватьХарактеристики = Истина, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры <> Неопределено
		...
		ПроверяемыеGTIN = ШтрихкодированиеИС.МассивGTINНаОстаткиДляВыбора(ВходящиеДанные[0], ДополнительныеПараметры.Объект);
		ПустаяУпаковка  = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("Упаковка");
		
		Для Каждого GTIN Из ПроверяемыеGTIN Цикл
			ДанныеШтрихкода = Новый Структура;
			ДанныеШтрихкода.Вставить("Номенклатура",   ВходящиеДанные.Номенклатура);
			ДанныеШтрихкода.Вставить("Характеристика", ВходящиеДанные.Характеристика);
	...
	Запрос.УстановитьПараметр("GTIN",         ПроверяемыеGTIN);
	...
	ОписаниеНоменклатуры = Новый Соответствие;
	Выборка = РезультатЗапроса[ВГраница].Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.ПотребительскаяУпаковка) Тогда
			Данные = Новый Структура("ПотребительскаяУпаковка, КоличествоВПотребительскойУпаковке");
			ЗаполнитьЗначенияСвойств(Данные, Выборка);
			ОписаниеНоменклатуры.Вставить(Выборка.Номенклатура, Данные);
		КонецЕсли;
	КонецЦикла;
	
	ОписаниеGTIN = Новый Соответствие;
	Выборка = РезультатЗапроса[ВГраница - 1].Выбрать();
	Пока Выборка.Следующий() Цикл
		Данные = Новый Структура("ВидУпаковки, КоличествоПотребительскихУпаковок");
		ЗаполнитьЗначенияСвойств(Данные, Выборка);
		ОписаниеGTIN.Вставить(Выборка.GTIN, Данные);
	КонецЦикла;
	...
	Для Каждого GTIN Из ПроверяемыеGTIN Цикл
		...
		ПотребительскаяУпаковка            = Неопределено;
		КоличествоВПотребительскойУпаковке = 1;
		Если ДанныеОписанияНоменклатуры <> Неопределено Тогда
		...
		Если ДанныеПоGTIN.Коэффициент < КоличествоВПотребительскойУпаковке
			Или ДанныеПоGTIN.Коэффициент % КоличествоВПотребительскойУпаковке > 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ПотребительскаяУпаковка <> Неопределено
		...
		Если ЗначениеЗаполнено(СтрокаДанных.Упаковка)
			...
			СтрокаДанных.КоличествоПотребительскихУпаковок = СтрокаДанных.Коэффициент / СтрокаДанных.КоличествоВПотребительскойУпаковке;
		КонецЕсли;
