...
Функция ПодготовитьСтруктуруДляОтчетаКомиссионераУПД_2019(ДеревоДанных) Экспорт
	...
	Если ДополнительныеСведения.Свойство("НомерДоговора")
		И ДополнительныеСведения.Свойство("ДатаДоговора")
...
Функция ПолучитьДополнительныеСведения(ДеревоДанных)
	
	СтруктураДополнительныхЗначений = Новый Структура();
	ДополнительныеДанные = ДеревоДанных.Строки.Найти("ДопДанныеДокументаОтгрузки", "ПолныйПуть");
	Если ДополнительныеДанные <> Неопределено Тогда
		ТекстоваяИнформация = ДополнительныеДанные.Строки.Найти("ДопДанныеДокументаОтгрузки.ТекстоваяИнформация", "ПолныйПуть");
		Для Каждого СтрокиСведений Из ТекстоваяИнформация.Строки Цикл
			НазваниеИдентификатора = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СтрокиСведений, "ДопДанныеДокументаОтгрузки.ТекстоваяИнформация.НомерСтроки.Идентификатор");// "ДопДанныеДокументаОтгрузки.ТекстоваяИнформация.НомерСтроки.Идентификатор", Ложь);
			Если НазваниеИдентификатора <> Неопределено Тогда
				ЗначениеИдентификатора = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СтрокиСведений, "ДопДанныеДокументаОтгрузки.ТекстоваяИнформация.НомерСтроки.Значение", Ложь);
...
Процедура ЗаполнитьДанныеУКД2020_ИнформацияПродавца(Основание, Настройки, Данные, Описание, Отказ) Экспорт
	...
	Если СтруктураЭД.Функция <> "ДИС" Тогда	
		...
		Иначе
			...
			Если МассивСчетовФактур.Количество() = 0 Тогда
				НоваяСчетФактура = Документы.СчетФактураВыданный.СоздатьДокумент();
				ЗаполнитьЗначенияСвойств(НоваяСчетФактура, Основание, , "Валюта");
				СтрокаОснований = НоваяСчетфактура.ДокументыОснования.Добавить();
...
Процедура ЗаполнитьДанныеПервичногоДокументаУПД_2019(СтруктураДанных, СтруктураЭД, ДеревоДанных, Настройки, Отказ) Экспорт
	...
	Если ВидСчетаФактуры <> "Авансовый" Тогда	
		...
		Если Не ДанныеШапки.ТолькоУслуги Тогда
			...
			// ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС
			Если ЭтоЭлектронноеАктированиеЕИС И МестаПоставкиНеЗаполнены Тогда
				СведенияОГрузополучателе.Вставить("ИдентификаторМестаПоставкиЕИС", ИдентификаторМестаПоставкиЕИС);
				МестаПоставкиНеЗаполнены = Ложь;
				ЭлектронноеАктированиеЕИСУТ.ЗаполнитьМестаПоставкиПриложенияЕИСПоДаннымУчастника(
					СведенияОГрузополучателе, СтрокаГрузополучателя, "СведенияОГрузополучателе", ДанныеПриложенияЕИС, ДанныеШапки.Дата);
			КонецЕсли;
			// Конец ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС
...
Функция ЗапросКонструктораДополнительныхПолейШапкиСведенийОРеализацииКомиссионером()
	...
	ТекстЗапроса = 
		...
		|	ОтчетКомитенту.УдалитьПорядокОплаты КАК УдалитьПорядокОплаты,
