		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				Если ЗаписатьДвижения Тогда
					...
					Набор.Записать();
					ДокументСтруктура = Новый Структура("Ссылка, ДополнительныеСвойства", Выборка.Регистратор, Новый Структура);
...
Процедура ВыполнитьРаспределениеВФоне() Экспорт
	...
	Пока Истина Цикл
		...
		Если ЭтоПроизводительныйРежим() Тогда
			...
			ВременныеТаблицыДанныхРегистра(Запрос);
...
Процедура ВременныеТаблицыДанныхРегистра(Запрос, ИмяТаблицыТовары = "ТоварыДляРаспределенияЗапасов", ИмяТаблицыКорректировки = "")
...
Процедура ВременнаяТаблицаЗапасыИДоступностьПоДатам(Запрос)
	...
	Запрос.Текст =
		...
		|	Запасы.Характеристика КАК Характеристика,
		|	Запасы.Склад КАК Склад,
		|	СУММА(Запасы.Запас) КАК ВНаличии
...
Процедура ВременнаяТаблицаРезультатРаспределенияЗапасовНаПотребности(Запрос, ИмяТаблицыТовары = "ТоварыДляРаспределенияЗапасов", ИмяТаблицыРезультат = "РаспределениеЗапасовНаПотребности", ИмяТаблицыРеестрДокументов = "РегистрСведений.РеестрДокументов") Экспорт
	...
	Таблица = Пакет[Пакет.Вграница() - 7].Выгрузить();
	График = Пакет[Пакет.Вграница() - 6].Выгрузить();
...
Процедура ДобавитьТекстЗапросаКонтроль(Запрос, ТекстыЗапроса, Документ, ЭтоПроизводительныйРежим) Экспорт
	...
	Если ЭтоПроизводительныйРежим Тогда
		...
		ТекстЗапроса =
			...
			|		ИЛИ ИзмененияДвижений.Отгрузка - ЕСТЬNULL(РезервыПоЗаказам.Зарезервировано, 0) > 0
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура, Характеристика, Склад, Назначение";
		...
		ТекстЗапроса =
			...
			|	ИзмененияДвижений.Назначение
			|ИМЕЮЩИЕ
			|	СУММА(ИзмененияДвижений.ВНаличииПриход
			|			+ ИзмененияДвижений.Поступит
			|			- ИзмененияДвижений.ЗакрытьГрафикПоступления) > 0
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|ГДЕ
			|	ИзмененияДвижений.КонтролироватьПревышениеОбособленногоОбеспечения
			|		И (ИзмененияДвижений.Поступило
			|				+ ИзмененияДвижений.ПоступитКДате
			|				- ИзмененияДвижений.ЗакрытьГрафикПоступления > 0
