...
Функция НовыеСведенияОТоварахРаботахУслугахСметы() Экспорт
	...
	// В блоке указываются недетализированные позиции ТРУ из сведений о контракте
...
// Новые сведения о недетализированной позиции ТРУСметы.
// 
// Возвращаемое значение:
//  Структура:
...
Функция НовыеСведенияОПозицииСметы(Результат = Неопределено) Экспорт
	...
	// соответстви с ОКПД2 и в атрибуте "Признак поставки объекта закупки с улучшенными характеристиками"
...
// Новые сведения о ТРУ в с оответствии со сметой контракта.
// 
// Возвращаемое значение:
...
Функция НовыеДанныеДетализацииПоРазделамВидовРабот() Экспорт
	...
	// - "Объем работы, фактически принятый заказч.
...
Функция НовыеСведенияОКонтрактеСоСметой()
	...
	// При приеме контролируется, что организация пользователя, от имени которого осуществляется загрузка, является поставищиком по контракту с реестровым номерорм, заданным в данном атрибуте.
