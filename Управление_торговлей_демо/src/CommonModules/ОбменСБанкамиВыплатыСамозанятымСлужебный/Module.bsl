...
//			** НастройкаОбмена - СправочникСсыдка.НастройкиОбменСБанками 
...
//		ДанныеДокумента - Структура - данные, собранные из документа выплаты самозянятым.
//		АдресФайла - Строка - АдресВременногоХранилища, содержит двоичные данные файла.
//
...
Процедура ПриПолученииXMLФайлаЧекиВыплатСамозанятым(АдресДанныхФайла, ОбъектВладелец, ДанныеОповещения) Экспорт
...
//										в котором содерижится XML данные по реестру чеков выплат самозанятым, 
//										отправленные банком
//
// Возвращаемое значение:
...
Функция ЗагрузитьРеестрЧековИзФайла(ПолноеИмяФайла)
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Неверный формат файла. Файл: '") + ПолноеИмяФайла;
		Операция = НСтр("ru = 'Загрузка реестра чеков из банка'");
		ПодробныйТекстОшибки = ТекстСообщения;
		ЭлектронноеВзаимодействие.ОбработатьОшибку(Операция, ПодробныйТекстОшибки, ТекстСообщения, "ОбменСБанками");
...
// Создвет структуру полей для последующего заполнения
//
...
//
// Параметры:
//  НастройкаОбмена - СправчочникСсылка.НастройкиОбменСБанками
...
Функция ДанныеЗаполненияРеестрВыплатСамозанятым() Экспорт
	...
	ФизЛица.Колонки.Добавить("Сумма", ОбщегоНазначения.ОписаниеТипаЧисло(15,2));
