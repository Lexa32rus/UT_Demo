...
Процедура ДеревоЦенУпаковкаПриИзменении(Форма, ИдентификаторСтроки, ИмяТекущейКолонки) Экспорт
	...
	Для Каждого СтрокаВидЦены Из Форма.ВыбранныеЦены Цикл
		...
		Если СтрокаВидЦены.Выбрана ИЛИ СтрокаВидЦены.Влияет Тогда
			...
			Если Не (ТекущаяСтрока.ХарактеристикиИспользуются И ТекущаяСтрока.Характеристика.Пустая()) Тогда
				НоваяСтрока = ТаблицаЗначений.Добавить();
			...
			Если Не ИспользоватьТаблицуЗначенийВместоДереваЗначений(Форма) Тогда
				Для Каждого ПодчиненнаяСтрока Из ТекущаяСтрока.ПолучитьЭлементы() Цикл
					...
					Если Не ЭтоРегистрацияЦенПоставщика И ПодчиненнаяСтрока[ИмяКолонкиЗапретаРедактирования] Тогда
						Продолжить
					КонецЕсли;
