// Регистрация ERP Синхронизация данных через универсальный формат (2.5.9, v.1) от 28.07.2022 20:41:54
...
Функция ПараметрыРегистрации() Экспорт
	...
	Результат.Вставить("Наименование", НСтр("ru = 'ERP Синхронизация данных через универсальный формат (2.5.9, v.1)'"));
	Результат.Вставить("ДатаВремяСоздания", Дата(2022,7,28,20,41,0));
	...
	Результат.Вставить("ВерсияКонфигурации", "2.5.9.68");
...
Процедура ПРО_РегистрСведений_СостоянияЗаказовКлиентов_ПередОбработкой(ПРО, Отказ, Объект, ОбъектМетаданных, Выгрузка)
	...
	Если ЗначениеЗаполнено(Заказ)
		И ОбщегоНазначения.СсылкаСуществует(Заказ)
