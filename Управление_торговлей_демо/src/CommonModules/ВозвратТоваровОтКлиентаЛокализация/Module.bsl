...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипДанныхЗаполнения = Тип("ДокументСсылка.ЧекККМ") Тогда
		РозничныеПродажиЛокализация.ПроверитьВозможностьВводаНаОсновании(ДанныеЗаполнения);
...
Процедура СформироватьШтрихКодыУпаковок(РаспоряженияДляОрдеров, ТаблицаШтрихкодыУпаковок) Экспорт
	...
	ТаблицаОрдеров = РегистрыНакопления.ТоварыКПоступлению.ПолучитьОрдераПоРаспоряжению(РаспоряженияДляОрдеров);
...
Процедура ЗаполнитьДокументНаОснованииЧекаККМ(Объект, Знач ДокументОснование)
	...
	Если Документы.ЧекККМКоррекции.ЕстьКоррекцияПоЧекуККМ(ВыборкаШапка.ЧекККМ) Тогда
		ВызватьИсключение НСтр("ru = 'По Чеку ККМ введен Чек ККМ коррекции. Ввод на основании чека документа ""Возврат товаров от клиента"" запрещен.'");
	КонецЕсли;
	Если Документы.ЧекККМВозврат.ЕстьВозвратПоЧекуККМ(ВыборкаШапка.ЧекККМ) Тогда
		ВызватьИсключение НСтр("ru = 'По Чеку ККМ введен Чек ККМ на возврат. Ввод на основании чека документа ""Возврат товаров от клиента"" запрещен.'");
	КонецЕсли;
