...
Функция ПолучитьСоставМонитора(ПараметрыСоставаМонитора) Экспорт
	...
	ВариантыАнализаПользовательскиеНастройки = СтруктураНастроек.ВариантыАнализаПользовательскиеНастройки; // ХранилищеЗначения
...
Функция ПолучитьСоставМонитораВзаимосвязанных(ПараметрыСоставаМонитора) Экспорт
	...
	ХранилищеАдресовДинамическихПараметров = ПараметрыСоставаМонитора.АдресаДинамическихПараметров; // ХранилищеЗначения
	АдресаДинамическихПараметров	= ХранилищеАдресовДинамическихПараметров.Получить();
	ОтборЗонаВнимания				= ПараметрыСоставаМонитора.ОтборЗонаВнимания;
...
Процедура РасчетИсточниковДанныхВариантовАнализа() Экспорт
	...
	Для Каждого Пользователь Из ТаблицаПользователей Цикл 
		...
		Если Не ПользовательИнформационнойБазы = Неопределено Тогда
			...
			ХранилищеВариантовАнализаПользователя = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
															"НастройкиМонитораЦелевыхПоказателей",
															"ВариантыАнализаПользовательскиеНастройки",,
															ИмяПользователяТаблицы); // ХранилищеЗначения
...
Процедура ВывестиОтдельныйПоказатель(ТабличныйДокумент, ОписаниеИсточникаДанных) Экспорт
	...
	ИсточникДанных = ОписаниеИсточникаДанных.ИсточникДанных; // ХранилищеЗначения
	ИсточникСПараметрами = ИсточникДанных.Получить(); // См. ИнициализироватьИсточникДанных
...
Функция ПолучитьСвойстваВариантаАнализа(ВариантАнализа, ИсточникДанных) Экспорт
	...
	Если Не РезультатЗапросаСвойствВариантаАнализа.Пустой() Тогда
		...
		ПериодАнализа = ВыборкаСвойствВариантаАнализа.ПериодАнализа; // ХранилищеЗначения
		СвойстваВариантаАнализа.ПериодАнализа = ПредставлениеСтандартногоПериода(ПериодАнализа.Получить());
		
		ХранилищеПериодаСравнения = ВыборкаСвойствВариантаАнализа.ПериодСравнения; // ХранилищеЗначения
		ПериодСравнения = ХранилищеПериодаСравнения.Получить();
		Если Не ПериодСравнения = Неопределено И СвойстваВариантаАнализа.ТипАнализа = Перечисления.ТипыАнализаПоказателей.СравнениеСПрошлымПериодом Тогда
			СвойстваВариантаАнализа.ПериодСравнения = ПредставлениеСтандартногоПериода(ХранилищеПериодаСравнения.Получить());
		Иначе
			СвойстваВариантаАнализа.ПериодСравнения = Неопределено;
		КонецЕсли;
		
		// Заполним информацию по пользовательским настройкам
		ХранилищеПользовательскихНастроек = ВыборкаСвойствВариантаАнализа.ХранилищеПользовательскихНастроекКомпоновкиДанных; // ХранилищеЗначения
		ПользовательскиеНастройки = ХранилищеПользовательскихНастроек.Получить(); // ПользовательскиеНастройкиКомпоновкиДанных - 
...
Функция ПолучитьИсточникДанныхВариантаАнализаИзКэша(ВариантАнализа, ВариантОтображения)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ХранилищеИсточникаДанных = Выборка.ИсточникДанных; // ХранилищеЗначения
