...
Процедура ВыгрузитьУстановленныеЦены()  
	...
	Если ИспользуетсяЦенообразование25 Тогда
		ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1000 РАЗЛИЧНЫЕ 
					   ...
					   |	И ЦеныНоменклатуры25СрезПоследних.Период > СтатусыПубликацииТоваровЯндексМаркет.ДатаУстановкиЦены"; 
	...
	Иначе 
		ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1000 РАЗЛИЧНЫЕ 
					   ...
					   |	И ЦеныНоменклатурыСрезПоследних.Период > СтатусыПубликацииТоваровЯндексМаркет.ДатаУстановкиЦены"; 
	...
	Если РезультатЗапроса.Количество()>0 Тогда
		...
		Иначе
			Если СтруктураОтвета.status = "ERROR" Тогда
				ТекстОшибки = ТекстОшибки(СтруктураОтвета.errors);	
			Иначе
				ТекстОшибки = "Код ошибки" + КодСостояния; 
			КонецЕсли; 	
			ТекстОписанияСобытия = НСтр("Ошибка вызова API Яндекс.Маркет'; "+ HTTPЗапрос.ПолучитьТелоКакСтроку());
			ЗаписьЖурналаРегистрации(ТекстОписанияСобытия, УровеньЖурналаРегистрации.Ошибка,,,ТекстОшибки);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьДатуУстановкиЦены(ТаблицаДатУстановкиЦен)
	...
	Для каждого Стр Из ТаблицаДатУстановкиЦен Цикл 
			...
			Если Набор.Количество() Тогда
				Запись = Набор[0];
				Запись.ДатаУстановкиЦены = Стр.Период;
				Набор.Записать();
			КонецЕсли;
	КонецЦикла;
...
Функция ВидыЦенПоИмениПоля(ИмяПоля, МассивРекомендованныхЦен)   
	...
	Иначе	
		Для каждого ЭлементМассива Из МассивРекомендованныхЦен Цикл 
			ЗначениеИзХранилища = ЭлементМассива.ХранилищеНастроек.Получить();				
