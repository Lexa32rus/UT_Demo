...
Функция ТекстЗапросаНастроекОтправкиОбъектовУчета() Экспорт
	...
	|		ПО ОписанияОбъектовУчета.ДоговорКонтрагента = ГосударственныеКонтрактыЕИС.ВладелецКонтракта
...
Процедура ЗаполнитьСведенияОПодписантах(ДеревоДанных,
	...
	Для каждого ДанныеПодписанта Из Подписанты Цикл
		...
		Подписант = ФорматыЭДО_ФНС.ПолучитьОбъектТипаCML(ПрефиксТипа + ".Подписант", ПространствоИменСхемы);
...
// 
// Возвращаемое значение:
//  Строка - Преобразованый код КЛАДР для ЕИС
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Если ЕстьПодсистемаДляЗаказчиков() Тогда
		МодульПодсистемыЗаказчика = ОбщегоНазначения.ОбщийМодуль("ЭлектронноеАктированиеЗаказчикаЕИС");
		МодульПодсистемыЗаказчика.ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание);		
	КонецЕсли;
...
Функция КодыАдреса(Адрес) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(КодыАдреса.КодКЛАДР) Тогда
		...
		КодыАдреса.КодКЛАДР = ЭлектронноеАктированиеЕИС.КодКЛАДРПроизвольногоАдреса(КодРегиона);
	КонецЕсли;
...
// * СведенияОРодительскойПозиции - Структура - см. НовыеСведенияОРодительскойПозицииТРУ()
...
// * ОКЕИ - Структура - см. НовоеЗначениеОКЕИ
// * КодОЕИ - Строка
...
// * СведенияОТаможеннойДекларации - Массив
// * СтоимостьСНДСВсего - Число -
...
Функция НовыеСведенияОСтроительствеДляПриложенияУПД() Экспорт
	...
	// то игнорируется при приеме, при передче заполняется значением true.
...
// * ЭтоЛекарственныйПрепарат - Булево
// * СведенияОЛекарственномПрепарате - Структура - см. НовыеСведенияОЛекарственномПрепаратеДляПриложения()
...
// * ОбластьПолномочий - Строка - область полномочий.
// * Статус - Строка - статус.
// * ОснованиеПолномочий - Строка - основание полномочий.
// * ИНН - Строка - ИНН.
// * Должность - Строка - должность.
// * Фамилия - Строка - фамилия.
// * Имя - Строка - имя.
// * Отчество - Строка - отчество
// * ТипПодписанта - ПеречислениеСсылка.ТипыПодписантовЕИС - тип подписанта.
...
// * КодОбластиПолномочий - Строка - код области полномочий.
// * НаименованиеОбластиПолномочий - Строка - Наименование области полномочий.
// * ОснованиеПолномочий - Строка - Основание полномочий.
...
Функция ДанныеГосконтрактаПоВладельцу(ВладелецКонтракта) Экспорт
	...
	ДанныеГосконтракта.Вставить("ГосконтрактЕИС",
		ПустаяСсылкаНаКонтракт);
...
Процедура ПослеЗаписиНаСервереВладельцаКонтракта(ДанныеФормы, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	ПредыдущееЗначение = ЭлектронноеАктированиеЕИС.НайтиГосударственныйКонтрактПоВладельцу(
...
// Заполнить банковские реквизиты поставщика.
// 
// Параметры:
...
Процедура ЗаполнитьБанковскиеРеквизитыПоставщика(
				...
				ТребуетсяКазначейскоеСопровождение = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(БанкРеквизитыВДереве) Тогда
		Если НЕ ТребуетсяКазначейскоеСопровождение Тогда
			...
			БанковскиеРеквизиты.ТипСчета = ТипСчетаРасчетныйСчет();
		...
		Иначе
			// Заполнение банковских реквизитов.
			БанковскиеРеквизиты.ТипСчета = ТипСчетаЛицевойСчетВТерОрганеКазначейства();
			БанковскиеРеквизиты.НаименованиеБанка = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
				СтрокаДереваДанных,
				ВидУчастника + ".БанковскиеРеквизиты.НаименованиеБанка");
			БанковскиеРеквизиты.БИК =  ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
...
//  ДанныеДляОтправкиИлиСтрока - Строка - Данные для отправки или строка
// 
// Возвращаемое значение:
...
// * ТипФайлогоХранилищаОтносительнойСсылки - Строка
...
//  УзелДерева - ДеревоЗначений, СтрокаДереваЗначений
Процедура ЗаполнитьУзелДереваXMLПоСтруктуре(СтруктураДанных, Знач УзелДерева) Экспорт
	...
	// той же последовательсти, что и в схеме xml.
...
//  ИмяКорневогоЭлемента - Неопределено, Строка
// 
// Возвращаемое значение:
//  Строка
...
Функция СведенияОбОбъектеПоставщика(Организация,
	...
	Если НЕ РезультатОтправкиЗапроса.Выполнено Тогда
		МаксимальноеЧислоПопытокПриОшибкеСоединения = 3;
...
// Выполнилняет замену имени справочника гос. контрактов в строке.
...
// 
// Возвращаемое значение:
//  Структура - Результат проверики возможности выполнения операции:
// * Выполнено - Булево
// * ОписаниеОшибки - Строка
// * Статус - Строка
...
// 
// Параметры:
//  ПутьККонтрактуИлиДвоичныеДанныеИлиОбъектXDTO - Строка, ДвоичныеДанные, ОбъектXDTO
...
Функция ПроверитьДоступностьСервераSOAP(АдресСервера, АдресРесурса)
	...
	Если НЕ РезультатАнализа.Выполнено Тогда
		Шаблон = НСтр("ru = 'Ошибка при подключении к серверу ЕИС %2:'")
			+ Символы.ПС + "%1";
...
Функция НовыеСведенияОТоварахРаботахУслугахДляТитулаПродавцаУПД()
	...
	// Если отраслевой специализацией документа является сторительство
	// и установлен признак накопительного итога
	// (в атибуте "Применяется накопительный итог"
...
Функция НовыеБанковскиеРеквизитыДляПриложения()
	...
	Результат.Вставить("ТипСчета");
	Тип = Новый ОписаниеТипов("Строка");
	Описание = НСтр("ru = 'Тип счета.'");
	Значение = ТипСчетаРасчетныйСчет();
	...
	// И
	// в связанном контракте не установлен признак "Требуется казначейскон сопро.
	...
	// или "ЛСФО - Лицевой счет в финансовом органе"
	// И
	// в связанном контракте установлен признак "Требуется казначейскон сопровождени.
	...
	// и в связанном контракте установлен признак "Требуется казначейскон сопровождение".
	// См. НовыйДанныеОКТМО
	...
	Для каждого ТРУ Из ТоварыРаботыУслуги Цикл
		...
		Если ТРУ.ЭтоДетализированныеСведения Тогда
			...
			Для каждого ДочерняяПозиция Из ТРУ.СведенияОДочернихПозициях Цикл
				...
				// Выполнено с начала выполнения работ (без учета отчетного периода).
				ДобавитьДанныеСНачалаВыполненияРабот(УзелДочернейПозиции, ДочерняяПозиция);
		...
		Иначе
			...
			Иначе
				...
				// Выполнено с начала выполнения работ (без учета отчетного периода).
				ДобавитьДанныеСНачалаВыполненияРабот(УзелСведенийНеЛекарство, ТРУ);
...
Процедура ЗаполнитьЮрЛицоПодписантаЕИС(Подписант, ПространствоИменСхемы, Ошибки, Данные, ПрефиксТипа)
	...
	ЮЛ = ФорматыЭДО_ФНС.ПолучитьОбъектТипаCML(ПрефиксТипа + ".Подписант.ЮЛ", ПространствоИменСхемы);
	...
	ФИО = ФорматыЭДО_ФНС.ПолучитьОбъектТипаCML(ПрефиксТипа + ".Подписант.ЮЛ.ФИО", ПространствоИменСхемы);
...
Функция ПространствоИменВерсииСхем()
	...
	ПространстваВерсий.Вставить("12.1", "DP_PAKET_EIS_01_00_12_1");
	ПространстваВерсий.Вставить("12.2", "DP_PAKET_EIS_01_00_12_2");
	...
	Если ЕстьСвойство(ДанныеПодписантовОрганизации, "signersInfo") Тогда
		...
		Для каждого Подписант Из ЗначениеВКоллекцию(ДанныеПодписантов) Цикл
			...
			ИначеЕсли ЕстьСвойство(Подписант.signerType, "individualPersonInfo") Тогда
				ДанныеПодписанта.ИНН = Подписант.signerType.individualPersonInfo.INN;
