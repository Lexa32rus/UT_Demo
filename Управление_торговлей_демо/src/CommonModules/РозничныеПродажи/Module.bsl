...
Функция МестоРасчетовККТ(ОборудованиеККТ) Экспорт
	...
	ПараметрыРегистрацииККТ = ОборудованиеЧекопечатающиеУстройстваВызовСервера.ПараметрыРегистрацииУстройства(ОборудованиеККТ);
...
Процедура СформироватьДокументыДляОперацииЗакрытияКассовойСмены(СостояниеКассовойСмены, ОписаниеОшибки, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	// Заполним по каждому типу налогообложения пару документов: Отчет о розничных продажах, Отчет о розничных возвратах
	ПарыДокументовСформированы = ЗаполнитьОтчетыОРозничныхПродажахИВозвратах(СостояниеКассовойСмены, ОписаниеОшибки, ВключатьНепроведенныеЧеки);
...
Функция ПредставлениеУстройства(СтрокаТЧ)
	...
	ИначеЕсли СтрокаТЧ.ТипОборудования = Перечисления.ТипыОфлайнОборудования.ККМ Тогда
...
Процедура ЗаполнитьТаблицуОборудование(Форма, ПоддерживаемыеТипыПодключаемогоОборудования) Экспорт
	...
	Для Каждого СтрокаТЧ Из СписокОборудования Цикл
		Если СтрокаТЧ.ТипОборудования = "ККТ"
			Или СтрокаТЧ.ТипОборудования = "ПринтерЧеков" Тогда
			Продолжить;
		КонецЕсли;
		НоваяСтрока = Форма.Оборудование.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ);
		Попытка
			НоваяСтрока.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования[СтрокаТЧ.ТипОборудования];
		Исключение
			ТекстСообщения = НСтр("ru = 'В перечислении ""Типы подключаемого оборудования"" не найден тип оборудования: %1'");
			ЗаписьЖурналаРегистрации(СтрШаблон(ТекстСообщения, СтрокаТЧ.ТипОборудования), УровеньЖурналаРегистрации.Ошибка);
		КонецПопытки;
...
Функция ПараметрыРегистрацииУстройства(Знач ИдентификаторУстройства)
	...
	Возврат ОборудованиеЧекопечатающиеУстройстваВызовСервера.ПараметрыРегистрацииУстройства(ИдентификаторУстройства);
