...
Функция АдресСведенияПорталИТС() Экспорт
	...
	Возврат "https://portal.1c.ru/applications/90#prices";
...
Процедура ПересчитатьПроблемныеРеквизитыТаблицыДокумента(Форма, СтрокаТаблицы) Экспорт
	...
	Пока ИндексПроблемы > 0 Цикл
		...
		Если СтрокаПроблемы.ИмяРеквизита = "ИтогоСуммаСкидки" Или 
			СтрокаПроблемы.ИмяРеквизита = "ИтогоСумма"
...
Процедура УстановитьЦветПоляПоУмолчанию(Форма, ИмяЭлемента) Экспорт
	...
	Иначе
		Если Не ИмяЭлемента = "Номенклатура" Тогда
...
//	Если ИмяКоманды = "СоздатьРеализацияАкт" Тогда
