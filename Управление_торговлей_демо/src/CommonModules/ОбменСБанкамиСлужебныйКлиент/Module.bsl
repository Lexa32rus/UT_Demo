...
Процедура НачатьПодключениеКомпонентыСбербанк(ЭтоВерсияОСWindows7, Оповещение, Идентификатор, ПараметрыПодключения)
	...
	Если ЭтоВерсияОСWindows7 Тогда
		ПодключаемыйМодуль = ОбменСБанкамиСлужебныйКлиент.ЗначениеИзКешаСбербанк("VPNKeyTLS_1_2_9_6");
		...
		ОповещениеПослеПопыткиПодключения = Новый ОписаниеОповещения(
			...
			ДополнительныеПараметры);
		ПараметрыПодключения = ОбщегоНазначенияКлиент.ПараметрыПодключенияКомпоненты();
		ПараметрыПодключения.ТекстПояснения = НСтр("ru = 'Для подключения обмена со Сбербанком требуется
			|внешняя компонента ""VpnKey-TLS для 1С:Предприятия 8. Версия 1.2.9.6"".'");
		ОбщегоНазначенияКлиент.ПодключитьКомпонентуИзМакета(
			ОповещениеПослеПопыткиПодключения, 
			Идентификатор,
			"Обработка.ОбменСБанками.Макет.VPNKeyTLS_1_2_9_6",
			ПараметрыПодключения);
	Иначе
		ВнешниеКомпонентыКлиент.ПодключитьКомпоненту(
			Оповещение, 
...
Процедура ПослеПопыткиПодключенияКомпонентыVPNKeyTLS_1_2_9_6(Результат, ДополнительныеПараметры) Экспорт
	
	ИнициализацияУспешна = Ложь;
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("Подключено", Результат.Подключено);
	СтруктураВозврата.Вставить("ПодключаемыйМодуль", Результат.ПодключаемыйМодуль);
	СтруктураВозврата.Вставить("ОписаниеОшибки", Результат.ОписаниеОшибки);
	
	Если Результат.Подключено Тогда
		ЗакешироватьПараметрСбербанка("VPNKeyTLS_1_2_9_6", Результат.ПодключаемыйМодуль);
	...
	Иначе
		НастройкаОбмена = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ДополнительныеПараметры.ОповещениеПослеПодключения.ДополнительныеПараметры,
			"НастройкаОбмена");
		Если НастройкаОбмена <> Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'Компонента подключена, но не инициализирована.'");
			ТекстОшибки = Результат.ОписаниеОшибки;
			ВидОперации = НСтр("ru = 'Подключение компоненты VPNKeyTLS_1_2_9_6 для Сбербанка'");
	...
	Если ДополнительныеПараметры.ПервичноеПодключение Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеПопыткиУстановкиКомпонентыVPNKeyTLS_1_2_9_6", ЭтотОбъект, ДополнительныеПараметры);
		ПараметрыУстановки = ОбщегоНазначенияКлиент.ПараметрыУстановкиКомпоненты();
		ПараметрыУстановки.ТекстПояснения = НСтр("ru = 'Для подключения обмена со Сбербанком требуется
		|внешняя компонента ""VpnKey-TLS для 1С:Предприятия 8. Версия 1.2.9.6"".'");
		ОбщегоНазначенияКлиент.УстановитьКомпонентуИзМакета(
			Оповещение, 
			"Обработка.ОбменСБанками.Макет.VPNKeyTLS_1_2_9_6",
			ПараметрыУстановки);
	Иначе
...
Процедура ПослеПопыткиУстановкиКомпонентыVPNKeyTLS_1_2_9_6(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		НастройкаОбмена = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			...
			"НастройкаОбмена");
		Если НастройкаОбмена <> Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'Компонента не установлена.'");
			ТекстОшибки = Результат.ОписаниеОшибки;
			ВидОперации = НСтр("ru = 'Установка компоненты VPNKeyTLS_1_2_9_6 для Сбербанка'");
			ОбработатьОшибку(ВидОперации, ТекстОшибки, ТекстСообщения, НастройкаОбмена);	
		КонецЕсли;
	КонецЕсли;
