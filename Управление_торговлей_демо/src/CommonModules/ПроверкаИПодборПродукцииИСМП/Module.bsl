...
Процедура ПересчитатьХешСуммыВсехУпаковок(Форма) Экспорт
	...
	Для Каждого СтрокаДерева Из Форма.ДеревоМаркированнойПродукции.ПолучитьЭлементы() Цикл
		Если ИнтеграцияИСКлиентСервер.ЭтоУпаковка(СтрокаДерева.ТипУпаковки)
			...
			Или СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИСМП.ОбъемноСортовойУчет Тогда
...
Функция ВидПродукцииРодительный(ВидПродукции) Экспорт
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.ПустаяСсылка() Тогда
		Возврат НРег(ПолучитьСклоненияСтроки("Немаркируемая продукция", "Л=ru_RU", "ПД=Родительный")[0]);
	КонецЕсли;
...
Функция ВидПродукцииВинительный(ВидПродукции) Экспорт
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.ПустаяСсылка() Тогда
		Возврат НРег(ПолучитьСклоненияСтроки("Немаркируемая продукция", "Л=ru_RU", "ПД=Винительный")[0]);
	КонецЕсли;
...
Процедура УправлениеВидимостьюЭлементовПроверкиИПодбора(Форма) Экспорт
	...
	ПроверитьДобавлениеНеМаркируемойПродукции = Ложь;
	...
	Для Каждого ВидПродукции Из ПараметрыИнтеграции.ВидыПродукции Цикл
		...
		Если ВидимостьЭлементов И ВидПродукции = Перечисления.ВидыПродукцииИС.ПустаяСсылка() Тогда
			ПроверитьДобавлениеНеМаркируемойПродукции = Истина;
			Продолжить;
		КонецЕсли;
	...
	Если ПроверитьДобавлениеНеМаркируемойПродукции Тогда
		
		УправлениеЭлементамиОткрытияФормыПроверкиИПодбора(Форма, Перечисления.ВидыПродукцииИС.ПустаяСсылка(), ДобавленыЭлементыПодбора);
...
Процедура ДополнитьТаблицуПодобраннойМаркируемойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ВидПродукции, ПараметрыСканирования) Экспорт
	...
	ДополнительныеПараметры.Вставить("ЭтоУпаковкаОбъемноСортовогоУчета",           Неопределено);
