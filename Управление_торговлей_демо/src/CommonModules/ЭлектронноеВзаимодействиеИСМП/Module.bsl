...
//  РезультатРазбора  - Неопределено, (см. ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода) - Результат разбора.
// 
// Возвращаемое значение:
...
// 	 * Количество         - Число  - количество маркируемой продукеции по коду маркировки
//
...
Функция Содержимое(Знач ШтрихкодыУпаковок, Знач ВидыПродукцииИС = Неопределено, ДоКомплектующих = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("ПустаяНоменклатура", ИнтеграцияИС.НезаполненныеЗначенияОпределяемогоТипа("Номенклатура"));
	Запрос.УстановитьПараметр("ДоКомплектующих", Истина);
	...
	Если Не ДоКомплектующих Тогда
	  Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДоКомплектующих",
	  "(ШтрихкодыУпаковок.Номенклатура В(&ПустаяНоменклатура)
	|			ИЛИ (ШтрихкодыУпаковок.Номенклатура = Вложенные.Штрихкод.Номенклатура
	|				И ШтрихкодыУпаковок.Характеристика = Вложенные.Штрихкод.Характеристика))");
	КонецЕсли;
