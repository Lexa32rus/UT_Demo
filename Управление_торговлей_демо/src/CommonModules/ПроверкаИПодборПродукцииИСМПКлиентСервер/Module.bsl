...
Процедура ПодобраннаяПродукцияПриДобавленииСтроки(Форма, СтрокаИзменений, Знач КоличествоКодовМаркировки, МодульКонтекста, ДополнительныеПараметры = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если ЭтоЗагрузкаДанныхДокументаПриОткрытииФормы И УточнитьКоличествоИС Тогда
		ЭтоУпаковкаОбъемноСортовогоУчета = Ложь;
	ИначеЕсли Форма.ПоддерживаетсяОбъемноСортовойУчет
		И ДополнительныеПараметры <> Неопределено
		И ДополнительныеПараметры.Свойство("ЭтоУпаковкаОбъемноСортовогоУчета") Тогда
		ЭтоУпаковкаОбъемноСортовогоУчета = ДополнительныеПараметры.ЭтоУпаковкаОбъемноСортовогоУчета;
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаИзменений, "ВидУпаковки") Тогда
		ЭтоУпаковкаОбъемноСортовогоУчета =
			СтрокаИзменений.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет");
	Иначе
		ЭтоУпаковкаОбъемноСортовогоУчета = Ложь;
	КонецЕсли;
	...
	ИначеЕсли СтрокаАгрегатор1 <> Неопределено И СтрокаАгрегатор2 <> Неопределено Тогда
		...
		ПараметрыУточнения.Вставить("ЭтоЗагрузкаДанныхДокументаПриОткрытииФормы",          ЭтоЗагрузкаДанныхДокументаПриОткрытииФормы);
...
Процедура ПодобраннаяПродукцияПриУдаленииСтроки(Форма, СтрокаИзменений, Знач КоличествоКодовМаркировки, МодульКонтекста, ДополнительныеПараметры = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	ПересчитатьПотребительскиеУпаковкиВСтруктуреУпаковок = ДополнительныеПараметры <> Неопределено
		И ДополнительныеПараметры.Свойство("УточнитьКоличествоИС")
		И ДополнительныеПараметры.УточнитьКоличествоИС
		И ДополнительныеПараметры.Свойство("ЭтоОбработчикУстановкиСтатуса")
			...
			Если СтрокаМаркируемойПродукции.КодыGTIN.Количество() = 1 Тогда
				...
				Если СвойстваВидаПродукции.ПроверятьСвойстваВидаПродукции Тогда
					...
					ИначеЕсли СвойстваВидаПродукции.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукцияБезВЕТИС") Тогда
						СтрокаАктуальна = ?(СтрокаМаркируемойПродукции.ГоденДо = '00010101', СтрокаАктуальна, Ложь);
					ИначеЕсли СвойстваВидаПродукции.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы") Тогда
						СтрокаАктуальна = ?(СтрокаМаркируемойПродукции.ГоденДо = '00010101', СтрокаАктуальна, Ложь);
					КонецЕсли;
	...
	Иначе
		...
		Если УточнитьТолькоКоличествоИС Тогда
			...
			Если ДанныеРаспределения <> Неопределено Тогда
				...
				Если ДанныеРаспределения.КоличествоКРаспределению > 0 И СтрокаАгрегатор.КоличествоПотребительскихУпаковокВСтруктуреУпаковок > 0 Тогда
		...
		Иначе
			...
			ПараметрыУточнения = Новый Структура;
			ПараметрыУточнения.Вставить("УточнитьКоличествоИС",          Не Форма.РежимПодбораСуществующихУпаковок);
			ПараметрыУточнения.Вставить("ЭтоОбработчикУстановкиСтатуса", Ложь);
...
Процедура ПодобраннаяПродукцияПриСопоставлениеНоменклатурыШтрихкодам(Форма, СтрокаИзменений, МодульКонтекста, ДополнительныеПараметры = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Для Каждого СтрокаМаркируемойПродукции Из ПодобраннаяМаркируемаяПродукция Цикл
		...
		Если СтрокаМаркируемойПродукции.Номенклатура <> СтрокаИзменений.Номенклатура
...
Функция РекомендуемыйРежимДобавленияКодовМаркировки(Форма) Экспорт
	...
	ИначеЕсли Не ПроверкаИПодборПродукцииИСМПКлиентСервер.ЭтоДокументРеализации(Форма.ПроверяемыйДокумент) Тогда
...
Процедура ОпределитьТипУпаковки(СтрокаДерева, ВключаяПодчиненные, ПараметрыЗаполнения, МодульКонтекста)
	...
	Если ПодчиненныеСтроки.Количество() = 0 Тогда
		...
		ИначеЕсли (СтрокаДерева.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Логистическая") Или ЭтоКодОСУ)
