...
// Возвращает пустую структуру данных RFID.
//
// Возвращаемое значение:
//  Структура:
//   * TID                 - Строка - TID
...
Функция ИнициализироватьДанныеRFID(СырыеДанные = Неопределено) Экспорт
	...
	Если СырыеДанные <> Неопределено Тогда
		...
		РезультатПроверки = ШтрихкодированиеИСКлиентСервер.ТребуетсяИндивидуализацияRFID(СырыеДанные, СырыеДанные.GTIN);
...
Функция СтатусRFID(ДанныеRFID, GTIN) Экспорт
	...
	Если ДанныеRFID = Неопределено
		Или Не ЗначениеЗаполнено(ДанныеRFID.TID) Тогда
		
		СтатусRFID = 0;
		
	Иначе
		
		ТребуетсяИндивидуализацияRFID = ШтрихкодированиеИСКлиентСервер.ТребуетсяИндивидуализацияRFID(ДанныеRFID, GTIN).НужноЗаписатьМетку;
