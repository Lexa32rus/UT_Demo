...
Процедура ОбработкаПроверкиЗаполнения(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты) Экспорт 
	...
	ПроверкаЗаполненияМаркируемойПродукции(ЭтотОбъект, Отказ);
...
// 
// Параметры:
//  ЭтотОбъект - ДокументОбъект.ПриходныйОрдерНаТовары -
//  ДанныеЗаполнения - Структура -
//  СтандартнаяОбработка - Булево -
Процедура ОбработкаЗаполнения(ЭтотОбъект, ДанныеЗаполнения, СтандартнаяОбработка)Экспорт

	//++ Локализация
	Если НЕ ЭтотОбъект.ЭтоНовый()
		ИЛИ НЕ ИнтеграцияИСМП.ИспользуетсяМаркируемаяПродукция() 
	...
	ЗаполнитьШтрихкодыУпаковокПоРаспоряжению(ЭтотОбъект, ДанныеЗаполнения);
...
Процедура ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка) Экспорт
	...
	//++ Локализация
	ЗаполнитьКешированныеЗначения(ЭтотОбъект);
	//-- Локализация
...
Процедура ПроверкаЗаполненияМаркируемойПродукции(ЭтотОбъект, Отказ)
	...
	Если ТипЗнч(ЭтотОбъект) <> Тип("ДокументОбъект.ПриходныйОрдерНаТовары")
		ИЛИ СтатусыКонтроля.Найти(ЭтотОбъект.Статус) = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	
	Если НЕ ИнтеграцияИСМП.ИспользуетсяМаркируемаяПродукция() Тогда
		Возврат;
	КонецЕсли; 
...
//	ЭтотОбъект - ДокументОбъект.ПриходныйОрдерНаТовары -
//	ДанныеЗаполнения - Структура - 
Процедура ЗаполнитьШтрихкодыУпаковокПоРаспоряжению(ЭтотОбъект, ДанныеЗаполнения)
	
	ТаблицаШтрихкодыУпаковок = ЭтотОбъект.ШтрихкодыУпаковок;
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыПроверкиИПодбораДокументовИСМП КАК СтатусыПроверкиИПодбораДокументовИСМП
	|		ПО УпаковкиОрдеров.ПриходныйОрдер = СтатусыПроверкиИПодбораДокументовИСМП.Документ
	|ГДЕ
	|	СтатусыПроверкиИПодбораДокументовИСМП.СтатусПроверкиИПодбора = ЗНАЧЕНИЕ(Перечисление.СтатусыПроверкиИПодбораИС.Завершено)
	|	И ВЫБОР
	|			КОГДА УпаковкиОрдеров.ЭтоМультитоварнаяУпаковка
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ УпаковкиОрдеров.ВидПродукцииИС = СтатусыПроверкиИПодбораДокументовИСМП.ВидМаркируемойПродукции
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
