...
Функция ВерсияПриложения() Экспорт
	...
	Если МетаданныеИмя = "УправлениеПредприятием" Тогда
		ИмяПрограммы = НСтр("ru = '1С:ERP Управление предприятием'");
	Иначе
...
Процедура СтавкаНДСДляПриложенияЕИС(Знач СтавкаНДС, СтавкаНДСПриложения) Экспорт
	...
	Если ТипЗнч(СтавкаНДС) = Тип("СправочникСсылка.СтавкиНДС") Тогда
		Если СтавкаНДС <> Справочники.СтавкиНДС.БезНДС Тогда
			СтавкаНДСПриложения = СтавкаНДС.Наименование;
		Иначе
...
Процедура ГосударственныйКонтрактДокумента(Документ, ГосударственныйКонтракт) Экспорт
	...
	ГосударственныйКонтракт = ЭлектронноеАктированиеЕИС.ГосударственныйКонтрактДоговора(Документ.Договор);
...
//
Процедура УстановкаВидимостиГруппыГосконтракта(Форма, ЭтоЗаказКлиента = Ложь) Экспорт
...
Процедура ЗаполнитьДанныеДереваДляЭлектронногоАктированияЕИС(ДеревоДанных,
	...
	Если ЗначениеЗаполнено(ВыборкаШапки.Менеджер) Тогда
		...
		ОтветственныеЗаОформление.Добавить(Менеджер.ФИО);
	КонецЕсли;
...
// Полное наименование поставщика в гос. контракте.
// 
// Параметры:
//  Договор - СправочникСсылка.ГосударственныеКонтракты
// 
// Возвращаемое значение:
//  Строка - полное наименование поставщика.
...
//
Процедура ДоговорПриИзмененииЭлектронноеАктированиеЕИС(Форма, Объект) Экспорт
	
	УстановкаВидимостиГруппыГосконтракта(Форма);
