...
Процедура ЗаполнитьКешШтрихкодовУпаковок(Форма, Настройки = Неопределено, Инициализация = Ложь) Экспорт
	...
	Если Не Настройки.ВключаяКомплектующие Тогда
		...
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст, "&НеВключаяКомплектующие",
			"(ШтрихкодыУпаковок.Номенклатура В(&ПустаяНоменклатура)
			|	ИЛИ (ЕстьВложенныеШтрихкоды.Штрихкод.Номенклатура = ШтрихкодыУпаковок.Номенклатура
			|		И ЕстьВложенныеШтрихкоды.Штрихкод.Характеристика = ШтрихкодыУпаковок.Характеристика))");
	КонецЕсли;
	Запрос.УстановитьПараметр("НеВключаяКомплектующие", Истина);
...
Процедура РассчитатьХешСуммыУпаковки(СтрокаДерева, ТаблицаХешСумм, ВключаяПодчиненные, ЕстьКолонкаСтатусПроверки = Истина, ЕстьКолонкаХешСуммаНормализации = Неопределено, ИмяПоляКоличество = Неопределено) Экспорт
	...
	Для Каждого ПодчиненнаяСтрока Из ПодчиненныеСтрокиДерева Цикл
		...
		Если ИнтеграцияИСКлиентСервер.ЭтоУпаковка(ПодчиненнаяСтрока.ТипУпаковки)
			Или ПодчиненнаяСтрока.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИСМП.ГруппировкаОбъемноСортовогоУчетаПоGTIN Тогда
	...
	Если ИмяПоляКоличество <> Неопределено
		...
		Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
			...
			СтрШаблон("%1|%2|%3|%4|%5",
				ЗначениеШтрихкода,
				Формат(СтрокаДерева.Количество, "ЧГ=0;"),
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоУпаковка(СтрокаДерева.ТипУпаковки)
		Или СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИСМП.ГруппировкаОбъемноСортовогоУчетаПоGTIN Тогда
...
Функция ИмяКнопкиИКоманды(ВидПродукции = Неопределено, ВозобновлениеПроверки = Ложь) Экспорт
	...
	Если ВидПродукции<>Неопределено Тогда
		Если ВидПродукции.Пустая() Тогда
			Индекс = 99;
		Иначе
