...
Функция ТекстСуммыПрочихРасходов() Экспорт
	...
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
	|					)
	|				И НЕ ЕСТЬNULL(ВтОВЗ.ВариантРаспределенияРасходовРегл, Статьи.ВариантРаспределенияРасходовРегл) 
	|					В (	
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
	|					)
	|				ТОГДА -(ДД.СуммаРегл - ДД.ПостояннаяРазница)
	|			КОГДА НЕ ЕСТЬNULL(ВтОВЗ.ВариантРаспределенияРасходовНУ, Статьи.ВариантРаспределенияРасходовНУ) 
	|					В (	
	|						ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
	|					)
	|				ТОГДА ДД.СуммаРегл - ДД.ПостояннаяРазница
...
Процедура СкорректироватьСтоимостьСписанияЗапасов(ПараметрыРасчета) Экспорт
	...
	#Область ВТНачальныеОстаткиСебестоимостиПоКорректировкам
	
		Если НЕ РасчетСебестоимостиПрикладныеАлгоритмы.ВременнаяТаблицаСуществует(ПараметрыРасчета, "ВТНачальныеОстаткиСебестоимостиПоКорректировкам") Тогда
			Запрос.Текст = Запрос.Текст + РасчетСебестоимостиРешениеСЛУ.НачальныеОстаткиСебестоимостиПоКорректировкам() + ";";
		КонецЕсли;
	...
	|	 		- ТаблицаКорректировки.РезервПодОбесценениеРегл КОНЕЦ) КАК РезервПодОбесценениеРегл,
	|
	|	(ВЫБОР
	...
	|	 		- ТаблицаКорректировки.РезервПодОбесценениеУпр КОНЕЦ) КАК РезервПодОбесценениеУпр,
	|
	...
	|		И ТаблицаКорректировки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноРеализации)
...
#Область ПартииПрочихРасходовРазвернутоеСальдо
	...
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров))
	|		 И Т.СтатьяРасходов.ВариантРаздельногоУчетаНДС <> ЗНАЧЕНИЕ(Перечисление.ВариантыРаздельногоУчетаНДС.Распределение))
	|	И (Т.Сумма <> 0 ИЛИ Т.СуммаБезНДС <> 0 ИЛИ Т.СуммаРегл <> 0 ИЛИ Т.СуммаУпр <> 0)
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	СтатьяРасходов,
	|	АналитикаРасходов,
