...
Функция ВерсияФФДОборудования(ПодключаемоеОборудование) Экспорт
	...
	Иначе
		...
		ПараметрыРегистрацииУстройства = ОборудованиеЧекопечатающиеУстройстваВызовСервера.ПараметрыРегистрацииУстройства(ПодключаемоеОборудование);
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Для каждого СтрокаДляДобавленияВТовары из МассивСтрокДляДобавленияВТовары Цикл
			...
			Если СтрокаДляДобавленияВТовары.КоличествоУпаковок * СтрокаДляДобавленияВТовары.КоэффициентПересчетаУпаковки > СтрокаТЧ.Количество
				...
				СтрокаТоваровСРаспределением.Цена               = (СтрокаТЧ.СуммаСНДС + СтрокаТЧ.СуммаСкидки) / СтрокаТЧ.КоличествоУпаковок;
			Иначе
	...
	Если ТаблицаДокументовЕдиногоЧека.Количество() > 0 Тогда
		Для Каждого СтрокаОбъектыРасчетов Из ОбъектыРасчетов Цикл
			Если ДокументПоставкиСВозможностьюПробитияЕдиногоЧека(СтрокаОбъектыРасчетов.Заказ) Тогда
				...
				СтрокаДокументЕдиногоЧека.Сумма = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаОбъектыРасчетов.Заказ, "СуммаДокумента");
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;
...
Функция РассчитатьСтруктуруОплатДокументаПоставки(ПризнакиСпособаРасчетаАвто, СуммаДокумента, ПризнакСпособаРасчетаЗаданВручную = Ложь, УточнениеТипаОплатПоСуммам = Неопределено)
	...
	Иначе
		...
		СуммаПредоплаты = ?(ПризнакиСпособаРасчетаАвто.Количество() > 0, ПризнакиСпособаРасчетаАвто[0].ДолгОрганизацииНаНачало, 0);
