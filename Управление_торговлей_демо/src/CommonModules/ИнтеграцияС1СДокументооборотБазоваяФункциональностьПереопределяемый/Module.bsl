...
Процедура ПередДобавлениемКомандПечати(ИмяФормы, КомандыПечати, СтандартнаяОбработка) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьИнтеграциюС1СДокументооборот") Тогда
		Если ИмяФормы = Метаданные.Документы.КоммерческоеПредложениеКлиенту.Формы.ФормаДокумента.Имя
				Или ИмяФормы = Метаданные.Документы.КоммерческоеПредложениеКлиенту.Формы.ФормаСписка.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаказКлиента.Формы.ФормаДокумента.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаказКлиента.Формы.ФормаСписка.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаказКлиента.Формы.ФормаСпискаДокументов.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаказПоставщику.Формы.ФормаДокумента.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаказПоставщику.Формы.ФормаСписка.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаказПоставщику.Формы.ФормаСпискаДокументов.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаявкаНаВозвратТоваровОтКлиента.Формы.ФормаДокумента.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаявкаНаВозвратТоваровОтКлиента.Формы.ФормаСписка.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаявкаНаВозвратТоваровОтКлиента.Формы.ФормаСпискаДокументов.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаявкаНаРасходованиеДенежныхСредств.Формы.ФормаДокумента.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаявкаНаРасходованиеДенежныхСредств.Формы.ФормаСписка.Имя
				Или ИмяФормы = Метаданные.Документы.ЗаявкаНаРасходованиеДенежныхСредств.Формы.ФормаСпискаДокументов.Имя Тогда
			КомандаПечати = КомандыПечати.Добавить();
			КомандаПечати.МенеджерПечати = "Обработка.ИнтеграцияС1СДокументооборот";
			КомандаПечати.Идентификатор = "ЛистСогласования";
			КомандаПечати.Представление = НСтр("ru = 'Документооборот: лист согласования'");
...
Процедура ПриОпределенииЗначенияРеквизитаПоОбъектуXDTO(Результат, ЗначениеЗаполнения, ТипРеквизита, ИмяРеквизита,
	...
	Если ТипРеквизита = Тип("СправочникСсылка.Контрагенты")
		...
		Результат = РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.СсылкаНаОбъектПоДаннымДокументооборота(
