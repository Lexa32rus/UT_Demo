...
Процедура РеквизитыПризнаковОсобенностейУчета(РеквизитыПризнаковОсобенностейУчета, МетаданныеОбъекта) Экспорт
...
Процедура ПриДобавленииОбработчиковОбновленияСерий(Обработчики) Экспорт

	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "НоменклатураЛокализация.ОбработатьСерииГИСМ";
	Обработчик.Версия = "2.5.8.264";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("287704f8-1bf1-43cf-94a8-d88cd9af929d");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "НоменклатураЛокализация.ЗарегистрироватьСерииГИСМ";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Выделение из серий номенклатуры, являющихся кодами маркировки изделий из натурального меха,
	                                    |штрихкодов упаковок товаров и данных по RFID-меткам'");
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.Справочники.СерииНоменклатуры.ПолноеИмя());
	Читаемые.Добавить(Метаданные.Справочники.ШтрихкодыУпаковокТоваров.ПолноеИмя());
	Читаемые.Добавить(Метаданные.РегистрыСведений.ДанныеRFIDИСМП.ПолноеИмя());
	Читаемые.Добавить(Метаданные.РегистрыНакопления.ДвиженияСерийТоваров.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Справочники.ШтрихкодыУпаковокТоваров.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ДанныеRFIDИСМП.ПолноеИмя());
...
Процедура ЗарегистрироватьСерииГИСМ(Параметры) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров КАК СозданныеШтрихкоды
	|		ПО СозданныеШтрихкоды.ЗначениеШтрихкода = СерииНоменклатуры.НомерКизГИСМ
	|ГДЕ
	|	СерииНоменклатуры.НомерКиЗГИСМ <> """"
	|	И СозданныеШтрихкоды.Ссылка ЕСТЬ NULL";
	
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
	
КонецПроцедуры

Процедура ОбработатьСерииГИСМ(Параметры) Экспорт
	
	ПолноеИмяОбъекта = "Справочник.СерииНоменклатуры";
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СерииНоменклатуры.Ссылка КАК Ссылка,
	|	СерииНоменклатуры.НомерКиЗГИСМ        КАК ЗначениеШтрихкода,
	...
	|ИЗ
	|	&ВременнаяТаблицаСсылок КАК СсылкиДляОбработки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК СерииНоменклатуры
	|		ПО СсылкиДляОбработки.Ссылка = СерииНоменклатуры.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеRFIDИСМП КАК ДанныеRFIDИСМП
	|		ПО ДанныеRFIDИСМП.RFIDTID = СерииНоменклатуры.RFIDTID
	|ГДЕ
	|	СерииНоменклатуры.RFIDTID <> """"
	...
	|		ПО СозданныеШтрихкоды.ЗначениеШтрихкода = СерииНоменклатуры.НомерКизГИСМ
	|ГДЕ
	|	СозданныеШтрихкоды.Ссылка ЕСТЬ NULL
	|ИНДЕКСИРОВАТЬ ПО
	|	Серия
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СерииБезШтрихкодов.Серия КАК Серия,
	|	МАКСИМУМ(СерииБезШтрихкодов.ЗначениеШтрихкода) КАК ЗначениеШтрихкода,
	|	МАКСИМУМ(ДвиженияСерийТоваров.Период) КАК Период
	|ПОМЕСТИТЬ ДатыДвиженияСерий
	|ИЗ
	|	РегистрНакопления.ДвиженияСерийТоваров КАК ДвиженияСерийТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СерииБезШтрихкодов КАК СерииБезШтрихкодов
	|		ПО СерииБезШтрихкодов.Серия = ДвиженияСерийТоваров.Серия
	|ГДЕ
	|	ДвиженияСерийТоваров.Количество <> 0
	|СГРУППИРОВАТЬ ПО
	|	СерииБезШтрихкодов.Серия
	|ИНДЕКСИРОВАТЬ ПО
	|	Серия,
	|	Период
	|;
	...
	|	ДатыДвиженияСерий.ЗначениеШтрихкода КАК Штрихкод,
	|	ДвиженияСерийТоваров.Номенклатура КАК Номенклатура,
	|	ДвиженияСерийТоваров.Характеристика КАК Характеристика,
	|	1 КАК Количество,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха) КАК ВидПродукции,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыУпаковок.МаркированныйТовар) КАК ТипУпаковки
	|ИЗ
	|	ДатыДвиженияСерий КАК ДатыДвиженияСерий
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДвиженияСерийТоваров КАК ДвиженияСерийТоваров
	|		ПО ДвиженияСерийТоваров.Серия = ДатыДвиженияСерий.Серия
	|		И ДатыДвиженияСерий.Период = ДвиженияСерийТоваров.Период
	|		И ДвиженияСерийТоваров.Количество > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	&ВременнаяТаблицаСсылок КАК СсылкиДляОбработки";
	Запрос.Текст = СтрЗаменить(запрос.Текст, "&ВременнаяТаблицаСсылок", Результат.ИмяВременнойТаблицы);
	
	КэшСтатусовУказанияСерий = Новый Соответствие;
	
	Пакет = Запрос.ВыполнитьПакет();
	ДанныеRFIDИСМП = Пакет[0].Выгрузить();
	ДанныеRFIDИСМП.Индексы.Добавить("Ссылка");
	
	ДанныеШтрихкодовУпаковок = Пакет[3].Выгрузить();
	ДанныеШтрихкодовУпаковок.Индексы.Добавить("Серия");
	...
	Пока Выборка.Следующий() Цикл 
		...
		Попытка
			...
			ДанныеRFID = ДанныеRFIDИСМП.Найти(Выборка.Ссылка, "Ссылка");
			Если ДанныеRFID <> Неопределено Тогда
				Элемент = РегистрыСведений.ДанныеRFIDИСМП.НовыйЭлементЗаписиДанных();
				ЗаполнитьЗначенияСвойств(Элемент, ДанныеRFID);
				ЗаписываемыеRFID = Новый Массив;
				ЗаписываемыеRFID.Добавить(Элемент);
				РегистрыСведений.ДанныеRFIDИСМП.ЗаписатьДанные(ЗаписываемыеRFID);
			КонецЕсли;
			
			ДанныеШтрихкодов = ДанныеШтрихкодовУпаковок.Найти(Выборка.Ссылка, "Серия");
			Если ДанныеШтрихкодов <> Неопределено Тогда 
				Справочники.ШтрихкодыУпаковокТоваров.СоздатьШтрихкодУпаковки(ДанныеШтрихкодов,,, КэшСтатусовУказанияСерий);
			КонецЕсли;
