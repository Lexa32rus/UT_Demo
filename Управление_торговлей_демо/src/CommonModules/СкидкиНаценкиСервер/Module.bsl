...
Функция РассчитатьПоРеализацииТоваровУслугПоЗаказу(Объект)
	...
	Для Каждого ВыборкаТовары Из ТаблицаТоваров Цикл
		...
		ТекСуммаНачисленныхБонусныхБалловВВалюте 	= Окр(ВыборкаТовары.ЗаказСуммаНачисленныхБонусныхБалловВВалюте * (ВыборкаТовары.РеализацияКоличество / ВыборкаТовары.ЗаказКоличество) * ВыборкаТовары.ВалютаКоэффициентПересчета, 2);
		ТекСуммаБонусныхБалловКСписаниюВВалюте 		= Окр(ВыборкаТовары.ЗаказСуммаБонусныхБалловКСписаниюВВалюте * (ВыборкаТовары.РеализацияКоличество / ВыборкаТовары.ЗаказКоличество) * ВыборкаТовары.ВалютаКоэффициентПересчета, 2);
		ТекСуммаБонусныхБалловКСписанию 			= Окр(ВыборкаТовары.ЗаказСуммаБонусныхБалловКСписанию * (ВыборкаТовары.РеализацияКоличество / ВыборкаТовары.ЗаказКоличество), 2);
...
Процедура ПрименитьРезультатРасчетаПоБонуснымБаллам(Объект, 
	...
	Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
		...
		Если ИспользоватьОплатуБонуснымиБаллами ИЛИ ИспользуетсяОдновременноеСписаниеИНачисление Тогда
