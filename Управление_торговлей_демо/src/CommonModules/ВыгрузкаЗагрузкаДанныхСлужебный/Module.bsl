...
Функция ЗагрузитьДанныеТекущейОбластиИзАрхива(Знач ДанныеФайла, Знач ПараметрыЗагрузки) Экспорт
	...
	Если ТипРезервнойКопии(ПараметрыЗагрузки) <> "Дифференциальная" И НЕ ЭтоПродолжениеЗагрузки() Тогда
	...
	Если ПараметрыЗагрузки.Свойство("ДанныеРасширений")
		И ПараметрыЗагрузки.ДанныеРасширений.Свойство("КаркасыРасширенийДляВосстановления", КаркасыРасширенийДляВосстановления) 
		И Не РаботаВМоделиСервиса.РазделениеВключено() Тогда
		
		ЕстьУстановленныеРасширения = ВосстановитьКаркасыРасширений(КаркасыРасширенийДляВосстановления);
...
Функция НачатьЗагрузкуДанныхТекущейОбластиИзАрхива(Знач ДанныеФайла, Знач ПараметрыЗагрузки, 
	...
	Если ТипРезервнойКопии(ПараметрыЗагрузки) = "Полная" Тогда
		Для ОбратныйИндекс = 1 - ПараметрыЗагрузки.ЗагружаемыеТипы.Количество() По 0 Цикл
			ОбъектМетаданных = ПараметрыЗагрузки.ЗагружаемыеТипы[-ОбратныйИндекс];
			Если Метаданные.ПланыОбмена.Содержит(ОбъектМетаданных)
				...
				ПараметрыЗагрузки.ЗагружаемыеТипы.Удалить(-ОбратныйИндекс);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если ВозможноПродолжениеПроцедурыЗагрузки(ХешСуммаИсточника, ХешСуммаПараметров) Тогда
		СоответствиеСсылок = ПродолжитьПроцедуруЗагрузки(
			ПараметрыЗагрузки,
			Контейнер,
			Обработчики);
		РезультатЗагрузки = Новый Структура("Предупреждения, СоответствиеСсылок", Контейнер.Предупреждения(), СоответствиеСсылок);
	...
	СтрогаяПроверка = ТипРезервнойКопии(ПараметрыЗагрузки) <> "Полная";
	...
	Если ТипРезервнойКопии(ПараметрыЗагрузки) <> "Дифференциальная" Тогда
	...
	СоответствиеСсылок = ЗагрузитьДанныеИнформационнойБазы(
		ПараметрыЗагрузки,
		Контейнер,
		Обработчики,
		Неопределено,
...
Функция ВосстановитьКаркасыРасширений(РасширенияДляВосстановления)
	...
	Для Каждого РасширениеДляВосстановления Из РасширенияДляВосстановления Цикл
		...
		ВосстанавливаемоеРасширение = РасширенияКонфигурации.Создать();
		ВосстанавливаемоеРасширение.БезопасныйРежим = Истина;
