...
// 
// Параметры:
//  ОписаниеСобытия - Структура - данные о событии оповещения.
...
Процедура ФискализацияЧековВОчереди_ЗавершениеПакетнойОперации(РезультатВыполнения, ОбщиеПараметры) Экспорт
	...
	Если РезультатВыполнения.РезультатФискализацииЧека <> Неопределено 
		...
		РезультатВыполнения.Вставить("ДополнительныеПараметры", РезультатВыполнения.РезультатФискализацииЧека.ДополнительныеПараметры);
	КонецЕсли;
...
#Область ВременныеАдаптерыДляПоддержкиБПО_2_1_6

// Устарела. Следует использовать ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьФискализациюЧекаНаФискальномУстройстве()
...
//   ИдентификаторУстройства - СправочникСсылка.ПодключаемоеОборудование - идентификатор устройства, если неопределенно - будет предложен выбор.
//   ТипОборудования         - Неопределено, ПеречислениеСсылка.ТипыПодключаемогоОборудования - Типы подключаемого оборудования.
//   ОповещениеПослеОткрытииЧека - ОписаниеОповещения - оповещение после открытия чека.
//   ОповещениеПриОшибкеПечатиЧека - ОписаниеОповещения - оповещение при ошибке печати чека.
//
...
// Устарела. Следует использовать ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьИнкассациюНаФискальномУстройстве()
...
//   ТипОборудования         - Неопределено, ПеречислениеСсылка.ТипыПодключаемогоОборудования - Типы подключаемого оборудования.
//
Процедура НачатьИнкассациюНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации,
	ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
