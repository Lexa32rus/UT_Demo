...
// Конструктор структу(ры результата установки действий в заказе
// 
// Возвращаемое значение:
//  Структура - структура с полями
...
Функция АдресТаблицыДвиженийВХранилище(Форма) Экспорт
	...
	Запрос.Текст = ВременнаяТаблицаДвиженийДокумента(Истина, ЭтоПроизводительныйРежим);
	Запрос.УстановитьПараметр("Ссылка", Неопределено);
	Запрос.УстановитьПараметр("СсылкаНовогоНазначения", Справочники.Назначения.ПолучитьСсылку());
	Запрос.Выполнить();
...
Функция ТекстЗапросаДоступныхОстатковПоДатамДляПодбора(ЕстьХарактеристика, СоставРазделовЗапроса) Экспорт
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		...
		ТекстЗапроса =
			...
			|	СУММА(Набор.Состояние) КАК Состояние
			|ПОМЕСТИТЬ Остатки
			|ИЗ(
			|	ВЫБРАТЬ
			...
			|				И Характеристика = &ХарактеристикаПараметр
			|				И Склад В(&Склады)) КАК Запасы
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			...
			|	ГДЕ
			|		РаспределениеЗапасов.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление)
			...
			|			И РаспределениеЗапасов.Склад В(&Склады)) КАК Набор
			|СГРУППИРОВАТЬ ПО
			|	Набор.Склад,
			|	Набор.Состояние,
			|	Набор.Период
		...
		Если Не ЕстьХарактеристика Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ХарактеристикаПараметр", "Характеристика");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Характеристика", "РаспределениеЗапасов.Характеристика");
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ХарактеристикаПараметр", "&Характеристика");
		КонецЕсли;
		
		СоставРазделовЗапроса.Добавить("ВременнаяТаблицаПланируемыеОстатки");
...
Функция РаспределениеЗапасовДляРедактируемогоДокумента(Форма, ОтборСостояний = Неопределено) Экспорт
	...
	ДобавитьВременнуюТаблицуДанныхДокумента(Запрос.МенеджерВременныхТаблиц, Таблицы, ПараметрыВстраивания);
	ЭтоПроизводительныйРежим = РаспределениеЗапасов.ЭтоПроизводительныйРежим();
	Запрос.Текст = ВременнаяТаблицаДвиженийДокумента(Ложь, ЭтоПроизводительныйРежим);
	Запрос.УстановитьПараметр("Ссылка", Неопределено);
	Запрос.УстановитьПараметр("СсылкаНовогоНазначения", Справочники.Назначения.ПолучитьСсылку());
	Запрос.Выполнить();
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|	Записи.Состояние В(&ОтборСостояний)
			|		И Записи.ЗаказНаОтгрузку В(&Заказы)
			|		И &ПроизводительныйРежим";
		
		ПроизводительныйРежимТекст = "ИСТИНА";
		ЭтоПроизводительныйРежим = РаспределениеЗапасов.ЭтоПроизводительныйРежим();
		Если ЭтоПроизводительныйРежим Тогда
			ПроизводительныйРежимТекст = "Записи.Состояние <> ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)";
			Тексты = Новый Массив();
		...
		Запрос.Текст = СтрЗаменить(Текст, "&ПроизводительныйРежим", ПроизводительныйРежимТекст);
...
// Возвращаемое значение:
//  Булево - Истина, если производительный режим используется.
Функция ЭтоПроизводительныйРежим() Экспорт
	
	Возврат РаспределениеЗапасов.ЭтоПроизводительныйРежим();
...
Функция ВременнаяТаблицаСвободныеОстатки(ИмяТаблицыФильтра, ИмяТаблицыРезультат) Экспорт
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|	Запасы.Склад КАК Склад,
			|	Запасы.Назначение КАК Назначение,
			|	Запасы.Свободно КАК Количество
			|ПОМЕСТИТЬ ИмяТаблицыРезультатПереопределяемый
			|ИЗ
			|	РегистрСведений.РаспределениеЗапасов КАК Запасы
			|ГДЕ
			|	Запасы.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)
			|		И Запасы.Свободно > 0
			|		И ИСТИНА В(
			|			ВЫБРАТЬ ПЕРВЫЕ 1
			|				ИСТИНА КАК ЕстЗаписи
			|			ИЗ
			|				ИмяТаблицыФильтраПереопределяемый КАК Фильтр
			|			ГДЕ
...
Функция ВременнаяТаблицаОстаткиВНаличииПоВсемНазначениям(ИмяТаблицыФильтра, ИмяТаблицыРезультат) Экспорт
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|	Запасы.Характеристика КАК Характеристика,
			|	Запасы.Склад КАК Склад,
			|	Запасы.ВНаличии КАК Количество
			|ПОМЕСТИТЬ ИмяТаблицыРезультатПереопределяемый
			|ИЗ
			|	РегистрСведений.РаспределениеЗапасов КАК Запасы
			|ГДЕ
			...
			|					И Фильтр.Характеристика = Запасы.Характеристика
			|					И Фильтр.Склад = Запасы.Склад
			|					И Фильтр.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка))
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура, Характеристика, Склад";
...
Процедура ABCXYZАнализНоменклатурыПодменитьСвободныеОстаткиПереопределяемый(ТекстЗапроса) Экспорт
	...
	Иначе
		...
		СтрокаЗамены =
			...
			|	ПО НоменклатураХарактеристика.Склад = СвободныеОстатки.Склад
			|	И НоменклатураХарактеристика.Номенклатура = СвободныеОстатки.Номенклатура
			|	И НоменклатураХарактеристика.Характеристика = СвободныеОстатки.Характеристика
			|	И СвободныеОстатки.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)
			|	И СвободныеОстатки.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)}"; //@Query-part
		
		СтрокаЗаменыПоля = "СвободныеОстатки.ВНаличии"; //@Query-part
...
Процедура СкидкиНаценкиПодменитьОстатокНаСкладеПереопределяемый(ТекстЗапроса) Экспорт
	...
	Если ЭтоПроизводительныйРежим() Тогда
		...
		СтрокаЗамены =
			...
			|	И ОстатокНаСкладе.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|	И ОстатокНаСкладе.Заказ = НЕОПРЕДЕЛЕНО
			|	И ОстатокНаСкладе.ДатаСобытия = ДАТАВРЕМЯ(1, 1, 1)}"; //@Query-part
	...
	Иначе
		...
		СтрокаЗамены =
			...
			|	И ФильтрПоНоменклатуре.Характеристика = ОстатокНаСкладе.Характеристика
			|	И ОстатокНаСкладе.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|	И ОстатокНаСкладе.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)}"; //@Query-part
		
		СтрокаЗаменыПоля =
			"ВЫБОР
			|	КОГДА ЕСТЬNULL(ОстатокНаСкладе.Свободно, 0) <= 0
			|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ДоступностьТовараДляКлиента.НетВНаличии)
			|	КОГДА ЕСТЬNULL(ОстатокНаСкладе.Свободно, 0) <= (ТоварныеОграничения.МинимальноеКоличествоЗапаса)
			|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ДоступностьТовараДляКлиента.УточняйтеНаличие)
			|	КОГДА ЕСТЬNULL(ОстатокНаСкладе.Свободно, 0) < (ТоварныеОграничения.СтраховоеКоличествоЗапаса + ТоварныеОграничения.МинимальноеКоличествоЗапаса)
			|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ДоступностьТовараДляКлиента.Мало)
			|	КОГДА ЕСТЬNULL(ОстатокНаСкладе.Свободно, 0) < ТоварныеОграничения.МаксимальноеКоличествоЗапаса
			|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ДоступностьТовараДляКлиента.Достаточно)
			|	КОГДА ЕСТЬNULL(ОстатокНаСкладе.Свободно, 0) >= ТоварныеОграничения.МаксимальноеКоличествоЗапаса
			|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ДоступностьТовараДляКлиента.Много)
			|	ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ДоступностьТовараДляКлиента.Неизвестно)
...
Процедура ПриИзмененииПроизводительногоРежима(УникальныйИдентификатор) Экспорт
	
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Переход на производительный режим хранения информации о доступности товаров и работ'");
	
	ДлительныеОперации.ВыполнитьВФоне("ОбеспечениеВДокументахСервер.ПриИзмененииПроизводительногоРежимаВФоне",
		ПараметрыПроцедуры, ПараметрыВыполнения);
	
КонецПроцедуры

Процедура ПриИзмененииПроизводительногоРежимаВФоне(Параметры, АдресРезультата) Экспорт
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		...
		Текст =
			...
			|ИЗ(
			// Неверные остатки на складе.
			|	ВЫБРАТЬ
			|		ОстаткиНовые.Номенклатура КАК Номенклатура,
			|		ОстаткиНовые.Характеристика КАК Характеристика,
			|		ОстаткиНовые.Склад КАК Склад,
			|		ОстаткиНовые.Назначение КАК Назначение
			|	ИЗ
			|		РегистрНакопления.ЗапасыИПотребности.Остатки КАК ОстаткиНовые
			|		
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ОстаткиСтарые
			|			ПО ОстаткиСтарые.Номенклатура = ОстаткиНовые.Номенклатура
			|			 И ОстаткиСтарые.Характеристика = ОстаткиНовые.Характеристика
			|			 И ОстаткиСтарые.Склад = ОстаткиНовые.Склад
			|			 И ОстаткиСтарые.Назначение = ОстаткиНовые.Назначение
			|			 И ОстаткиСтарые.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)
			|			 И ОстаткиСтарые.Запас <> 0
			|	ГДЕ
			|		ОстаткиНовые.Заказ = НЕОПРЕДЕЛЕНО
			|			И ОстаткиНовые.ДатаСобытия = ДАТАВРЕМЯ(1, 1, 1)
			|			И ОстаткиНовые.ВНаличииОстаток <> 0
			|			И ОстаткиНовые.ВНаличииОстаток <> ЕСТЬNULL(ОстаткиСтарые.Запас, 0)
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			|		ОстаткиСтарые.Номенклатура КАК Номенклатура,
			|		ОстаткиСтарые.Характеристика КАК Характеристика,
			|		ОстаткиСтарые.Склад КАК Склад,
			|		ОстаткиСтарые.Назначение КАК Назначение
			|	ИЗ
			|		РегистрСведений.РаспределениеЗапасов КАК ОстаткиСтарые
			|		
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыИПотребности.Остатки КАК ОстаткиНовые
			|			ПО ОстаткиНовые.Номенклатура = ОстаткиСтарые.Номенклатура
			|			 И ОстаткиНовые.Характеристика = ОстаткиСтарые.Характеристика
			|			 И ОстаткиНовые.Склад = ОстаткиСтарые.Склад
			|			 И ОстаткиНовые.Назначение = ОстаткиСтарые.Назначение
			|			 И ОстаткиНовые.Заказ = НЕОПРЕДЕЛЕНО
			|			 И ОстаткиНовые.ДатаСобытия = ДАТАВРЕМЯ(1, 1, 1)
			|			 И ОстаткиНовые.ВНаличииОстаток <> 0
			|	ГДЕ
			|		ОстаткиСтарые.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)
			|			И ОстаткиСтарые.Запас <> 0
			|			И ОстаткиНовые.Номенклатура ЕСТЬ NULL
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			// Неверные заказы на отгрузку.
			|	ВЫБРАТЬ
			|		ОстаткиНовые.Номенклатура КАК Номенклатура,
			|		ОстаткиНовые.Характеристика КАК Характеристика,
			|		ОстаткиНовые.Склад КАК Склад,
			...
			|			 И ОстаткиСтарые.Склад = ОстаткиНовые.Склад
			|			 И ОстаткиСтарые.Назначение = ОстаткиНовые.Назначение
			|			 И ОстаткиСтарые.ЗаказНаОтгрузку = ОстаткиНовые.Заказ
			|			 И ОстаткиСтарые.ЖелаемаяДатаОтгрузки = ОстаткиНовые.ДатаСобытия
			|			 И ОстаткиСтарые.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемаяОтгрузка)
			|			 И (ОстаткиСтарые.РезервироватьПоМереПоступления <> 0
			|					ИЛИ ОстаткиСтарые.КОбеспечениюБезРезерва <> 0
			|					ИЛИ ОстаткиСтарые.ОтложитьРезервирование <> 0
			|					ИЛИ ОстаткиСтарые.Резервировать <> 0
			|					ИЛИ ОстаткиСтарые.НеОбеспечивать <> 0)
			|	ГДЕ
			|		(ОстаткиНовые.РезервироватьПоМереПоступленияОстаток <> 0
			|				ИЛИ ОстаткиНовые.КОбеспечениюОстаток <> 0
			|				ИЛИ ОстаткиНовые.ОтложитьРезервированиеОстаток <> 0
			|				ИЛИ ОстаткиНовые.РезервироватьНаСкладеОстаток <> 0
			|				ИЛИ ОстаткиНовые.НеОбеспечиватьОстаток <> 0)
			|			И (ОстаткиНовые.РезервироватьПоМереПоступленияОстаток <> ЕСТЬNULL(ОстаткиСтарые.РезервироватьПоМереПоступления, 0)
			|				ИЛИ ОстаткиНовые.КОбеспечениюОстаток <> ЕСТЬNULL(ОстаткиСтарые.КОбеспечениюБезРезерва, 0)
			|				ИЛИ ОстаткиНовые.ОтложитьРезервированиеОстаток <> ЕСТЬNULL(ОстаткиСтарые.ОтложитьРезервирование, 0)
			|				ИЛИ ОстаткиНовые.РезервироватьНаСкладеОстаток <> ЕСТЬNULL(ОстаткиСтарые.Резервировать, 0)
			|				ИЛИ ОстаткиНовые.НеОбеспечиватьОстаток <> ЕСТЬNULL(ОстаткиСтарые.НеОбеспечивать, 0))
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			...
			|			 И ОстаткиНовые.Склад = ОстаткиСтарые.Склад
			|			 И ОстаткиНовые.Назначение = ОстаткиСтарые.Назначение
			|			 И ОстаткиНовые.Заказ = ОстаткиСтарые.ЗаказНаОтгрузку
			|			 И ОстаткиНовые.ДатаСобытия = ОстаткиСтарые.ЖелаемаяДатаОтгрузки
			|			 И (ОстаткиНовые.РезервироватьПоМереПоступленияОстаток <> 0
			|				ИЛИ ОстаткиНовые.КОбеспечениюОстаток <> 0
			|				ИЛИ ОстаткиНовые.ОтложитьРезервированиеОстаток <> 0
			|				ИЛИ ОстаткиНовые.РезервироватьНаСкладеОстаток <> 0
			|				ИЛИ ОстаткиНовые.НеОбеспечиватьОстаток <> 0)
			|	ГДЕ
			|		ОстаткиСтарые.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемаяОтгрузка)
			|			И (ОстаткиСтарые.РезервироватьПоМереПоступления <> 0
			|					ИЛИ ОстаткиСтарые.КОбеспечениюБезРезерва <> 0
			|					ИЛИ ОстаткиСтарые.ОтложитьРезервирование <> 0
			|					ИЛИ ОстаткиСтарые.Резервировать <> 0
			|					ИЛИ ОстаткиСтарые.НеОбеспечивать <> 0)
			|			И ОстаткиНовые.Номенклатура ЕСТЬ NULL) КАК Набор";
			
		Запрос = Новый Запрос(Текст);
		МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		Запрос.Выполнить();
		ДокументСтруктура = Новый Структура("Ссылка, ДополнительныеСвойства", Неопределено, Новый Структура);
...
Функция ОстаткиДляУстановкиДействий(Запрос, СкладыГруппы)
	...
	ТекстЗапроса =
		...
		|	Номенклатура, Характеристика, Склад, Назначение";
	Тексты.Добавить(ТекстЗапроса);
	ТекстЗапроса = ВременнаяТаблицаСвободныеОстатки("ТаблицаФильтраТоваров", "ТаблицаОстатков");
	Тексты.Добавить(ТекстЗапроса);
	...
	ТекстЗапроса =
		...
		|		ЕСТЬNULL(СкладыГруппы.Ссылка, Товары.Склад) КАК Склад,
		|		Товары.Назначение                           КАК Назначение,
		|		ЕСТЬNULL(СвободныйОстаток.Количество, 0)      КАК ВНаличии
		|	ИЗ
		|		ВыделенныеТоварыСУчетомСклада КАК Товары
		|			
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СкладыГруппы
...
Функция ОстаткиДляПодбораСкладов(Запрос, СкладыГруппы, КоэффициентУпаковки)
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|	Запасы.Запас - Запасы.Свободно КАК Резерв
			|ПОМЕСТИТЬ ВременнаяТаблицаОстаток
			|ИЗ
			|	РегистрСведений.РаспределениеЗапасов КАК Запасы
			|ГДЕ
			...
			|				ИСТИНА КАК ЕстЗаписи
			|			ИЗ
			|				ТаблицаФильтраТоваров КАК Фильтр
			|			ГДЕ
			|				Фильтр.Номенклатура = Запасы.Номенклатура
			|					И Фильтр.Характеристика = Запасы.Характеристика
			|					И Фильтр.Склад = Запасы.Склад
...
Функция ВременнаяТаблицаДвиженийДокумента(ИгнорироватьГотовностьКОбеспечению = Ложь, ЭтоПроизводительныйРежим = Ложь)
	...
	Если ЭтоПроизводительныйРежим Тогда
		...
		ВложенныйЗапрос = РаспределениеЗапасовДвижения.ТекстЗапросаРаспоряжениеНаОбеспечениеИОтгрузкуПроизводительныйРежим(ТекстЗапроса,
...
Функция ОстаткиДляТаблицыДокумента(МенеджерВременныхТаблиц, КэшОстатков, ТаблицаИзменений)
	...
	Если КэшОстатков = Неопределено Тогда
		...
		Текст = ВременнаяТаблицаСвободныеОстатки("ТаблицаТоваровБезУпаковок", "ВременнаяТаблицаОстаток");
		Тексты.Добавить(Текст);
