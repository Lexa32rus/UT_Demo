	...
	Пока ВыборкаПравила.Следующий() Цикл
		...
		Для Каждого Строка Из ПравилоОбъект.ПравилаЗаполненияРеквизитовДО Цикл
			Если Строка.Вариант = Перечисления.ВариантыПравилЗаполненияРеквизитов.ВыражениеНаВстроенномЯзыке Тогда
				ОбновитьВыражениеНаВстроенномЯзыке(Строка.ВычисляемоеВыражение, БылиИзменения);
			КонецЕсли;
		КонецЦикла;
...
Процедура ОбновитьВыражениеНаВстроенномЯзыке(ВычисляемоеВыражение, БылиИзменения)
...
Функция МодифицированныеМетоды()
	...
	Методы.Вставить(
		...
		"ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПометитьНаУдалениеСнятьПометку(");
	Методы.Вставить(
