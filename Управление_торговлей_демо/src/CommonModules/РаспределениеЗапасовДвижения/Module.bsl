...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ЗапасыИПотребностиИзменение") Тогда
			...
			РаспределениеЗапасов.ДобавитьТекстЗапросаКонтроль(Запрос, ТекстыЗапроса, Документ, Истина);
			
		КонецЕсли;
	Иначе
		Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияРаспределениеЗапасовДвиженияИзменение") Тогда
			
			РаспределениеЗапасов.ДобавитьТекстЗапросаКонтроль(Запрос, ТекстыЗапроса, Документ, Ложь);
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ЭтоПроизводительныйРежим
				И ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ЗапасыИПотребностиИзменение")
			Или Не ЭтоПроизводительныйРежим
				И ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияРаспределениеЗапасовДвиженияИзменение") Тогда
...
Процедура РасходЗапаса(Запрос, ТекстыЗапроса, Регистры, ТекстЗапросаТабЧасть, ТекстыШаблоновВременныхТаблиц = Неопределено) Экспорт
	...
	ТекстыШаблоновОтражения = ТекстыШаблоновОтражения(Регистры, ТекстЗапросаОперации, "РаспределениеЗапасовДвижения");
...
//	Возвращаемое значение:
//   Строка - Текст запроса.
Функция ТекстЗапросаРаспоряжениеНаОбеспечениеИОтгрузкуПроизводительныйРежим(ТекстЗапросаТабЧасть, ИгнорироватьГотовностьКОбеспечению) Экспорт
...
Процедура ПередЗаписьюДвиженийДокумента(Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	НаборЗапасыИПотребности = Документ.Движения.ЗапасыИПотребности;
	
	Если Набор.ОбменДанными.Загрузка И НаборЗапасыИПотребности.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли;
	...
	Если ИзменилосьМестоВОчереди Тогда
		...
		НаборЗапасыИПотребности.ДополнительныеСвойства.Вставить("ИзменилосьМестоВОчередиРаспределенияЗапасов", Истина);
	КонецЕсли;
...
// Возвращаемое значение:
//  Строка - текст запроса.
Функция ТекстыЗапросовПроведенияДокументов(ПолноеИмяДокумента, ИмяРегистра) Экспорт
...
// Возвращаемое значение:
//  Массив Из Строка - Имена документов регистраторов регистра накопления РаспределениеЗапасовДвижения.
...
Процедура ОтразитьЗаданияКРаспределениюЗапасов(Документ, МенеджерВременныхТаблиц) Экспорт
	...
	Если Не РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		Возврат;
	КонецЕсли;
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ЗапасыИПотребностиИзменение") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если РаспределениеЗапасов.ДосчитыватьРегистрРегламентнымЗаданием() Тогда
		...
		Если Документ.Движения.ЗапасыИПотребности.ДополнительныеСвойства.Свойство("ИзменилосьМестоВОчередиРаспределенияЗапасов") Тогда
...
Процедура ЗаписатьРаспределениеЗапасов(Запрос, ПолучатьГраницыПериодаОбеспечения, ИмяТаблицыТовары = "ТоварыДляРаспределенияЗапасов")
	...
	ДобавитьПустуюТаблицуИзменениеОстаткаПоТоварамДляКонтроляОстатков(Запрос);
