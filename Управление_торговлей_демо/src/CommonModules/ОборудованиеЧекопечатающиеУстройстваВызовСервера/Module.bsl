...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт
	...
	ИначеЕсли Команда = "PrintReceiptCorrection" Тогда
		...
		Иначе
			...
			КорректируемыйДокументОперация = ДанныеФискальнойОперации(ПараметрыОперации.КорректируемыйДокумент, ,ТипыДокументов, ПараметрыОперации.ТипРасчета); 
...
//  ПараметрыПакетнойОперации - Структура - см. ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыПакетнойОперацииВОчередиЧеков()
// Возвращаемое значение:
...
Функция ДобавитьЧекВОчередьЧековККТ(ПараметрыЧекаККТ, ПараметрыПакетнойОперации = Неопределено) Экспорт
	...
	Исключение                 
		...
		#Если НЕ МобильноеПриложениеСервер Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка добавления чека в очередь'", 
...
Функция ДанныеЧекаВОчереди(ИдентификаторЗаписи) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если ТипЗнч(ДанныеЧека.ДанныеЧека) = Тип("ХранилищеЗначения") Тогда
			ДанныеЧека.ДанныеЧека = ДанныеЧека.ДанныеЧека.Получить();
		КонецЕсли;
		Если ТипЗнч(ДанныеЧека.РезультатПакетнойОперации) = Тип("ХранилищеЗначения") Тогда
			ДанныеЧека.РезультатПакетнойОперации = ДанныеЧека.РезультатПакетнойОперации.Получить();
		КонецЕсли;
...
Функция ЧекиВОчередиНаФискализацию() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ДанныеЧека.ДанныеЧека) = Тип("ХранилищеЗначения") Тогда
			ДанныеЧека.ДанныеЧека = ДанныеЧека.ДанныеЧека.Получить();
		КонецЕсли;
		Если ТипЗнч(ДанныеЧека.ПараметрыПакетнойОперации) = Тип("ХранилищеЗначения") Тогда
			ДанныеЧека.ПараметрыПакетнойОперации = ДанныеЧека.ПараметрыПакетнойОперации.Получить();
		КонецЕсли;
...
// Параметры:
//  ЗаписьXML - ЗаписьXML
//  ДанныеАгента - Структура - см. ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыДанныеАгента()
//  РевизияИнтерфейса - Число
//
...
// Параметры:
//  ЗаписьXML - ЗаписьXML
//  ДанныеПоставщика - Структура - см. ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыДанныеПоставщика()
//  РевизияИнтерфейса - Число
//
...
// Параметры:
//   Контекст - Структура - контекст который вернула функция ПараметрыВыводЧека()
//   ПозицияЧека - Структура - параметры строки фискального документа загруженная из данных XML
//                    см. ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыФискальнойСтрокиЧека()
