...
Функция WSПроксиСбербанк(Таймаут = 30) Экспорт
	...
	WSОпределениеСбербанк = WSОпределениеСбербанкСертификат();
	...
	Исключение
		...
		УзелНеПрошелПроверку = СтрНайти(КраткоеПредставлениеОшибки, ТекстОшибки1) > 0
			Или СтрНайти(КраткоеПредставлениеОшибки, ТекстОшибки2) > 0;
	КонецПопытки;
	
	Если УзелНеПрошелПроверку Тогда
		ЗащищенноеСоединение = НовыйЗащищенноеСоединение(Истина);
		WSОпределениеСбербанк = WSОпределениеСбербанкСертификат(Истина);
		WSПрокси = Новый WSПрокси(
			WSОпределениеСбербанк,
			URI,
			ИмяСервиса,
...
Функция WSОпределениеСбербанкСертификат(СертификатИзМакета = Ложь)
	...
	Прокси = ИнтернетСоединениеБЭД.СформироватьПрокси("https");
	ЗащищенноеСоединение = НовыйЗащищенноеСоединение(СертификатИзМакета);
...
Функция НовыйЗащищенноеСоединение(СертификатИзМакета = Ложь)
	
	Если СертификатИзМакета Тогда
		ИмяФайла = СертификатСбербанк();
...
Функция СертификатСбербанк()
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("pem");
	ДвоичныеДанные = Обработки.ОбменСБанками.ПолучитьМакет("sberca");
