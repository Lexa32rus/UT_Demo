...
Функция ЗаписатьИзмененияОбъектов(Сообщение) Экспорт
	...
	Попытка
		...
		Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
			...
			ТипXDTO = ФабрикаDM.Тип("http://www.1c.ru/dm", ЧтениеXML.Имя);
			ОбъектXDTO = ФабрикаDM.ПрочитатьXML(ЧтениеXML, ТипXDTO);
			
			Попытка
				
				Если ОбъектXDTO.Тип() = ФабрикаDM.Тип("http://www.1c.ru/dm", "DMApprovalStateRecord") Тогда
		...
		Иначе
			Ошибка = ИнтеграцияС1СДокументооборот3.СоздатьОбъектБИД("DMILError");
			Ошибка.subject = НСтр("ru = 'Ошибка при загрузке сообщения обмена из 1С:Документооборот'");
			Ошибка.description = СтрСоединить(
				МассивОшибок,
				"
				|-----
				|
	...
	Исключение
		...
		Ошибка.description = ИнтеграцияС1СДокументооборот3.ПолучитьОписаниеОшибки(Инфо);
		Возврат Ошибка;
...
Функция ЗарегистрированныеДанные(Прокси, УзелОбмена, ОбъектыКУдалениюИзРегистрацииИзменений) Экспорт
	...
	СоответствиеСпискаВыраженийОбъектамИС = ИнтеграцияС1СДокументооборот3.СоответствиеСпискаВыраженийОбъектамИС(
		Прокси,
		СписокОбъектовИС,
