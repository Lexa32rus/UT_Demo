...
Процедура СформироватьСтоимостьПартийНДСНаКонецПериода(ПараметрыРасчета)
	...
	|	Т.Организация В(&ОрганизацииСУчетомПартийНДСВерсии24)
	|	И НЕ Т.ТипЗаписи В (&ТипыЗаписейКонвертацииДанных)
	|	И НЕ &ВыполненРасчетПеременныхЗатрат
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ВыполненРасчетПеременныхЗатрат", "ЛОЖЬ");
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		"ВТПередачиБезНДС, ВТСебестоимостьДляСтоимостиПартийНДС, ВТОборотыСтоимостьПартийНДСПредварительная,
		|ВТДокументыПоступленияНДС, ВТДокументыСписанияНДС, ПериодПредыдущегоРассчитанногоПериода, ВТОстаткиПеременныхЗатрат");
...
Функция ТекстИнициализацияРаспределениеПартийНДСФИФОСкользящая()
	Возврат
		...
		|	ИЗ
		|		РегистрНакопления.ПрочиеРасходы.Остатки(&НачалоПериода, Организация В(&МассивОрганизаций)) КАК Т
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		Т.АналитикаРасходов КАК АналитикаРасходов,
		|		Т.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|		ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА Т.Сумма ИНАЧЕ -Т.Сумма КОНЕЦ КАК Сумма,
		|		ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА Т.СуммаБезНДС ИНАЧЕ -Т.СуммаБезНДС КОНЕЦ КАК СуммаБезНДС,
		|		ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА Т.СуммаРегл ИНАЧЕ -Т.СуммаРегл КОНЕЦ КАК СуммаРегл
		|	ИЗ
		|		РегистрНакопления.ПрочиеРасходы КАК Т
		|	ГДЕ
		|		Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
		...
		|	Т.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|	Т.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	СУММА(Т.Стоимость * Статьи.КоэффициентУпр * ВЫБОР КОГДА ЕСТЬNULL(ПрочиеРасходы.Сумма, 0) <> 0 ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК Стоимость,
		|	СУММА(Т.СтоимостьБезНДС * Статьи.КоэффициентУпр * ВЫБОР КОГДА ЕСТЬNULL(ПрочиеРасходы.СуммаБезНДС, 0) <> 0 ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК СтоимостьБезНДС,
		|	СУММА(Т.СтоимостьРегл * Статьи.КоэффициентРегл * ВЫБОР КОГДА ЕСТЬNULL(ПрочиеРасходы.СуммаРегл, 0) <> 0 ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК СтоимостьРегл,
		|	СУММА(Т.НДСРегл * Статьи.КоэффициентРегл * ВЫБОР КОГДА ЕСТЬNULL(ПрочиеРасходы.СуммаРегл, 0) <> 0 ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК НДСРегл,
		|	СУММА(Т.НДСУпр * Статьи.КоэффициентУпр * ВЫБОР КОГДА ЕСТЬNULL(ПрочиеРасходы.Сумма, 0) <> 0 ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК НДСУпр
...
Функция ТекстЗапросаДетализацияПартийНДС(ТекстОрганизация = "", ТекстНачалоПериода = "", ТекстКонецПериода = "", СтруктураОтбора = Неопределено) Экспорт
	...
	СхемаЗапроса.ПакетЗапросов[0].ТаблицаДляПомещения = "ВТНастройкиСистемыНалогообложения";
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиСистемыНалогообложения КАК ОСНО
