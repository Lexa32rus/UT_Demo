...
Функция ПолучитьНастройки() Экспорт
	...
	Запрос = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMGetSettingsRequest");
	
	НастройкиXDTO = Прокси.execute(Запрос);
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, НастройкиXDTO);
...
Функция ДанныеЗаполненияИнтегрируемогоОбъекта(Правило, ТипОбъектаДО, ИдентификаторОбъектаДО) Экспорт
	...
	ОбъектИд = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ИдентификаторОбъектаДО, ТипОбъектаДО);
...
Функция ПодходящиеПравила(ОбъектИС = Неопределено, ОбъектДО = Неопределено, ТипОбъектаИС = Неопределено,
	...
	Пока Не Достаточно Цикл
		...
		Если Правила.Количество() > 1
			...
			ЗапросОбъекта = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMRetrieveRequest");
...
Функция НастроенаИнтеграцияДляОбъекта(ОбъектИС) Экспорт
	...
	Если Не ИнтеграцияС1СДокументооборот.ИспользоватьИнтеграцию() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ПереключитьХронометражПоОбъектамДокументооборота(Источники) Экспорт
	...
	Для Каждого Источник Из Источники Цикл
		...
		ОбъектXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
...
Функция ШаблоныБизнесПроцесса(Знач ТипБизнесПроцесса, Знач ПредметБизнесПроцесса) Экспорт
	...
	Если ПредметБизнесПроцесса <> Неопределено Тогда
		ПредметБизнесПроцессаИд = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
...
Функция ЗаполнитьБизнесПроцессПоШаблону(Знач Форма, Знач ДанныеОШаблоне) Экспорт
	...
	ШаблонБизнесПроцессаИд = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
...
Функция ПользователюАдресованыЗадачиСогласования(Знач ПредметСогласования) Экспорт
	...
	Результат = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ДанныеОбъектаДОПоВнешнемуОбъекту(ПредметСогласования);
...
Функция ПрерватьСогласование(Знач ПредметДО) Экспорт
	...
	ЗапросНаЗапись = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMUpdateRequest");
	СписокОбъектовНаЗапись = ЗапросНаЗапись.objects; // СписокXDTO
	
	Для Каждого Элемент Из Ответ.items Цикл
		
		Процесс = Элемент.object;
		DMRetrieveRequest = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMRetrieveRequest");
		СписокОбъектов = DMRetrieveRequest.objectIDs; // СписокXDTO
		
		ObjectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, 
		...
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, DMRetrieveResponse);
...
Функция ВыполнитьСогласование(Знач Задачи, Знач Результат, Знач Комментарий, ТекстСообщенияОбОшибке) Экспорт
	...
	Для Каждого ИдентификаторЗадачи Из Задачи Цикл
		...
		ОбъектыXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси,
		...
		Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаписатьОбъект(Прокси, Задача);
...
Процедура ОстановитьПроцесс(ID, Тип) Экспорт
	...
	ЗапросНаЧтение = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMRetrieveRequest");
	СписокОбъектовНаЧтение = ЗапросНаЧтение.objectIDs; // СписокXDTO
	
	ObjectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ID, Тип);
...
Процедура ЗарегистрироватьДокументПриНеобходимости(ДокументТип, ДокументID) Экспорт
	...
	ОбъектыXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси, ДокументТип, ДокументID, Колонки);
	ОбъектXDTO = ОбъектыXDTO.objects[0];
	...
	ОбъектXDTO.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ДокументID, ДокументТип);
	ОбъектXDTO.name = "";
...
Процедура ЗанестиИнформациюОПодписяхОбъекта(ID, Имя, ОписаниеФайла, Тип,
	...
	Объект.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ID, Тип);
	...
	Для Каждого ДанныеПодписи Из МассивСуществующихПодписей Цикл
		...
		ПодписьXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMSignature");
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаписатьОбъект(Прокси, Объект);
...
Процедура ОбновитьСписокФайлов(ФайлыXDTO, Файлы, ЭлементДляОбновленияЗаголовка = Неопределено) Экспорт
	...
	ТекущийПользователь = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТекущийПользовательДокументооборота();
	...
	Для Каждого СведенияОФайле Из ФайлыXDTO Цикл
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СведенияОФайле, "deletionMark")
...
Функция ПолучитьФайлИПоместитьВХранилище(ID, УникальныйИдентификатор, ДатаЗаема = Неопределено,
	...
	ОбъектID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ID, "DMFile");
...
Функция ПолучитьВерсиюФайлаИПоместитьВХранилище(ID, УникальныйИдентификатор, Размер = Неопределено,
	...
	ОбъектID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ID, "DMFileVersion");
...
Функция ДвоичныеДанныеФайлов(Файлы) Экспорт
	...
	Для Каждого Файл Из Файлы Цикл
		...
		objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, Файл, "DMFile");
		СписокОбъектов.Добавить(objectID);
...
Функция ОбновитьФайлДвоичнымиДаннымиВременногоХранилища(Знач Параметры) Экспорт
	...
	Запрос = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMUpdateRequest");
	СписокОбъектов = Запрос.objects; // СписокXDTO
	
	Файл = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMFile");
	Файл.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, 
...
Функция ОписаниеФайла(ID, Форма = Неопределено, АдресВременногоХранилищаФайла = "") Экспорт
	...
	Файл = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ID, "DMFile");
...
Функция ВерсииФайла(ИдентификаторФайла, ВключатьПомеченныеНаУдаление = Ложь) Экспорт
	...
	Условие.value = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ИдентификаторФайла, "DMFile");
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
		ПолучаемыеПоля.Добавить("modificationDateUniversal");
...
Функция СделатьВерсиюАктивной(ИдентификаторФайла, ИдентификаторВерсии, ТекстСообщения = "") Экспорт
	...
	Версия.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, 
...
Функция СоздатьИзФайлаНаДискеСервер(ПараметрыСоздания, ID, Тип, Представление = "",
	...
	ОбъектВладелец = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObject");
	ОбъектВладелец.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ID, Тип);
	...
	Если ОбъектИС = Неопределено Тогда
		...
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПриПоявленииПрисоединенныхФайловДокументооборота(ID, Тип, ПараметрыСоздания.Владелец);
	Иначе
...
Процедура СохранитьСведенияОРедактированииФайла(Сведения) Экспорт
	...
	Запрос.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, 
...
Функция ЕстьХранимыеФайлы(ВнешнийОбъект) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПриОпределенииНаличияПрисоединенныхФайлов(ВнешнийОбъект, ЕстьХранимыеФайлы);
...
Функция ПолучитьХранимыеФайлы(ВнешнийОбъект) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПриПолученииПрисоединенныхФайлов(ВнешнийОбъект, ХранимыеФайлы);
