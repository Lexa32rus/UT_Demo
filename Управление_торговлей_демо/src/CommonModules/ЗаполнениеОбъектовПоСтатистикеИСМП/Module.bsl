...
Процедура ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(Объект, МассивСтрок = Неопределено, Знач ДополнительныеПоляПоиска = "") Экспорт
	...
	ЗаполнятьРеквизитыФормы = ТипЗнч(Объект) = Тип("ДанныеФормыСтруктура");
	
	ОбрабатыватьГрупповыеУпаковки = Ложь;
	ШаблонГрупповойУпаковки       = Неопределено;
	ОбрабатыватьНаборы            = Ложь;
	ШаблонНабора                  = Неопределено;
	Если ДополнительныеПоляПоиска.Найти("Шаблон") = Неопределено Тогда
		...
		Если ОбрабатыватьГрупповыеУпаковки Тогда
			ШаблонГрупповойУпаковки = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(Объект.ВидПродукции, 4);
		КонецЕсли;
		...
		Если ОбрабатыватьНаборы Тогда
			ШаблонНабора = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(Объект.ВидПродукции, 5);
		КонецЕсли;
	КонецЕсли;
	...
	ТаблицаДанных = ДанныеЗаполненияТоварыЗаказаНаЭмиссиюСУЗ(
		ВременнаяТаблица, Объект, ДополнительныеПоляПоиска, ШаблонГрупповойУпаковки, ШаблонНабора);
	
	Если ОбрабатыватьГрупповыеУпаковки Тогда
		КоличествоПотребительскихУпаковокВГрупповойУпаковке = КоличествоПотребительскихУпаковокВГрупповойУпаковке(
			ТаблицаДанных, ШаблонГрупповойУпаковки);
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		Если СтрокаДанных.НомерСтроки = НомерСтроки Тогда
			...
			Если ОбрабатыватьГрупповыеУпаковки
				И СтрокаДанных.Шаблон = ШаблонГрупповойУпаковки
				...
				И СтрокаТовары.Шаблон <> ШаблонГрупповойУпаковки Тогда
		...
		Иначе
			...
			Если ОбрабатыватьНаборы
				...
				И СтрокаДанных.Шаблон = ШаблонНабора
				И Не ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца)
				И ЗначениеЗаполнено(СтрокаТовары.Шаблон)
				И СтрокаТовары.Шаблон = ШаблонНабора Тогда
	...
	Если СтрокиНаборов.Количество() > 0 Тогда
		...
		ЗаполнитьСоставляющиеНаборов(Объект, СтрокиНаборов, ДобавленныеСтроки);
		
		Если ДобавленныеСтроки.Количество() > 0 Тогда
			ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(Объект, ДобавленныеСтроки);
		КонецЕсли;
...
Функция ДанныеЗаполненияТоварыОтгрузкаТоваровИСМП(ИсходнаяТаблица, Объект) Экспорт
	...
	Запрос.Текст =
		...
		|	ОтгрузкаТоваровИСМПТовары.GTIN              КАК GTIN,
		|	ОтгрузкаТоваровИСМПТовары.Ссылка            КАК Ссылка,
		|	ОтгрузкаТоваровИСМПТовары.Ссылка.Дата       КАК Дата
		...
		|	ОтгрузкаТоваровИСМПТовары.GTIN,
		|	ОтгрузкаТоваровИСМПТовары.Ссылка,
...
Функция ДанныеЗаполненияТоварыЗаказаНаЭмиссиюСУЗ(ИсходнаяТаблица, Объект, МассивПолейСвязи, ШаблонГрупповойУпаковки, ШаблонНабора)
	...
	Если ШаблонГрупповойУпаковки <> Неопределено Тогда
		ШаблоныУпаковок.Добавить(ШаблонГрупповойУпаковки);
	КонецЕсли;
	
	Если ШаблонНабора <> Неопределено Тогда
		ШаблоныУпаковок.Добавить(ШаблонНабора);
	КонецЕсли;
...
Функция КоличествоПотребительскихУпаковокВГрупповойУпаковке(ТаблицаДанных, ШаблонГрупповойУпаковки)
	
	Данные = Новый Соответствие;
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		Если Не ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
			Или СтрокаДанных.Шаблон <> ШаблонГрупповойУпаковки
...
Процедура ЗаполнитьСоставляющиеНаборов(Объект, МассивСтрок, ДобавленныеСтроки)
