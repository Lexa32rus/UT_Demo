...
Процедура ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры) Экспорт
	...
	Если НужнаПереоценка Тогда
		...
		КурсВалютыВзаиморасчетов = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(ГлобальныеПеременные.ВалютаРасчетов, ДатаНачалаКурсовыхРазниц, ВалютаРегламентированногоУчета);
		Если КурсВалютыВзаиморасчетов.КурсЗнаменатель = 0 Тогда
			КоэффициентВалютыВзаиморачетов = 0;
		Иначе
			КоэффициентВалютыВзаиморачетов = КурсВалютыВзаиморасчетов.КурсЧислитель / КурсВалютыВзаиморасчетов.КурсЗнаменатель;
		КонецЕсли;
		
		КурсВалютыУпр = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(ВалютаУправленческогоУчета, ДатаНачалаКурсовыхРазниц, ВалютаРегламентированногоУчета);
		Если КурсВалютыУпр.КурсЗнаменатель = 0 Тогда
			КоэффициентВалютыУпр = 0;
		Иначе
			КоэффициентВалютыУпр = КурсВалютыУпр.КурсЧислитель / КурсВалютыУпр.КурсЗнаменатель;
		КонецЕсли;
		
		ТочностьОбратногоПересчетаРегл = ?(КоэффициентВалютыВзаиморачетов = Цел(КоэффициентВалютыВзаиморачетов), 0,
			СтрДлина(Формат(КоэффициентВалютыВзаиморачетов,"ЧН=; ЧГ=")) - СтрДлина(Цел(Формат(КоэффициентВалютыВзаиморачетов,"ЧН=; ЧГ="))) - 1);
		ТочностьОбратногоПересчетаУпр = ТочностьОбратногоПересчетаРегл 
			+ ?(КоэффициентВалютыУпр = Цел(КоэффициентВалютыУпр), 0,
				СтрДлина(Формат(КоэффициентВалютыУпр,"ЧН=; ЧГ=")) - СтрДлина(Цел(Формат(КоэффициентВалютыУпр,"ЧН=; ЧГ="))) - 1);
...
Процедура ВыполнитьПереоценкуЗаПериод(МассивОрганизаций, Знач НачалоПериода, Знач КонецПериода, Параметры) Экспорт
	...
	УдалитьКурсовыеРазницыЗаПериод(МассивОрганизаций, НачалоПериода, КонецПериода, Параметры.ЭтоРасчетыСКлиентами);
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные)
	...
	|				И (РасчетыПоСрокам.ОбъектРасчетов <> &ОбъектРасчетов ИЛИ РасчетыПоСрокам.ОбъектРасчетов <> &АналитикаУчетаПоПартнерам)
...
//  *ВыполняетсяРасчетНУ - Булево - Истина если выполняется расчет курсовых по НУ.
//
Функция ПараметрыПереоценки(ЭтоРасчетыСКлиентами = Неопределено, ПереоценкаПоДням = Неопределено) Экспорт
	...
	КонецЕсли;
	Результат.Вставить("ВыполняетсяРасчетНУ", Ложь);
	Возврат Результат;
...
Процедура УдалитьКурсовыеРазницыЗаПериод(МассивОрганизаций, НачалоПериода, Знач КонецПериода, ЭтоРасчетыСКлиентами = Неопределено)
	...
	ИмяЗамера = "Взаиморасчеты.РасчетКурсовыхРазницПоСрокам" + ?(ЭтоРасчетыСКлиентами,".Клиенты",".Поставщики")+".Удаление";
	ОписаниеЗамера = ОценкаПроизводительности.НачатьЗамерДлительнойОперации(ИмяЗамера);
	ВсегоОбработано = 0;
	...
	Иначе
		Если Курс = 0 Тогда
			Если СуммаВВалюте > СуммаВзаиморасчетов Тогда
				Возврат Окр(Окр(СуммаВВалюте / СуммаВзаиморасчетов, Точность) * СуммаКСписанию, 2);
			Иначе
				Возврат Окр(СуммаКСписанию / Окр(СуммаВзаиморасчетов/СуммаВВалюте,Точность), 2);
			КонецЕсли;
		Иначе
