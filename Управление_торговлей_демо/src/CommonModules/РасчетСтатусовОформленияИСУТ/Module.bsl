...
Процедура ПриОпределенииЗапросаДляИСМП(
	...
	Для Каждого ВидПродукцииИСМП Из УчитываемыеВидыПродукции Цикл
		Если ВидПродукцииИСМП = Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха Тогда
			Если ИнтеграцияИСМП.ВидыПродукцииТестовогоПериода().Найти(ВидПродукцииИСМП)<>Неопределено Тогда
				Продолжить;
			Иначе
	...
	ОтборНоменклатурыДляОтгрузки = СтрЗаменить(ОтборНоменклатурыДляОтгрузки,  "ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха),", "");
	
	ОтборНоменклатурыДляВыводаИзОборота = ОтборНоменклатуры;
	
	//Отгрузка, ЕАЭС, 5 товарных групп
	Если МетаданныеДокументаОснования = Метаданные.Документы.РеализацияТоваровУслуг
		...
		|		ТОГДА %1
		|		ИНАЧЕ %2
		|	КОНЕЦ", ОтборНоменклатурыДляОтгрузкиЕАЭС, ОтборНоменклатурыДляОтгрузки);
