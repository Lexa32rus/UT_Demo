...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
Процедура КассаККМЗаписатьПараметрыПодключенияНСПК(Форма) Экспорт
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
Процедура ОрганизацияЗаписатьПараметрыПодключенияНСПК(Форма) Экспорт
...
Процедура ПроверитьВозможностьВводаНаОсновании(ЧекККМ) Экспорт
	...
	|	ТаблицаТоваров.Номенклатура.КодТРУ КАК КодТРУ
	|ИЗ
	|	Документ.ЧекККМ.Товары КАК ТаблицаТоваров
	|ГДЕ
	|	ТаблицаТоваров.Ссылка = &Ссылка
	|	И ТаблицаТоваров.Номенклатура.КодТРУ <> """"";
	Запрос.УстановитьПараметр("Ссылка", ЧекККМ);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя вводить документ на основаниии ""Чек ККМ"" с товарами ФСС.'");
	КонецЕсли;
