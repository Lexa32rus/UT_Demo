...
Функция КакСостояниеТекущегоБаланса(Ответ) Экспорт
	...
	Попытка
		...
		Результат.Вставить("АвторизацияТикетИТС", Истина);
		Результат.Вставить("ДатаОкончания", '00010101');
		Результат.Вставить("ТарификацияВключена", Истина);
		Результат.Вставить("Баланс", 0);
		Результат.Вставить("Лимит", 0);
		Результат.Вставить("ИспользованоСегодня", 0);
		
		ЗаполнитьСостояниеТекущегоБаланса(Результат, ОтветJson);
...
Процедура ЗаполнитьСостояниеТекущегоБаланса(Результат, ОтветJson)
	
	Результат.АвторизацияТикетИТС = ОтветJson["is_its_user"];
	Результат.ДатаОкончания = ПрочитатьДату(ОтветJson["expiration_date"]);
	Результат.ТарификацияВключена = (ОтветJson["billing"] = "enabled");
	Результат.Баланс = ПрочитатьЧисло(ОтветJson["balance"]);
	Результат.Лимит = ПрочитатьЧисло(ОтветJson["limit"]);
	Результат.ИспользованоСегодня = ПрочитатьЧисло(ОтветJson["used_today"]);
