...
Процедура ПолучитьДокументы(
	...
	Пока НЕ ПолученыВсеДокументы Цикл
		...
		ДатаКонечнаяЗапросаПоДокументам = Мин(ДатаКонечнаяПоДокументам,
			ДатаНачальнаяЗапросаПоДокументам + СекундВ30Днях);
		...
		РезультатПолученияСпискаДокументов = ЭлектронноеАктированиеЕИС.
			СписокОбъектовПоставщикаПоДатамИСведениямОЗаказчике(
				...
				ДатаКонечнаяЗапросаПоДокументам);
		Если НЕ РезультатПолученияСпискаДокументов.Выполнено Тогда
			ОписаниеОшибки = НСтр("ru = 'Ошибка при получении из ЕИС документов поставщика %1 за период с %2 по %3.'");
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + "%4";
			СтрокаДатаНачальная = Формат(ДатаНачальнаяЗапросаПоДокументам, "ДФ=dd.MM.yyyy");
			СтрокаДатаКонечная = Формат(ДатаКонечнаяЗапросаПоДокументам, "ДФ=dd.MM.yyyy");
			Комментарий = РезультатПолученияСпискаДокументов.ОписаниеОшибки;
		...
		МассивОписанийДокументов = РезультатПолученияСпискаДокументов.СписокОбъектов;
		Если МассивОписанийДокументов = Неопределено Тогда
			ОписаниеОшибки = НСтр("ru = 'Ошибка при получении документов поставщика из ЕИС за период с %1 по %2'");
			ОписаниеОшибки = СтрШаблон(ОписаниеОшибки,
				ДатаНачальнаяЗапросаПоДокументам,
				ДатаКонечнаяЗапросаПоДокументам);
		...
		Для Каждого ОписаниеДокумента Из МассивОписанийДокументов Цикл
			...
			Если ОписаниеДокумента.ВидДокумента = ВидДокументаТитулПоставщика Тогда
				Продолжить;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ОписаниеДокумента.Идентификатор) Тогда
				ОписаниеОшибки = НСтр("ru = 'Не заполнен идентификатор эл.документа поставщика по гос.контракту.'");
			...
			Если НЕ РезультатПолученияЭД.Выполнено Тогда
				ОписаниеОшибки = НСтр("ru = 'Произошла ошибка при получении из ЕИС данных электронного документа вида %1 с ИД %2.'");
				ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + "%3";
			...
			ДвоичныеДанныеДокумента = РезультатПолученияЭД.ДанныеОбъекта.ДвоичныеДанныеСодержимого;
			Если ДвоичныеДанныеДокумента = Неопределено Тогда
				ОписаниеОшибки = НСтр("ru = 'Отсутствуют данные электронного документа из ЕИС с ИД %1'");
				ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, ОписаниеДокумента.Идентификатор);
				Комментарий = "";
				ЗаписьЖурналаРегистрации(ОписаниеОшибки,
					УровеньЖурналаРегистрации.Ошибка, , ,
					Комментарий);
			...
			Исключение
				...
				ОтменитьТранзакцию();
				КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
				ОписаниеОшибки = НСтр("ru = 'Ошибка при сохранении транспортного контейнера документа ЕИС в сфере закупок'",
					КодЯзыка);
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					ОписаниеОшибки,
	...
	Пока Не ПолученыВсеКонтракты Цикл
		...
		Для Каждого ОписаниеКонтракта Из МассивОписанийКонтрактов Цикл
			...
			Иначе
				...
				ОписаниеОшибки = СтрШаблон(Шаблон, ОписаниеКонтракта.РеестровыйНомер);
				ВызватьИсключение(ОписаниеОшибки);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
