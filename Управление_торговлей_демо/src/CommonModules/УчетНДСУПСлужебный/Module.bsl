...
Функция РассчитатьНДСАвансыПолученныеПереносыПриход(ПараметрыРасчета)
	...
	Для Каждого СтруктураОтбораЗачета Из ЭтапыЗачета Цикл
		Для каждого Строка Из НачисленияСПолученногоАвансаИтоги Цикл
			...
			ВыполнитьЗачетАвансаПолученного(ПараметрыРасчета, СтрокаАванса, СтрокиНачислениеНДС, СтрокиЗачетАванса, НДСАвансыПолученные, Истина, Ложь);
...
Функция РассчитатьНДСАвансыПолученныеПереносыРасход(ПараметрыРасчета)
	...
	Для Каждого СтруктураОтбораЗачета Из ЭтапыЗачета Цикл
		Для каждого Строка Из НачисленияСПолученногоАвансаИтоги Цикл
			...
			ВыполнитьЗачетАвансаПолученного(ПараметрыРасчета, СтрокаАванса, СтрокиНачислениеНДС, СтрокиЗачетАванса, НДСАвансыПолученные, Ложь, Ложь);
...
Процедура ВыполнитьЗачетАвансаПолученного(ПараметрыРасчета, СтрокаАвансаПоДокументуОплаты, СтрокиАвансаНачислениеНДС, СтрокиЗачетаАванса, ТаблицаНДСАвансыПолученные, ТолькоРаспределение = Ложь, ОбновитьИдентификатор = Истина)
...
Функция АвансыВыданныеПоДокументамОплаты(ПараметрыРасчета)
	...
	|		ДД.ПредоплатаУпр
	|	ИЗ
	|		РасчетыСПоставщиками_ПереносыАвансов КАК ДД) КАК Расчеты
	|
	|СГРУППИРОВАТЬ ПО
	|	Расчеты.НаправлениеДеятельности,
	|	Расчеты.Контрагент,
...
Функция РассчитатьНДСАвансыВыданныеЗачеты(ПараметрыРасчета)
	...
	ЗачетыАвансов = Запрос.Выполнить().Выгрузить(); // ТаблицаЗначений - 
	ЗачетыАвансов.Индексы.Добавить("ДокументОплаты,Контрагент,НаправлениеДеятельности,ОбъектРасчетов");
	ЗачетыАвансов.Индексы.Добавить("ДокументОплаты,Контрагент,НаправлениеДеятельности");
	ЗачетыАвансов.Индексы.Добавить("ДокументОплаты,Контрагент,ОбъектРасчетов");
	ЗачетыАвансов.Индексы.Добавить("ДокументОплаты,Контрагент");
	
	ВычетНДССВыданногоАванса = ВычетНДСАвансыВыданные(ПараметрыРасчета); // ТаблицаЗначений -
	ВычетНДССВыданногоАванса.Индексы.Добавить("ДокументОплаты,Контрагент,НаправлениеДеятельности,ОбъектРасчетов,Исправление,ДатаИсправления");
...
Процедура ВыполнитьЗачетАвансаВыданного(ПараметрыРасчета, СтрокаАвансаПоДокументуОплаты, СтрокиАвансаНачислениеНДС, СтрокиЗачетаАванса, ТаблицаНДСАвансыВыданные,  ТолькоРаспределение = Ложь, ОбновитьИдентификатор = Истина)
...
Функция ПолучитьОстаткиАвансовВыданныхДоПереноса(ПараметрыРасчета)
...
Функция РассчитатьНДСАвансыВыданныеПереносыРасход(ПараметрыРасчета)
	...
	Если ЕстьИСФ Тогда
		...
		|	ДД.ПредоплатаРегл                 КАК ПредоплатаРегл,
		|	ДД.ПредоплатаУпр                  КАК ПредоплатаУпр
		|ПОМЕСТИТЬ РасчетыСПоставщиками_ПереносыАвансовПромежуточная
		|ИЗ
		|	РасчетыСПоставщиками_ПереносыАвансов КАК ДД
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
	...
	ПодготовитьВТ_РасчетыСПоставщиками_ПереносыАвансов(
		...
		|	ДД.ПредоплатаРегл                 КАК ПредоплатаРегл,
		|	ДД.ПредоплатаУпр                  КАК ПредоплатаУпр
		|ПОМЕСТИТЬ РасчетыСПоставщиками_ПереносыАвансовИтог
		|ИЗ
		|	РасчетыСПоставщиками_ПереносыАвансовПромежуточная КАК ДД
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ РасчетыСПоставщиками_ПереносыАвансовПромежуточная
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ РасчетыСПоставщиками_ПереносыАвансов
		...
		|ИЗ
		|	РасчетыСПоставщиками_ПереносыАвансовИтог КАК ДД
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ РасчетыСПоставщиками_ПереносыАвансовИтог
		|";
	...
	Для Каждого СтруктураОтбораЗачета Из ЭтапыЗачета Цикл
		Для каждого Строка Из НачисленияСВыданногоАвансаИтоги Цикл
			...
			ВыполнитьЗачетАвансаВыданного(ПараметрыРасчета, СтрокаАванса, СтрокиНачислениеНДС, СтрокиЗачетАванса, НДСАвансыВыданные, Истина, Ложь);
...
Функция РассчитатьНДСАвансыВыданныеПереносыПриход(ПараметрыРасчета)
	...
	Для Каждого СтруктураОтбораЗачета Из ЭтапыЗачета Цикл
		Для каждого Строка Из НачисленияСВыданногоАвансаИтоги Цикл
			...
			ВыполнитьЗачетАвансаВыданного(ПараметрыРасчета, СтрокаАванса, СтрокиНачислениеНДС, СтрокиЗачетАванса, НДСАвансыВыданные, Ложь, Ложь);
...
// Создает временную таблицу с колонками: см. ВзаиморасчетыСервер.ПодготовитьВТ_РасчетыСКлиентами_ПереносыАвансов
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - 
//  ПараметрыРасчета        - см. ИнициализироватьПараметрыПодготовкиРасчетовАвансовВЦеляхНДС
...
Процедура ПодготовитьВТ_ОстаткиАвансыПолученныеДоПереноса(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	|	ГДЕ
	|		&ТекстОтборПоПараметрам) КАК НачислениеСПолученногоАванса
	|
	|СГРУППИРОВАТЬ ПО
	|	НачислениеСПолученногоАванса.Организация,
	|	НачислениеСПолученногоАванса.ДокументОплаты,
...
// Создает временную таблицу с колонками: см. ВзаиморасчетыСервер.ПодготовитьВТ_РасчетыСПоставщиками_ПереносыАвансов
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - 
//  ПараметрыРасчета        - см. ИнициализироватьПараметрыПодготовкиРасчетовАвансовВЦеляхНДС
...
//		* ОсновноеНалогообложениеНДСПродажи - ПеречислениеСсылка.ТипыНалогообложенияНДС - 
//		* ОсновнойВидДеятельностиНДСЗакупки - ПеречислениеСсылка.ТипыНалогообложенияНДС - 
Процедура ПодготовитьВТ_РасчетыСПоставщиками_ПереносыАвансов(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
