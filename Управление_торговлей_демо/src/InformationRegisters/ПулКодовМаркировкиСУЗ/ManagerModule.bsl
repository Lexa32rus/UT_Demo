		...
		Если ТекущийКодМаркировки <> Выборка.КодМаркировки Тогда
			...
			Если ТекущаяХешСуммаУпаковки <> Выборка.ХешСуммаУпаковки Тогда
				...
				Если ТекущийВидПродукции <> Выборка.ВидПродукции Тогда
					...
					ШаблонГрупповойУпаковки                 = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(ТекущийВидПродукции, 4);
				...
				Если ВидПродукцииИспользуетГрупповыеУпаковки
					И Выборка.Шаблон = ШаблонГрупповойУпаковки Тогда
...
Процедура РаспечататьЭтикеткиИЗафиксироватьДатуПечати(
	...
	Если Не ЭтоАдресВременногоХранилища(ДанныеПечати.АдресВХранилище) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьДанныеКодаМаркировки(ДанныеШтрихкода, ПараметрыСканирования, ДанныеЗаписи = Неопределено) Экспорт
	...
	Если ДанныеШтрихкода.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак
		...
		КонецЕсли;
	ИначеЕсли ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
...
Процедура РаспечататьЭтикеткиИЗафиксироватьДатуПечатиДляОбъектовПечати(ДанныеПечати, СтруктураОбъектовПечати,
	...
	Если РежимВыборочноВТаблицу Тогда
		...
		СтруктураНастроек.КомпоновщикНастроекПечатиЭтикетокВТаблицу = СтруктураОбъектовПечати.КомпоновщикНастроекПечатиЭтикетокВТаблицу;
	КонецЕсли;
		...
		Если ДанныеПечати.Свойство("ДополнитьПолныйКодМаркировки")
			...
			Для Каждого СтрокаТЧ Из ОбщаяТаблицаПечати Цикл
				...
				Если Лев(Прав(Штрихкод, 6), 2) = "93" Тогда
					Штрихкод = Штрихкод + РазделительGS + "3103" + СтрЗаменить(Формат(СтрокаТЧ.УчетноеКоличество, "ЧЦ=6; ЧДЦ=3; ЧН=000000; ЧВН=; ЧГ=0;"), ",", "");
...
Процедура ПечатьЭтикетокИСМПВТаблицу(ТабличныйДокумент, ТаблицаОбъектыПечати, СтруктураНастроек)
	...
	Для Каждого ПолеВыбора Из СтруктураНастроек.КомпоновщикНастроекПечатиЭтикетокВТаблицу.Настройки.Выбор.Элементы Цикл
		НовоеПолеВыбора = Компоновщик.Настройки.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		ЗаполнитьЗначенияСвойств(НовоеПолеВыбора, ПолеВыбора);
	КонецЦикла;
...
Процедура УстановитьСостояниеРаспечатанПоТаблицеДляПечати(ТаблицаДляПечати)
	...
	|	Таблица.ЗаказНаЭмиссию,
	|	Таблица.КодМаркировки,
	|	Таблица.ХешСуммаКодаМаркировки,
	|	Таблица.ПолныйКодМаркировки,
	|	Таблица.ПолныйКодМаркировкиИзменен
	|ПОМЕСТИТЬ Таблица
	|ИЗ
	|	&Таблица КАК Таблица
	|;
	...
	|		И (Таблица.ПолныйКодМаркировкиИзменен
	...
	|		КОЛИЧЕСТВО(1)
	|	ИЗ
	|		ТаблицаДляПечати КАК ТаблицаДляПечати) КАК ТаблицаИтоги
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|ГДЕ
	|	ВЫБОР
	|		КОГДА ТаблицаИтоги.ЗаписейДляИзменения = 0
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ТаблицаИтоги.ЗаписейПоДокументам / ТаблицаИтоги.ЗаписейДляИзменения < 100
	|	КОНЕЦ
	|
	|УПОРЯДОЧИТЬ ПО
	...
	ФормироватьНаборыЗаписей = РезультатЗапроса[ВГраница].Пустой();
	
	Если ФормироватьНаборыЗаписей Тогда
		
		НаборЗаписей = Неопределено;
		ТекущийЗаказ = Неопределено;
		Выборка = РезультатЗапроса[ВГраница - 1].Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.ЗаказНаЭмиссию <> ТекущийЗаказ Тогда
				ТекущийЗаказ = Выборка.ЗаказНаЭмиссию;
...
Процедура ОбработатьСтрокуСоставаМонотоварнойУпаковкиОдинаковойСтруктуры(ТаблицаДляЗаписиИстории, ПараметрыУпаковки, СтрокаМаркированнойПродукции)
	...
	Для Индекс = 0 По ВГраница Цикл
		...
		СтрокаСоставаУпаковки.ХешСуммаУпаковки = Справочники.ШтрихкодыУпаковокТоваров.ХэшСуммаСтроки(
