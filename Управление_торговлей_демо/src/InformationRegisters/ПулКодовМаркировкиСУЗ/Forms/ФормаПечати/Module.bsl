...
Процедура УстановитьВидимостьЭлементовПоРежимуФормирования(Форма)
	...
	Форма.Элементы.ДеревоАгрегированныхКодовШаблонЭтикетки.Видимость        = ФормированиеЭтикеток;
	Форма.Элементы.ДеревоАгрегированныхКодовКоличествоЭкземпляров.Видимость = ФормированиеЭтикеток;
	Форма.Элементы.СписокКодовМаркировкиШаблонЭтикетки.Видимость            = ФормированиеЭтикеток;
	...
	Иначе
		...
		Иначе
			Если НайденнаяСтрока = Неопределено
				...
				ШаблонГрупповойУпаковки = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(ВидПродукции, 4);
...
Функция ДанныеДляПечатиСвободныеКоды()
	...
	Иначе
		...
		ИтоговаяСтруктура.Вставить("КомпоновщикНастроекПечатиЭтикетокВТаблицу",
			КомпоновщикНастроекТабличногоДокумента);
	КонецЕсли;
