			...
			Пока ВыборкаПоТипуЗаписи.Следующий() Цикл
				...
				Иначе
					БазаСНДСРеглПоДокументу = 
						...
						- Окр(РасчетыПредоплатаВВалютеДоОтгрузки * КурсВалютыВзаиморасчетовДляБазыНДС, 2);
				КонецЕсли;
...
Процедура ПересчитатьТаблицуВВалютыУчета(МенеджерВременныхТаблиц)
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ОбъектыРасчетов.НалогообложениеНДС, Неопределено) КАК НалогообложениеНДС,
	...
	|	МАКСИМУМ(ТаблицаТоваров.ПериодБазыНДС)
	|ПО
...
Процедура ПересчитатьТаблицуВВалютыУчетаПострочно(ВыборкаЗнак, СуммыДокументовВВалютахУчета, КэшКурсовВалют)
	...
	Пока Выборка.Следующий() Цикл
		...
		ПериодБазыНДС = ?(ЗначениеЗаполнено(ВыборкаЗнак.ПериодБазыНДС), ВыборкаЗнак.ПериодБазыНДС, ВыборкаЗнак.Период);
