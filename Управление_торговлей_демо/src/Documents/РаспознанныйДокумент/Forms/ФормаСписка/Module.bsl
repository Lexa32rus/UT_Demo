...
&НаКлиенте
Перем СостоянияПоБалансу; // Хранилище состояний

&НаКлиенте
Перем СостоянияПоСтатусам; // Хранилище состояний
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АдресРезультатаРаспознавания = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
	...
	Элементы.СписокФильтрСтатусВОбработке.ЦветФона = WindowsЦвета.ГраницаНеактивногоОкна;
	Элементы.СписокФильтрСтатусНовые.ЦветФона = WindowsЦвета.ГраницаНеактивногоОкна;
	Элементы.СписокФильтрСтатусСОшибкой.ЦветФона = WindowsЦвета.ГраницаНеактивногоОкна;
	...
	//Элементы.ГрупповойВводДокументов.Видимость = Ложь;
	//Элементы.ВводКомплектовДокументов.Видимость = Истина;
...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("ЗагрузкаРаспознанныхДокументов", 0.1, Истина);
	
	ОбновитьПредставлениеБалансаНаКлиенте();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "РаспознанныйДокумент_СтатусОбработан" Тогда
		...
		ПодключитьОбработчикОжидания("Подключаемый_РазвернутьСтрокиДерева", 0.1, Истина);
...
Процедура ПроверитьЗаданияИЗагрузить(Команда)
	...
	ЗагрузкаРаспознанныхДокументов();
...
//	ОбщегоНазначенияБПКлиент.ОткрытьФормуСОжиданием(ПараметрыОткрытия, ПараметрыФормы);
	...
	ОткрытьФорму("Обработка.РаспознаваниеДокументов.Форма.СписокКомплектов", ПараметрыФормы);
...
Процедура ГрупповаяОбработкаДокументов(Команда)
	...
	ПараметрыФормы = Новый Структура("ОтборИзСписка", ОтборИзСписка);
	ОткрытьФорму("Обработка.ГрупповаяОбработкаРаспознанныхДокументов.Форма.ГрупповаяОбработка", ПараметрыФормы);
...
Процедура СохранитьАдресЭлПочты(Команда)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СостоянияПоБалансу) Тогда
			СостоянияПоБалансу.АдресЭлПочты = АдресЭлПочты;
		КонецЕсли;
...
Процедура ПослеДобавленияФайлов(КоличествоДобавленныхФайлов, ДополнительныеПараметры) Экспорт
	...
	Если СостоянияПоСтатусам <> Неопределено И ТипЗнч(КоличествоДобавленныхФайлов) = Тип("Число") Тогда
		СостоянияПоСтатусам.ФайловКОбработке = СостоянияПоСтатусам.ФайловКОбработке + КоличествоДобавленныхФайлов;
...
Процедура ОбработатьНажатиеКнопкиСтатистики(Фильтр, Кнопка)
	...
	Если Кнопка.Пометка Тогда
		Кнопка.ЦветФона = WindowsЦвета.ГраницаНеактивногоОкна;
	Иначе
...
Процедура ОбновитьКоличествоФайловПоСтатусам()
	...
	//КонецЕсли;
	//
	//Элементы.СписокФильтрСтатусВОбработке.Заголовок = СтрШаблон(НСтр("ru = 'На распознавании: %1'"), ФайловКОбработке);
	...
	// Вызывавется РаспознаваниеДокументов.СостоянияПоСтатусам()

	Если СостоянияПоСтатусам = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьПредставлениеБалансаНаКлиенте()
	...
	ИначеЕсли СостоянияПоБалансу.СостояниеБаланса = "ПилотныйРежимДатаОкончания" Тогда
		...
		Элементы.ГруппаБиллинг.ЦветФона = СтандартныеПодсистемыКлиент.ЭлементыСтиля()["ЦветФонаПредупреждения"];
...
Процедура ЗагрузкаРаспознанныхДокументов()
	...
	НеВыполнятьОбновление = Не Элементы.ГруппаБиллингПодробно.Скрыта()
	...
	Если ВыполняетсяЗагрузка Тогда
		...
		Результат = ПолучитьИзВременногоХранилища(АдресРезультатаРаспознавания);
		
		// Обработка результата
		Если Результат <> Неопределено Тогда
			
			СостоянияПоСтатусам = Результат.СостоянияПоСтатусам;
			СостоянияПоБалансу = Результат.СостоянияПоБалансу;
			
			Элементы.Список.Обновить();
			
			ВыполняетсяЗагрузка = Ложь;
			ОтключитьОбработчикОжидания("ЗагрузкаРаспознанныхДокументов");
			ПодключитьОбработчикОжидания("ЗагрузкаРаспознанныхДокументов", 30);
	...
	Иначе
		...
		ВыполняетсяЗагрузка = ЗапросНовогоРезультата(АдресРезультатаРаспознавания);
		
		ОтключитьОбработчикОжидания("ЗагрузкаРаспознанныхДокументов");
		ПодключитьОбработчикОжидания("ЗагрузкаРаспознанныхДокументов", 3);
...
&НаСервереБезКонтекста
Функция ЗапросНовогоРезультата(Знач АдресРезультатаРаспознавания)
	
	Возврат РаспознаваниеДокументовСлужебный.ЗагрузкаРаспознанныхДокументов(АдресРезультатаРаспознавания);
...
&НаКлиенте
Процедура Подключаемый_РазвернутьСтрокиДерева()
