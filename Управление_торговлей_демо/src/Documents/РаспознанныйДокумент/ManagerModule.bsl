...
Процедура ЗаписатьИзмененияРеквизитов(ДанныеИзменений, АдресРезультата = Неопределено) Экспорт
	...
	Для Каждого СтрокаИзменений Из ДанныеИзменений Цикл
		...
		РаспознаваниеДокументовСлужебныйКлиентСервер.УстановитьЗначениеРеквизитаДокумента(ДокументОбъект, СтрокаИзменений.ИмяРеквизита, СтрокаИзменений.Значение);
...
Процедура УдалитьУстаревшиеДокументы() Экспорт
	...
	Дата = Перечисления.ПериодыОтсрочкиУдаленияУстаревшихДокументов.ДатаОтбораДляУдаления(Настройки.ПериодОтсрочкиУдаленияУстаревшихДокументов);
	...
	Запрос.Текст = 
		...
		|			ИЛИ РаспознанныйДокумент.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСозданныхДокументовРаспознаваниеДокументов.Обработан))
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		НачатьТранзакцию();
		Попытка
			УдаляемыеОбъекты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка);
			Результат = УдалениеПомеченныхОбъектов.УдалитьПомеченныеОбъекты(УдаляемыеОбъекты);
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				...
				Объект.Ссылка,
				СтрШаблон(
					НСтр("ru = 'Не удалось удалить документ как устаревший:
					           |%1'"),
		...
		Успешно = (Результат.НеУдаленные.Количество() = 0);
		...
		Если Не Успешно Тогда
			...
			ЗаписьЖурналаРегистрации(
				...
				Объект.Ссылка,
				СтрШаблон(
					НСтр("ru = 'Не удалось удалить документ как устаревший, т.к. обнаружены ссылки на объект:
					           |%1'"),
					ПредставлениеПрепятствующихУдалению(Результат.ПрепятствующиеУдалению)
				)
			);
		КонецЕсли;
...
Функция ПредставлениеПрепятствующихУдалению(ПрепятствующиеУдалению)
	
	Строки = Новый Массив;
	НомерСтроки = 0;
	Для Каждого Препятствующий Из ПрепятствующиеУдалению Цикл
		...
		Строки.Добавить(
			СтрШаблон(
				...
				Препятствующий.Метаданные,
				Препятствующий.МестоИспользования,
				Препятствующий.ПодробноеОписаниеОшибки
			)
		);
	КонецЦикла;
...
Функция СоздатьРаспознанныйДокументV2(СвойстваРаспознанногоДокумента, РаспознанныйДокумент, ИдентификаторЗадания, СостояниеЗадания) Экспорт
	...
	ТаблицаПроблем = РаспознаваниеДокументовСлужебный.ПроблемныеРеквизиты(НовыйДокумент);
	НовыйДокумент.КоличествоОшибок = ТаблицаПроблем.Количество();
	НовыйДокумент.КоличествоОшибокВТаблицах = НовыйДокумент.КоличествоОшибок - ТаблицаПроблем.Скопировать(Новый Структура("НомерСтроки", 0), "ИмяРеквизита").Количество();
