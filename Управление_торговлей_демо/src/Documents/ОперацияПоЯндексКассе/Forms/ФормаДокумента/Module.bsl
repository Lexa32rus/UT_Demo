...
Процедура ОснованиеПлатежаОбработкаВыбораСервер(ВыбранноеЗначение)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту Тогда
		...
		Выборка = ВыборкаОснованияПлатежаДляВозврата();
		Выборка.Следующий();
		
		Если Выборка.Количество() = 1 Тогда
			
			Объект.ОснованиеПлатежа = Выборка.Документ;
...
Функция ДанныеЭквайераПоУмолчанию(ДатаСреза)
	...
	Возврат Справочники.НастройкиОнлайнОплат.ДанныеЭквайераПоУмолчанию(ДатаСреза);
...
&НаСервере
Функция ВыборкаОснованияПлатежаДляВозврата()

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ОперацияПоЯндексКассе.Ссылка КАК Документ,
		|	ОперацияПоЯндексКассе.Партнер КАК Партнер,
		|	ОперацияПоЯндексКассе.Контрагент КАК Контрагент,
		|	ОперацияПоЯндексКассе.СуммаДокумента КАК СуммаДокумента,
		|	ОперацияПоЯндексКассе.СуммаКомиссии КАК СуммаКомиссии,
		|	ОперацияПоЯндексКассе.Валюта КАК Валюта,
		|	ОперацияПоЯндексКассе.ВалютаКомиссии КАК ВалютаКомиссии,
		|	ОперацияПоЯндексКассе.СуммаНДС20 КАК СуммаНДС20,
		|	ОперацияПоЯндексКассе.СуммаНДС10 КАК СуммаНДС10
		|ИЗ
