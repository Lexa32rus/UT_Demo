...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаРозничнаяПродажа
		...
		Или Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаПродажаЧерезВендинговыйАппарат Тогда
...
Функция СтатусПослеПередачиДанных(ДокументСсылка, Операция, СтатусОбработки) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаРозничнаяПродажа
		...
		Или Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаУтилизацияТовара
		Или Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаФасовка Тогда
...
Функция ВыводИзОборотаJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Иначе
		...
		ИспользованиеКонтрагента               = ИнтеграцияИСМПСлужебныйКлиентСервер.ИспользованиеКонтрагентаПриВыводеИзОборота(Шапка.Операция);
		ИспользованиеКонтрагентаВсегда         = ИнтеграцияИСМПСлужебныйКлиентСервер.ИспользованиеКонтрагентаПриВыводеИзОборота(Шапка.Операция, Истина);
		...
		Для Каждого СтрокаТЧМаркированныеТовары Из ДанныеПоТоварам Цикл
			...
			Если НайденныеСтроки.Количество() = 1 Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаТЧТовары.Цена) И ИспользованиеЦеныВсегда Тогда
					ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
						СообщениеJSON,
						СтрШаблон(
				...
				Если ИспользованиеЦены Тогда
					...
					Если Шапка.ОбъемноСортовойУчет
						...
						СтрокаТЧ.Вставить("product_cost", 0);
					ИначеЕсли ЗначениеЗаполнено(СтрокаТЧТовары.Цена) Тогда
...
Функция ЗаполнитьДоступныеGTIN(Объект) Экспорт
	
	ТребуетсяОбновитьКлючСессии = Ложь;
	ОграничениеПоВидуУпаковки   = Новый Массив();
	ОграничениеПоВидуУпаковки.Добавить(Перечисления.ВидыУпаковокИС.Потребительская);
	ОграничениеПоВидуУпаковки.Добавить(Перечисления.ВидыУпаковокИС.Набор);
	ОграничениеПоВидуУпаковки.Добавить(Перечисления.ВидыУпаковокИС.Неопределен);
	
	ДанныеДоступныхGTIN = Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ДанныеДоступныхGTIN(Объект,,, ОграничениеПоВидуУпаковки);
	...
	Если ДанныеДоступныхGTIN.ПроверяемыеGTIN.Количество() > 0 Тогда
		...
		Для Каждого СтрокаДокумента Из Объект.Товары Цикл
			...
			Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
				...
				Если СтрокаТаблицы.ЕстьGTIN
					И СтрокаТаблицы.ДоступныеGTIN.Количество()
					И Не ЗначениеЗаполнено(СтрокаДокумента.GTIN) Тогда
