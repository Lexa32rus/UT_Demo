...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты ""Настройка счетов учета"", ""Группа финансового учета""'");
	...
	НоваяСтрока.Процедура = "ГруппыФинансовогоУчетаРасчетов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.Бронирование КАК Документ
	|ГДЕ
	|	Документ.ГруппаФинансовогоУчета = ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаРасчетов.ПустаяСсылка)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	ДанныеСправочника.РасчетыПоБронированию
	|	И НЕ ДанныеСправочника.ПометкаУдаления
	|";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ГФУБилетов = Выборка.Ссылка;
	КонецЕсли;
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(ДокументОбъект.ГруппаФинансовогоУчета) Тогда
					ДокументОбъект.ГруппаФинансовогоУчета = ГФУБилетов;
				КонецЕсли;
			КонецЕсли;
