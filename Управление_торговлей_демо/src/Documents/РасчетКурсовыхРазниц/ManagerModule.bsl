...
Процедура РассчитатьКурсовыеРазницы(Знач Параметры, КоличествоОбработанныхЭлементов = 0) Экспорт
	...
	ЭтоАренда = Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереоценкаРасчетовПоАренде;
...
Функция ТекстЗапросаКурсовыеРазницыРасчетыПоФинансовымИнструментам()
	...
	|	ВЫБОР КОГДА ОборотыЗаДень.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаРеглПриход,
	|	ВЫБОР КОГДА ОборотыЗаДень.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		ТОГДА ОборотыЗаДень.Сумма
	|		ИНАЧЕ 0
	...
	|	И (ОборотыЗаДень.Договор.ВалютаВзаиморасчетов <> &ВалютаУправленческогоУчета
	|		ИЛИ ОборотыЗаДень.Договор.ВалютаВзаиморасчетов <> &ВалютаРегламентированногоУчета)
	|	И (ОборотыЗаДень.ТипСуммы В (&ТипыСумм) ИЛИ &ПоВсемТипамСумм)
	|	И (&ВсеДвижения
	|		ИЛИ ОборотыЗаДень.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
