...
Процедура ВидПродукцииПриИзменении(Элемент, ПерезаполнитьПоОснованию = Ложь)
	...
	ОперацииНанесенияПоВидуПродукции = ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки(Ложь, Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха"));
...
Процедура ИдентификаторПроисхожденияВЕТИССтрокойПриИзменении(Элемент)
	...
	ИдентификаторПроисхожденияВЕТИССтрокой = ИнтеграцияИСМПКлиентСервер.ПреобразоватьИдентификаторВСД(ИдентификаторПроисхожденияВЕТИССтрокой);
...
Процедура ПроверитьСостояниеКодовМаркировкиНемедленно(Команда)
	...
	УстановленОтборПоОшибке = ЭтотОбъект.ОтборСтрок = "СОшибками";
...
Функция ОбновитьСтатусыКодовМаркировкиНаСервере(ВыполнитьПолнуюПроверку)
	...
	УстановитьВидимостьЭлементовСтатусИННВладельца(ЭтотОбъект.Элементы, Истина);
...
Функция ЗапроситьСтатусыКодовМаркировки(ТаблицаПроверки, Документ, ВидМаркируемойПродукции, Организация, ПолучитьСтатусыВсехКодов = Ложь)
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияМОТП(ВидМаркируемойПродукции) Тогда
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ИнтеграцияИСПереопределяемый.НастроитьПодключаемоеОборудование(ЭтотОбъект, "Товары,ПредставлениеКИЗ");
	ИнтеграцияИСПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект, Объект.Товары);
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	ДоступенОтчетПроизводственнойЛинии =
		Не ЭтоМех
			...
			И (ЭтоПродукцияМОТП Или ЭтоПиво Или Не ОперацииМаркировки.ЭтоОперацияНанесения))
	...
	Если (ЭтоМолочнаяПродукция Или ЭтоБАД) И ПрисутствуетОперацияНанесения Тогда
	...
	Элементы.ДатаПроизводства.Видимость = (ЭтоКонтрактноеПроизводство
		...
		Или (ЭтоОперацияНанесения И (ЭтоБАД Или ЭтоПиво)));
	...
	Элементы.ГруппаСрокГодности.Видимость      = ЭтоМолочнаяПродукция Или ЭтоБАД;
	...
	ИначеЕсли ВозможнаОперацияНанесения Тогда
		...
		Если ЭтоМолочнаяПродукция Или ЭтоБАД Тогда
	...
	ИначеЕсли ЭтоИмпортСФТС Тогда
		...
		ТребуетсяУказатьДанныеДекларации(Форма);
		ИнтеграцияИСКлиентСервер.УправлениеДоступностьюЭлементовФормы(
			Форма,
...
Процедура ЗаполнитьДоступныеОперации()
	...
	ВидыПродукцииОперацииИмпортСФТС.Вставить(Перечисления.ВидыПродукцииИС.Велосипеды, Истина);
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Объект.ВидПродукции, Ложь, Истина, Ложь) Тогда
		...
		Если Не ЭтоОбувь И Не ЭтоМех Тогда
			Элементы.Операция.СписокВыбора.Добавить(
		...
		Если (ИспользуетсяПодсистемаВЕТИС Или Не ЭтоМолочнаяПродукция) И Не ЭтоМех Тогда
			Элементы.Операция.СписокВыбора.Добавить(
		...
		Элементы.ВариантВыполненияАгрегации.СписокВыбора.Добавить(Перечисления.ВариантыВыполненияАгрегацииИСМП.ПустаяСсылка(),
...
Процедура ОбновитьИденификаторПримененияВЕТИССрокГодности()
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(Объект.ВидПродукции)
		Или Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы Тогда
...
Процедура ОчиститьНеиспользуемыеКолонкиТаблицыТовары(Форма, ПараметрыОбработки = Неопределено)
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(Объект.ВидПродукции)
		Или Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы") Тогда
		НужноЗначениеСрокГодности = ЗначениеЗаполнено(Объект.ОперацияНанесения)
			Или ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(Объект.Операция)<>Неопределено;
	КонецЕсли;
...
Функция КлючСвязиСтатусаСтрок(Форма, Номенклатура = Неопределено)
	...
	Если ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(Форма.Объект.Операция) <> Неопределено
		...
		ИначеЕсли Форма.Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукцияБезВЕТИС")
				Или Форма.Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы") Тогда
			МассивКлючейСвязи.Добавить("СрокГодности");
		КонецЕсли;
...
Процедура ОпределитьФорматРедактированияСрокГодности()
	...
	ИначеЕсли Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы") Тогда
		ФорматДаты = "ДФ=dd.MM.yyyy;";
		Элементы.ТоварыСрокГодности.ФорматРедактирования = ФорматДаты;
	Иначе
...
Процедура УстановитьФорматСрокГодности(Форма)
	...
	ИначеЕсли Форма.Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы") Тогда
		ФорматДаты = "ДФ=dd.MM.yyyy;";
		Форма.Элементы.СрокГодности.Формат               = ФорматДаты;
		Форма.Элементы.СрокГодности.ФорматРедактирования = ФорматДаты;
	Иначе
