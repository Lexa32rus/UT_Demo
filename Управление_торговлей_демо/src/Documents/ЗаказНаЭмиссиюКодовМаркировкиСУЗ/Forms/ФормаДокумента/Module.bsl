...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Иначе
		...
		Если (ИспользуютсяНаборы
			И ДанныеСтроки.Шаблон = ШаблонНабора)
			Или (ИспользуютсяГрупповыеУпаковки
			И ДанныеСтроки.Шаблон = ШаблонГрупповойУпаковки) Тогда
...
Процедура ТоварыТоварныйЗнакПриИзменении(Элемент)
	...
	Если ИспользуютсяГрупповыеУпаковки
		И ТекущиеДанные.Шаблон = ШаблонГрупповойУпаковки
		И ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтрокиВладельца) Тогда
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтрокиВладельца) Тогда
		Если ИспользуютсяНаборы
			И ТекущиеДанные.Шаблон = ШаблонНабора Тогда
		...
		ИначеЕсли ИспользуютсяГрупповыеУпаковки
			И ТекущиеДанные.Шаблон = ШаблонГрупповойУпаковки Тогда
...
Процедура УстановитьОтборПоНаборуГрупповойУпаковке(Команда)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтрокиВладельца) Тогда
		Если (ИспользуютсяНаборы
			И ТекущиеДанные.Шаблон = ШаблонНабора)
			Или (ИспользуютсяГрупповыеУпаковки
			И ТекущиеДанные.Шаблон = ШаблонГрупповойУпаковки) Тогда
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы(Форма)
	...
	Форма.ШаблонГрупповойУпаковки       = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(Объект.ВидПродукции, 4);
	Форма.ШаблонНабора                  = ИнтеграцияИСМПКлиентСервер.ШаблонКодаМаркировкиПоВидуПродукции(Объект.ВидПродукции, 5);
...
Процедура ПослеОбработкиШтрихкодовСервер(РезультатОбработкиШтрихкода, КэшированныеЗначения)
	...
	ДобавленныеСтрокиНаборов           = Новый Массив;
	...
	Для Каждого ДобавленнаяСтрока Из ДобавленныеСтроки Цикл
		...
		Если ИспользуютсяНаборы И ДобавленнаяСтрока.Шаблон = ШаблонНабора Тогда
			ДобавленныеСтрокиНаборов.Добавить(ДобавленнаяСтрока);
		ИначеЕсли ИспользуютсяГрупповыеУпаковки И ДобавленнаяСтрока.Шаблон = ШаблонГрупповойУпаковки Тогда
			ДобавленныеСтрокиГрупповыхУпаковок.Добавить(ДобавленнаяСтрока.ПолучитьИдентификатор());
		КонецЕсли;

	КонецЦикла;
	
	Если ДобавленныеСтрокиНаборов.Количество() > 0 Тогда
		ПерезаполнитьНаборы(ДобавленныеСтрокиНаборов, КэшированныеЗначения);
	КонецЕсли;
	...
	Для Каждого ИзмененнаяСтрока Из ИзмененныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ИзмененнаяСтрока.ИдентификаторСтрокиВладельца) Тогда
			Если ИспользуютсяНаборы
				И ИзмененнаяСтрока.Шаблон = ШаблонНабора Тогда
				ПересчитатьКоличествоСоставляющихНабора(ИзмененнаяСтрока, КэшированныеЗначения);
			ИначеЕсли ИспользуютсяГрупповыеУпаковки
				И ИзмененнаяСтрока.Шаблон = ШаблонГрупповойУпаковки Тогда
	...
	Для Каждого Строка Из МассивСтрок Цикл
		Если ЗначениеЗаполнено(Строка.ИдентификаторСтрокиВладельца) Тогда
			...
			Если СтрокаВладелец = Неопределено Тогда
				...
				КонецЕсли;
				Если СтрокаВладелец.Шаблон = Форма.ШаблонНабора Тогда
					СтрокаВладелец.ЭтоНабор = Истина;
				ИначеЕсли СтрокаВладелец.Шаблон = Форма.ШаблонГрупповойУпаковки Тогда
...
Процедура ОбработатьДобавленныеСтрокиТоваров(ДобавленныеСтроки, ДополнительныеПоляПоиска = "")
	...
	ЗаполнениеОбъектовПоСтатистикеИСМП.ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(
		Объект, ДобавленныеСтроки, ДополнительныеПоляПоиска);
...
Процедура ЗаполнитьПустойGTINВСтроках(МассивСтрок)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
		...
		Если СтрокаТовары.Шаблон = ШаблонГрупповойУпаковки Тогда
			ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая;
		ИначеЕсли СтрокаТовары.Шаблон = ШаблонНабора Тогда
...
Процедура ВыполнитьПерезаполнениеНаборовИГрупповыхУпаковок(МассивСтрок)
	...
	Для Каждого Строка Из МассивСтрок Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца)
			...
			Или ИспользуютсяНаборы И СтрокаТовары.Шаблон = ШаблонНабора
			Или ИспользуютсяГрупповыеУпаковки И СтрокаТовары.Шаблон = ШаблонГрупповойУпаковки Тогда
			...
			Если ИспользуютсяНаборы И СтрокаТовары.Шаблон = ШаблонНабора
				Или ИспользуютсяГрупповыеУпаковки И СтрокаТовары.Шаблон = ШаблонГрупповойУпаковки Тогда
	...
	Для Каждого Строка Из СтрокиДляОбработки Цикл
		...
		КонецЕсли;
		Если Строка.Шаблон = ШаблонНабора Тогда
			СтрокиДляОбработкиНаборы.Добавить(Строка.ПолучитьИдентификатор());
		ИначеЕсли Строка.Шаблон = ШаблонГрупповойУпаковки Тогда
...
Процедура ПерезаполнитьНаборы(МассивСтрок, КэшированныеЗначения)
	...
	ЗаполнитьСоставляющиеНаборов(ПолучитьМассивСтрокПоИдентификаторам(МассивСтрок, Объект.Товары), КэшированныеЗначения, ДобавленныеСтроки);
...
Процедура ОбработатьИзменениеШаблонаНабораИГрупповойУпаковки(ДанныеСтроки)
	...
	Если ДанныеСтроки.Шаблон <> ШаблонДоИзменения
		И ((ДанныеСтроки.Шаблон = ШаблонГрупповойУпаковки Или ДанныеСтроки.Шаблон = ШаблонНабора)
			Или (ШаблонДоИзменения = ШаблонГрупповойУпаковки Или ШаблонДоИзменения = ШаблонНабора)
			И ЗначениеЗаполнено(ДанныеСтроки.ИдентификаторСтрокиВладельца)) Тогда

		Если ДанныеСтроки.Шаблон = ШаблонГрупповойУпаковки И ШаблонДоИзменения = ШаблонНабора
			Или ДанныеСтроки.Шаблон = ШаблонНабора И ШаблонДоИзменения = ШаблонГрупповойУпаковки Тогда
			...
			Если ДанныеСтроки.Шаблон = ШаблонГрупповойУпаковки Тогда
		...
		ИначеЕсли ДанныеСтроки.Шаблон = ШаблонГрупповойУпаковки Или ДанныеСтроки.Шаблон = ШаблонНабора Тогда
			...
			Если ДанныеСтроки.Шаблон = ШаблонНабора
