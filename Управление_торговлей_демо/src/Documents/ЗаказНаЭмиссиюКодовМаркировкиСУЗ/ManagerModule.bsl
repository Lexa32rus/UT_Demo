...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды = Неопределено) Экспорт
	...
	Если Результат.ДобавленныеСтроки.Количество() Тогда
		ЗаполнениеОбъектовПоСтатистикеИСМП.ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(
			Форма.Объект, Результат.ДобавленныеСтроки, "GTIN");
	КонецЕсли;
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияМОТП(Шапка.ВидПродукции) Тогда
		...
		Для Каждого СтрокаТЧ Из ТабличнаяЧастьТовары Цикл
			...
			Иначе
				...
				ИнтеграцияИСМП.ДанныеИзСтрокиКонтактнойИнформации(Шапка.ПроизводственныйОбъектАдрес, ДанныеАдреса);
	...
	ИначеЕсли ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции) Тогда
		...
		Для Каждого СодержаниеЗаказа Из МассивЗаказов Цикл
			...
			Для Каждого СтрокаТЧ Из СодержаниеЗаказа Цикл
				...
				Если Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха Тогда
					Если ЗначениеЗаполнено(СтрокаТЧ.GTIN) Тогда
						СтрокаЗаказаКодовМаркировки.Вставить("gcp",  Лев(СтрокаТЧ.GTIN, 7));
					КонецЕсли;
...
Функция МассивЗаказовПоТаблицеТоваров(ВходнаяТаблицаТовары, МаксимальноеКоличествоКодовПоСтроке, ВидПродукции)
	...
	Иначе
		ПоляСвертки  = "GTIN, СпособФормированияСерийногоНомера, Шаблон, СрокГодности";
		ПоляПодсчета = "GTIN";
	КонецЕсли;
	...
	Пока Истина Цикл
		...
		Если ЭтоМех Тогда
			ТаблицаПодсчетаGTIN.Сортировать("GTIN Убыв, КоличествоСтрок Убыв, Количество Убыв");
		Иначе
	...
	Для Каждого СтрокаПрисоединенныйФайл Из ПрисоединенныеФайлы Цикл
		...
		Если ТипЗнч(ПротоколОбмена) <> Тип("ТаблицаЗначений") Тогда
			...
			СообщениеJSON.ТекстОшибки =  СтрШаблон(
				...
				СтрокаПрисоединенныйФайл.СтрокаПрисоединенныйФайл);
			СообщенияJSON.Добавить(СообщениеJSON);
			Возврат СообщенияJSON;
		КонецЕсли;
