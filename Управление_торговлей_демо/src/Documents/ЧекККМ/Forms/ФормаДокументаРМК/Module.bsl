...
Функция ИнформацияОбОплате(Форма)
	...
	ИнформацияОбОплате.Вставить("ПлатежныеКарты",        ОплатыПлатежнымиКартами(Форма.Объект.ОплатаПлатежнымиКартами));
	...
	ИтогоОплачено = ИнформацияОбОплате.Наличные
...
Функция ОплатыПлатежнымиКартами(ЗНАЧ ОплатаПлатежнымиКартами)
	...
	ОтборОплатыПлатежнымиКартами = Новый Структура("ВидОплаты", ПредопределенноеЗначение("Перечисление.ТипыПлатежнойСистемыККТ.ПустаяСсылка"));
	МассивСтрокОплаты = ОплатаПлатежнымиКартами.НайтиСтроки(ОтборОплатыПлатежнымиКартами);
...
Функция ПараметрыОперацииФискализацииЧекаНаСервереЛокализация()
	...
	ФормированиеПараметровФискальногоЧекаСервер.ОбновитьПараметрыФискальногоЧекаСертификатыФСС(
		ПараметрыОперацииФискализацииЧека,
		ОплатыЭлектроннымиСертификатами());
...
&НаСервере
Функция ОплатыЭлектроннымиСертификатами()
	
	ОплатыЭлектроннымиСертификатами = Новый Структура;
	ОплатыЭлектроннымиСертификатами.Вставить("ОплатаЭСФССиМИР", Объект.ОплатаПлатежнымиКартами);
	
	Возврат ОплатыЭлектроннымиСертификатами;
...
Процедура ОтправитьПодтверждениеНСПКНаКлиенте(ТоварыФССКОплате)
	...
	ДополнительныеПараметры = Новый Структура("СуммаСертификатамиФСС", СуммаОплатыСертификатамиФСС);
	ОтправитьПодтверждениеНСПКНаСервере(Объект.Ссылка, ДополнительныеПараметры.СуммаСертификатамиФСС);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОтправитьПодтверждениеНСПКНаСервере(ЧекККМ, СуммаСертификатамиФСС)
	
	Результат = ЭлектронныеСертификатыНСПКУТ.ПередачаДанныхФискальногоЧека(ЧекККМ, СуммаСертификатамиФСС);
...
Процедура КонтрольКодовТРУ(Знач ТолькоПриИзмененииКоличестваСтрок = Ложь)
	...
	Для каждого Элем Из ПолучитьЗапросомТРУ Цикл
		...
		Для каждого ЭлемСтрокаТовары Из Объект.Товары.НайтиСтроки(ОтборСтрок) Цикл
			...
			ЕстьТоварыСВозможнотьюОплатыЭС = ЕстьТоварыСВозможнотьюОплатыЭС ИЛИ ЕстьКодТРУ;
		КонецЦикла;
	КонецЦикла;
	
	КэшКодовТРУ.КоличествоСтрокНаМоментОбработки = Объект.Товары.Количество();
	
	КнопкаОплаты.Доступность = ЕстьТоварыСВозможнотьюОплатыЭС;
