...
Процедура СтатусТоваровФСС(Команда)
	...
	ДанныеДляПечати.Вставить("ТоварыФСС", ТоварыФСС);
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если НЕ ДополнительныеПараметры = Неопределено И ДополнительныеПараметры.Свойство("СледующаяОперация") Тогда
			Если ДополнительныеПараметры.СледующаяОперация = "ПредварительноеОдобрениеИспользования" Тогда
				СогласоватьЭСФССНаСервере(ДополнительныеПараметры.ТоварныеПозиции, ХешНомерКарты);
			ИначеЕсли ДополнительныеПараметры.СледующаяОперация = "ПредварительноеОдобрениеВозврата" Тогда
				СогласоватьВозвратЭСФССНаСервере(ДополнительныеПараметры.ТоварныеПозиции, ХешНомерКарты);
			Иначе
...
&НаСервере
Процедура СогласоватьЭСФССНаСервере(ТоварныеПозиции, ХешНомерКарты)
	
	ПараметрыОперации = ЭлектронныеСертификатыНСПКУТ.ПараметрыОперацииНСПКПоКассеККМ(КассаККМ);
...
&НаСервере
Процедура СогласоватьВозвратЭСФССНаСервере(ТоварныеПозиции, ХешНомерКарты)
	
	ПараметрыОперации = ЭлектронныеСертификатыНСПКУТ.ПараметрыОперацииНСПКПоКассеККМ(КассаККМ, ИдентификаторКорзины);
...
Функция ПроверитьВозможностьОплатыЭСФССНаСервере(Организация, КассаККМ, Знач ПодключенныеЭквайринговыеТерминалы)
	...
	АдресМагазина = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
		КассаККМ.Склад, Перечисления.ТипыКонтактнойИнформации.Адрес, ТекущаяДатаСеанса(), Ложь);
	Если Не ЗначениеЗаполнено(АдресМагазина) Тогда
			ЧастиФорматированнойСтроки.Добавить(
				...
				ПолучитьНавигационнуюСсылку(КассаККМ.Склад)));
	...
	Если Не ЕстьНастроенныйТерминал Тогда
		ЧастиФорматированнойСтроки.Добавить(
			СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'В доступные виды оплаты эквайрингового терминала необходимо добавить вид оплаты Оплата ЭС ФСС'")));
			ЧастиФорматированнойСтроки.Добавить(Символы.ПС);
	КонецЕсли;
...
Процедура УстановитьЗаголовкиОплатыДляЭСФСС(Форма)
	...
	Иначе
		...
		Если Форма.ЭтоВозврат Тогда
			ТекстПодсказки = НСтр("ru = 'Перед возвратом необходимо проверить доступную сумму к возврату ЭС ФСС.'");
		Иначе
			ТекстПодсказки = НСтр("ru = 'Перед оплатой необходимо проверить доступную сумму к оплате ЭС ФСС.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьВидимостьЭлементов(ИнформацияОбОплате)
	...
	Если ЭтоОплатаЭСФСС Тогда
		...
		Если ЭтоВозврат Тогда
			Заголовок = НСтр("ru = 'Возврат оплаты ЭС ФСС'");
		Иначе
			Заголовок = НСтр("ru = 'Оплата ЭС ФСС'");
		КонецЕсли;
