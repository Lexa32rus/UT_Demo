...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.СписаниеПроверкаСтатусаОбработкиДокумента Тогда
...
#Область ДанныеОтчетаПроизводственнойЛинии
		...
		Если ЭтоТабак И Не ЕстьОшибки И КодыМаркировки.Количество() Тогда
			// 2. Поиск.
...
#КонецОбласти
	...
	ИначеЕсли Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак
		...
		Если ДанныеСтраны.Ссылка = ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
			...
			ИнтеграцияИСМПСлужебный.ЗаполнитьСвойствоXDTO(АдресРФXDTO, "НаселенныйПункт", НаселенныйПункт, СообщениеJSON);
		...
		Если Шапка.ОтчетПроизводственнойЛинии Тогда
			...
			Для Каждого СтрокаДанныхОтчета Из НормализованныеДанныеОтчета Цикл
				...
				МассивКодовМаркировки.Добавить(СтрокаДанныхОтчета.КодДляПередачиИСМП);
		...
		Иначе
			...
			Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
				...
				РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкодаПоДаннымРазбора(
					СтрокаТЧМаркированныеТовары.Штрихкод, Шапка.ВидПродукции, СтрокаТЧМаркированныеТовары.ДанныеРазбора);
