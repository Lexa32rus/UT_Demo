...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|	ТОГДА
		|		ВЫРАЗИТЬ (ЕСТЬNULL(ОплаченоПоЗаказам.КОплатеРасход, 0) * 100 / ДокументЗаявка.СуммаЗамены КАК ЧИСЛО(20, 10))
		|	ИНАЧЕ
		|		0
		...
		|ПО
		|	ДокументЗаявка.Ссылка = ОплаченоПоЗаказам.ОбъектРасчетов.Объект
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
