...
Функция ТаблицаСнятияРезерва(Назначение, Товары, Регистратор) Экспорт
	...
	Текст =
		...
		|	СУММА(ТаблицаИтогов.Обеспечено) - МАКСИМУМ(ЕСТЬNULL(ЗаказыНаПеремещение.КоличествоБезУчетаДинамическихПеремещений, 0)) КАК Обеспечено,
...
Функция ТаблицаРезервирования(Назначение, Источники, ТолькоСкладОтгрузки, Организация, Товары, Регистратор) Экспорт
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		Запрос.Текст =
			...
			|ГДЕ
			|	ВЫБОР КОГДА &ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21)
			|			ТОГДА
...
Функция ВременнаяТаблицаЗаказыНаПеремещение(ВТаблицеИтоговЕстьНазначение)
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		Текст =
			...
			|	Таблица.Номенклатура КАК Номенклатура,
			|	Таблица.Характеристика КАК Характеристика,
			|	СУММА(ВЫБОР КОГДА Таблица.ЭтоДинамическоеПеремещение ТОГДА
			|				0
			|				ИНАЧЕ
			|					ВЫБОР КОГДА Таблица.КОбеспечению > Таблица.Запас Тогда
			|								Таблица.Запас
			|							ИНАЧЕ
			|						Таблица.КОбеспечению
			|					КОНЕЦ
			|			КОНЕЦ) КАК КоличествоБезУчетаДинамическихПеремещений,
			...
			|ИЗ(
			|	ВЫБРАТЬ
			|		ЛОЖЬ КАК ЭтоДинамическоеПеремещение,
	...
	Иначе
		Текст =
			...
			|		Набор.Номенклатура КАК Номенклатура,
			|		Набор.Характеристика КАК Характеристика,
			|		Набор.ЭтоДинамическоеПеремещение КАК ЭтоДинамическоеПеремещение,
			...
			|		Набор.Характеристика,
			|		Набор.Назначение,
			|		Набор.ЭтоДинамическоеПеремещение
			|	ИМЕЮЩИЕ
