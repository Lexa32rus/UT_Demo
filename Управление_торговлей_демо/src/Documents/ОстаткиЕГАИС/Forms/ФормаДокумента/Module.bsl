...
Процедура ВыполнитьОчереднойЗапрос()
	...
	Если ДанныеТекущегоЗапроса <> Неопределено Тогда
		...
		Если Не ДанныеТекущегоЗапроса.ЗапросОтправлен Тогда
			...
			Если ОсталосьДоОтправкиЗапроса > 0 Тогда
				СостояниеЗапросаМарок = Новый ФорматированнаяСтрока(
					СтрШаблон(
						НСтр("ru = 'Запрос марок готовится к отправке'"),
		...
		Иначе
			...
			// Ожидали ответ 12 минут, он не пришел, необходимо выполнить обмен безусловно
			Если ДанныеТекущегоЗапроса.Ссылка <> Неопределено
				И ТекущаяДата() - ДанныеТекущегоЗапроса.ДатаЗапроса >= 10*60
			...
			// Ожидали ответ 12 минут, он не пришел (после контрольного сеанса обмена)
			...
			// 12 минут с момента отправки запроса еще не прошло, ждем ответа (раз в минуту).
