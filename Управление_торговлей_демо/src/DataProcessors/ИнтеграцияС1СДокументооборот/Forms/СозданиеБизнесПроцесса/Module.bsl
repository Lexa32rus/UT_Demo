...
Функция СводкаШаблона(ШаблонID, ШаблонТип)
	...
	Запрос.template = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
...
Процедура ЗаполнитьДеревоШаблонов(ПолучатьВидДокумента = Ложь)
	...
	Если ПолучатьВидДокумента Тогда
		...
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, РезультатВидДокумента);
