...
Процедура ЗаполнитьСтрокуЗадачиXDTO(СтрокаЗадачи, ЗадачаXDTO)
	...
	ИначеЕсли СтрокаЗадачи.ТочкаМаршрутаКратко = "Ознакомиться" //@NON-NLS-1
		...
		ПроцессXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси,
		...
		Для Каждого ФайлXDTO Из ЗадачаXDTO.files Цикл
			...
			Если ДоступенЗахватФайлов Тогда
				...
				Если ФайлXDTO.Установлено("editingUser") Тогда
					ТекущийПользовательДокументооборота = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.
...
Функция ЗаписатьЗадачу(Тип, Идентификатор, Комментарий, ВыполнитьЗадачу, НомерКнопки, Знач ПредметыЗадачи,
	...
	Попытка
		ОбъектыXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси, Тип, Идентификатор);
	Исключение
		ОбработатьИсключение(ИнформацияОбОшибке());
		Возврат Ложь;
	КонецПопытки;
...
Процедура ЗаписатьПредметыЗадачи(ПараметрыЗадачи, Знач ПредметыЗадачи)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, ОбъектыXDTO);
	...
	Результат = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаписатьОбъект(Прокси, Задача);
