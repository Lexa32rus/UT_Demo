...
Процедура ЗаполнитьВерсииФайла()
	...
	Файл = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ИдентификаторФайла, "DMFile");
	СписокОбъектов.Добавить(Файл);
	
	ПолучаемыеПоля.Добавить("activeVersion");
	
	СведенияОФайлах = Прокси.execute(Запрос);
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, СведенияОФайлах);
