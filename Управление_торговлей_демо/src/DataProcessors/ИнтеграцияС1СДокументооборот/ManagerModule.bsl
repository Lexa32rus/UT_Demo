...
Процедура ПолучитьДополнительныеРеквизитыИПоместитьНаФорму(Прокси, Форма) Экспорт
	...
	Объект.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
	...
	Если Найти(Форма.Тип, "Document") <> 0 Тогда
		Если ЗначениеЗаполнено(Форма.ВидДокумента) Тогда
			...
			Объект.documentType.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
	...
	ИначеЕсли Форма.Тип = "DMCorrespondent" Тогда
		Если ЗначениеЗаполнено(Форма.ЮрФизЛицо) Тогда
			...
			Объект.legalPrivatePerson.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
...
Процедура ПриИзмененииВидаНаФормеДокумента(Форма, ДанныеОбъектовДО = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.Тип) И ЗначениеЗаполнено(Форма.ID) Тогда
		...
		Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси,
			Форма.Тип,
			Форма.ID,
			Колонки);
...
Процедура УстановитьДоступностьРегистрации(Форма, ОбъектXDTO) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ОбъектXDTO, "registrationAvailable") Тогда
...
Функция ВидДокументаXDTOПоФормеДокумента(Форма) Экспорт
	...
	Если ЗначениеЗаполнено(ВидДокументаID) И ЗначениеЗаполнено(ВидДокументаТип) Тогда
		ОбъектИд = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси, ВидДокументаID, ВидДокументаТип);
...
Функция ПодготовитьШапкуБизнесПроцесса(Прокси, Тип, Форма, Исключения = "") Экспорт
	...
	Если ЗначениеЗаполнено(Форма.ГлавнаяЗадачаID) Тогда
		Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси, Форма.ГлавнаяЗадачаТип, Форма.ГлавнаяЗадачаID);
		ГлавнаяЗадачаПриемник = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси,"DMBusinessProcessTask");
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьЗначенияСвойствXDTO(Прокси,ГлавнаяЗадачаПриемник,Ответ.Objects[0]);
...
Процедура ЗаполнитьПроцессПредметамиИзФормы(Прокси, Предметы, Процесс)
	...
	Для Каждого СтрокаПредмета Из Предметы Цикл
		...
		Если Не Процесс.Установлено("target") Тогда
			...
			Процесс.target.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
		...
		target.role.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
		...
		target.target.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMObjectID");
...
Функция ПодготовитьЛистСогласования(ПредметыСогласования)
	...
	Для Каждого ПредметСогласования Из МассивПредметов Цикл
		...
		ПредметВДО = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.
		...
		Иначе
			...
			Запрос.object = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, ПредметВДО.type);
