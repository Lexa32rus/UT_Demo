...
Процедура ТаблицаПотребностейПоЗаказамИОстатков(Параметры, АдресРезультата) Экспорт
	...
	Текст =
		...
		|	Т.Характеристика                                 КАК Характеристика,
		|	Т.Склад                                          КАК Склад,
		|	ЕСТЬNULL(ТопологияСкладов.Центр, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК РаспределительныйЦентр,
	...
	Если Параметры.ТипОтбора <> "ОтборПоЗначениям" Тогда
		...
		Иначе
			ТекстЗапроса =
				...
				|			ВтТовары КАК Работы
				|		ГДЕ
				|			Работы.Номенклатура = Остатки.Номенклатура
				|				И Работы.Характеристика = Остатки.Номенклатура
				|				И Работы.Склад = Остатки.Склад
				|				И Работы.Назначение = Остатки.Назначение
				|				И Работы.Склад ССЫЛКА Справочник.СтруктураПредприятия)
...
Функция ВременнаяТаблицаСвободныеОстатки(ВТаблицеОтбораЕстьНазначения)
	...
	Иначе
		Текст =
			...
			|				РазличныеТоварыИСклады КАК ТаблицаОтбора
			|			ГДЕ
			|				ТаблицаОтбора.Номенклатура = Остатки.Номенклатура
			|					И ТаблицаОтбора.Характеристика = Остатки.Характеристика
			|					И ТаблицаОтбора.Склад = Остатки.Склад)
			|		И &УсловиеОтбораНазначений
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура, Характеристика, Склад, Назначение";
	КонецЕсли;
	Тексты.Добавить(Текст);
...
Функция ВременнаяТаблицаПотребностиВыбранныхСкладов()
	...
	Текст =
		...
		|	
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТопологияСкладов КАК ТопологияДругиеРЦ
		|		ПО ТопологияДругиеРЦ.Номенклатура   = ВсеТовары.Номенклатура
		|		 И ТопологияДругиеРЦ.Характеристика = ВсеТовары.Характеристика
		|		 И ТопологияДругиеРЦ.Склад          = ВсеТовары.Склад
		|		 И ТопологияДругиеРЦ.Центр          = ВсеТовары.Склад
		|	
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТопологияСкладов КАК СкладыСети
		|		ПО СкладыСети.Номенклатура   =  Топология.Номенклатура
		|		 И СкладыСети.Характеристика =  Топология.Характеристика
		...
		|		И ТопологияДругиеРЦ.Номенклатура ЕСТЬ NULL
		|
		|{ГДЕ
		|	ВсеТовары.Склад.*                                                                                 КАК Склад,
...
Функция ВременнаяТаблицаЗаказыНаПеремещениеВсехСкладов()
	...
	Иначе
		ТекстЗапроса =
			...
			|					И Фильтр.Назначение = Сведения.Назначение)";
	КонецЕсли;
	ТекстВложенныйЗапрос = СтрСоединить(Тексты, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
	Текст = СтрЗаменить(Текст, "ВложенныйЗапросПереопределяемый", СтрШаблон("(%1)", ТекстВложенныйЗапрос));
	Возврат Текст;
