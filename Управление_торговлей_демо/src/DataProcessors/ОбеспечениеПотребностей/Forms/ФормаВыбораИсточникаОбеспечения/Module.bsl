...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыИПотребности.Остатки КАК ОстатокНаСкладеПереопределяемый
		...
		|		 И ОстатокНаСкладеПереопределяемый.Заказ = НЕОПРЕДЕЛЕНО
		|		 И ОстатокНаСкладеПереопределяемый.ДАТАСОБЫТИЯ = ДАТАВРЕМЯ(1, 1, 1)
