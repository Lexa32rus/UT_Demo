...
Функция ТекстЗапроса(ИмяЭлемента, ПодменятьТекущийЗаказ = Ложь)
	...
	ТекстЗапроса =
		...
		|							КОГДА ОжидаемаяОтгрузка.КОбеспечениюБезРезерва > 0 ТОГДА
		|								ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.КОбеспечению)
		|							КОГДА ОжидаемаяОтгрузка.РезервироватьПоМереПоступления + ОжидаемаяОтгрузка.ОтложитьРезервирование > 0 ТОГДА
	...
	ИначеЕсли ИмяЭлемента = Элементы.Набор.Имя Тогда
		...
		Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Движения.ЗаказНаОтгрузку", "Движения.Заказ");
		КонецЕсли;
