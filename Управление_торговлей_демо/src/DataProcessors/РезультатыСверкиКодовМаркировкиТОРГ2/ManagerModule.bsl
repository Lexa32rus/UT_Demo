...
Процедура ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(СтрокаИсточника, КоллекцияСтрокПриемника, ДанныеДокумента)
	...
	ИначеЕсли СтрокаИсточника.Родитель = Неопределено
		...
		Иначе
			Для Каждого ПодчиненнаяСтрокаИсточника Из СтрокаИсточника.Строки Цикл
				ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(ПодчиненнаяСтрокаИсточника, ЭлементГруппировкиПоGTIN.Строки, ДанныеДокумента);
			КонецЦикла;
			Возврат;
		КонецЕсли;
...
Процедура ВосстановитьДеревоДокументаПоРезультатамПроверки(Параметры, ДанныеДокумента, ОписаниеЗамера, АдресРезультата)
	...
	Пока ВыборкаРесурсовПроверки.Следующий() Цикл
		...
		Если ДанныеПроверкиИПодбора.Свойство("ДеревоМаркированнойПродукции") Тогда
			...
			ЗаполнитьДеревоМаркированнойПродукции(ДанныеПроверкиИПодбора.ДеревоМаркированнойПродукции, ДанныеДокумента);
		КонецЕсли;
