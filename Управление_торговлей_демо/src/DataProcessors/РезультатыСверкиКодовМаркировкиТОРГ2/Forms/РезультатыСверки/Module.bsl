...
Процедура ЗаполнитьСлужебныеРеквизитыКоллекцииСтрок(КоллекцияСтрок, 
	...
	Для Каждого СтрокаДерева Из КоллекцияСтрок Цикл
		...
		Если СтрокаДерева.Строки.Количество() = 0 Тогда
			...
			ТекущаяСтрокаСоответствуетОтбору = (Не СтрокаДерева.ДобавленаПоДаннымЭДО Или ДанныеПроверкиИПодбора.ДоступноСогласованиеРасхождений)
				И СтрокаДерева.СтатусПроверки <> Перечисления.СтатусыПроверкиНаличияПродукцииИС.ВНаличии;
...
Процедура ДополнитьУпаковкиПоДаннымЭДО(ДеревоМаркированнойПродукцииОбъект, ТаблицаРасхожденийПолная, ДанныеПроверкиИПодбора)
	...
	Для Каждого СтрокаРасхождений Из ТаблицаРасхождений Цикл
		...
		Если НайденнаяСтрокаДерева.ДобавленаПоДаннымЭДО И Не ДанныеПроверкиИПодбора.ДоступноСогласованиеРасхождений Тогда
			// протестировать
		//	И Не (ДанныеПроверкиИПодбора.СверкаПоДаннымКорректировки Или ДанныеПроверкиИПодбора.ДоступноСогласованиеРасхождений)Тогда
...
Функция НачатьЗагрузкуДанныхДокумента(ПараметрыСканирования)
	...
	ВозможностьЗагрузкиДанныхБезПодключенияИСМП = Неопределено;
	Если Не (ДоступноСогласованиеРасхождений Или СверкаПоДаннымКорректировки) Или ПроверкаЭлектронногоДокумента Тогда
		Если ИнтеграцияИСМПКлиентСерверПовтИсп.ЗапрашиватьДанныеСервиса() Тогда
			ВозможностьЗагрузкиДанныхБезПодключенияИСМП = Ложь;
		Иначе
			ВозможностьЗагрузкиДанныхБезПодключенияИСМП = Истина;
		КонецЕсли;
	КонецЕсли;
	ПараметрыПроцедуры.Вставить("ВозможностьЗагрузкиДанныхБезПодключенияИСМП", ВозможностьЗагрузкиДанныхБезПодключенияИСМП);
