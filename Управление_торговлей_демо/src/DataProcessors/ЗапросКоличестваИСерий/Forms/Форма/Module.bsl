	...
	Если Объект.ИндексСтрокиДокумента > -1 Тогда
		...
		Если СтрокаДокумента.ПроверятьДоступность
			...
			Если ДоступноПоСтроке < Объект.КоличествоУпаковок Тогда  
				...
				ОкруглениеКорректно = НоменклатураСервер.ОкруглениеКоличестваВыполненоКорректно(СтруктураОбъект, ПараметрыПроверки);
				Если ОкруглениеКорректно Тогда
...
Процедура КУказаниюСерийНаСервере()
	...
	ФорматнаяСтрока = "ЧДЦ=3; ЧН=0,000";
	Текст = Формат(Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока).Подобрано, ФорматнаяСтрока);
...
Процедура ДобавитьПодобранныеТовары(ПодобранныеТовары, СтрокиТовары, Упаковка, Цена, КоэффициентУпаковки)
	...
	Для Каждого СтрокаТовары Из СтрокиТовары Цикл
		...
		СтруктураОбъект.Вставить("Упаковка", Упаковка);
		СтруктураОбъект.Вставить("Количество", КоличествоСерий);
		СтруктураОбъект.Вставить("КоличествоУпаковок", СтрокаТовары.Подобрано);
		
		ОкруглениеКорректно = Подобрано <= 0
			Или Окр(КоличествоСерий, 0, РежимОкругления.Окр15как20) = Окр(СтрокаТовары.Подобрано * КоэффициентУпаковки, 0, РежимОкругления.Окр15как20)
				И НоменклатураСервер.ОкруглениеКоличестваВыполненоКорректно(СтруктураОбъект, ПараметрыПроверки);
