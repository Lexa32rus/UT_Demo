...
Функция ЗакончитьОтгрузкуНаСервере()
	...
	Для Каждого Строка Из ТоварыОтгружено Цикл
		...
		Если РезультатПоиска.Количество() > 0 Тогда
			...
			РазницаКоличествоУпаковок = РезультатПоиска[0].КоличествоУпаковок - Строка.КоличествоУпаковок;
			РазницаКоличество = РезультатПоиска[0].Количество - Строка.Количество;
			
			РезультатПоиска[0].КоличествоУпаковок = Строка.КоличествоУпаковок;
			РезультатПоиска[0].Количество = Строка.Количество;
			РезультатПоиска[0].Действие = Перечисления.ДействияСоСтрокамиОрдеровНаОтгрузку.Отгрузить;
			РезультатПоиска[0].Упаковка = Строка.Упаковка;
