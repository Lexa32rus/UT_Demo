...
Процедура СформироватьЗаголовкиКомандМеню()
	Элементы.КомандаРазместить.Заголовок = СтрШаблон(НСтр("ru = 'Разместить (%1)';
															|en = 'My (%1)'"), ТоварыРазместить.Количество());
	Элементы.КомандаРазмещено.Заголовок = СтрШаблон(НСтр("ru = 'Размещено (%1)';
														|en = 'Free (%1)'"), ТоварыРазмещено.Количество());
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьФорму()
	
	СтруктураДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Склад, Комментарий, Помещение, ЗонаПриемки, Исполнитель");
	...
	Для Каждого Строка Из Ссылка.ТоварыРазмещение Цикл
