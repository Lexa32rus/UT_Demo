...
Процедура ДеревоМаркированнойПродукцииСтатусПроверкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекущиеДанные = Неопределено
		Или ВыбранноеЗначение = ТекущиеДанные.СтатусПроверки Тогда
		Возврат;
	КонецЕсли;
	...
	Если ДопустимаПроизвольнаяЕдиницаУчета
		...
		И ТекущиеДанные.ПроизвольнаяЕдиницаУчета Тогда
		...
		ИначеЕсли ТекущиеДанные.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет") Тогда
			Если ТекущиеДанные.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии")
	...
	Если ТекущиеДанные.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет") Тогда
		ПриИзмененииСтатусаПроверкиСтрокиОСУ(ТекущиеДанные, ВыбранноеЗначение);
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура ДеревоМаркированнойПродукцииПослеУдаления(Элемент)
	...
	Для Каждого ИдентификаторРодителя Из ИдентификаторыРодителейУдаляемыхЭлементов Цикл
		...
		Если СтрокаДляПересчета.ТипУпаковки = ТипУпаковкиГруппировкаОбъемноСортовогоУчетаПоGTIN Тогда
			Если СтрокаДляПересчета.ПолучитьЭлементы().Количество() = 0 Тогда
...
Процедура ВыгрузитьВТСД(Команда)
	...
	МенеджерОборудованияКлиентИС.НачатьВыгрузкуДанныеВТСД(
...
Процедура ЗаполнитьКодыМаркировки(Команда)
	...
	Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетОтчетыОНанесенииКодовМаркировки(ВидМаркируемойПродукции)
		И ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(ПараметрыСканирования.ВидОперацииИСМП)=Неопределено
		И ПараметрыСканирования.ВидОперацииИСМП <> ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ПодтверждениеПоступленияКИЗ") Тогда
...
Процедура ЗаполнитьКоличествоПоДокументуОСУ(Команда)
	...
	Для Каждого СтрокаМаркируемойПродукции Из ПодобраннаяМаркируемаяПродукция Цикл
		...
		Если СтрокаМаркируемойПродукции.ТребуетВзвешивания
			Или СтрокаМаркируемойПродукции.Количество <= СтрокаМаркируемойПродукции.КоличествоПодобрано
		...
		Если СтрокаМаркируемойПродукции.ПроизвольнаяЕдиницаУчета Тогда
			АнализируемыеGTIN.Добавить(СтрокаМаркируемойПродукции.GTIN);
		КонецЕсли;

	КонецЦикла;

	Если СтрокиБезGTIN.Количество() > 0 Тогда
		СоответствиеСтрокБезGTIN = ПолучитьСоответствиеСтрокБезGTIN(СтрокиБезGTIN);
		Если СоответствиеСтрокБезGTIN.Количество() > 0 Тогда
			Для Каждого КлючИЗначение Из СоответствиеСтрокБезGTIN Цикл
				...
				СтрокиЗаполнения.Добавить(СтрокаМаркируемойПродукции);
				Если СтрокаМаркируемойПродукции.ПроизвольнаяЕдиницаУчета Тогда
					АнализируемыеGTIN.Добавить(КлючИЗначение.Значение);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	КонецЕсли;
	ВесаПоGTIN = ВесПоGTIN(АнализируемыеGTIN, ПараметрыСканирования);
	...
	Для Каждого СтрокаМаркируемойПродукции Из СтрокиЗаполнения Цикл
		...
		Если СтрокаМаркируемойПродукции.ПроизвольнаяЕдиницаУчета Тогда
			...
			Пока (КоличествоПодобрано + Вес) <= КоличествоКИзменению Цикл
				КоличествоПодобрано       = КоличествоПодобрано       + Вес;
				КоличествоВложенныхЕдиниц = КоличествоВложенныхЕдиниц + 1;
			КонецЦикла;
		...
		ДанныеШтрихкода.Вставить("ТипШтрихкода",                      ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.Code128"));
...
Процедура УстановитьСтатусДляВыделенныхСтрок(ВыделенныеСтроки, НовыйСтатус, ЭтоВосстановлениеДетализации = Ложь)
	...
	Для Каждого ИдентификаторВыделеннойСтроки Из ВыделенныеСтроки Цикл
		...
		Если Не ЭтоВосстановлениеДетализации Тогда
			...
			Если ДопустимаПроизвольнаяЕдиницаУчета
				...
				И ДанныеСтроки.ПроизвольнаяЕдиницаУчета Тогда
				...
				Если ДанныеСтроки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет") Тогда
					...
					Если ДанныеСтроки.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии")
						И (ДанныеСтроки.КоличествоПодчиненныхНеПроверялось > 0 Или ДанныеСтроки.КоличествоПодчиненныхНеЧислилось > 0) Тогда
						// Действия:
		...
		Если ДанныеСтроки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет") Тогда
			...
			ПриИзмененииСтатусаПроверкиСтрокиОСУ(ДанныеСтроки, НовыйСтатус);
	...
	Если МассивУпаковокСИзменившимсяСоставом.Количество() > 0
		Или ТаблицаИзмененийПодобраннойПродукции.Количество() > 0 Тогда
		ДополнительныеПараметры = Новый Структура("ЭтоОбработчикУстановкиСтатуса", Истина);
...
&НаКлиенте
Процедура ПриИзмененииСтатусаПроверкиСтрокиОСУ(СтрокаОбъемноСортовогоКода, НовыйСтатусПроверки)
	
	Если СтрокаОбъемноСортовогоКода.СтатусПроверки = НовыйСтатусПроверки Тогда
		Возврат;
	...
	СтрокаИзменений.Вставить("ТипШтрихкода",                       ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.Code128"));
	...
	Если НеобходимоУвеличитьКоличествоПодобраннойПродукции(РежимПодбораСуществующихУпаковок, СтрокаОбъемноСортовогоКода, НовыйСтатусПроверки) Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ЭтоУпаковкаОбъемноСортовогоУчета", Истина);
		ДополнительныеПараметры.Вставить("УточнитьКоличествоИС",             Ложь);
		...
		ПроверкаИПодборПродукцииИСМПКлиентСервер.ПодобраннаяПродукцияПриДобавленииСтроки(
			ЭтотОбъект, СтрокаИзменений, КоличествоПотребительскихУпаковок, ПроверкаИПодборПродукцииИСМПКлиент, ДополнительныеПараметры, КэшированныеЗначения);
	...
	ИначеЕсли НеобходимоУменьшитьКоличествоПодобраннойПродукции(РежимПодбораСуществующихУпаковок, СтрокаОбъемноСортовогоКода, НовыйСтатусПроверки) Тогда
		...
		Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхНеЧислилось > 0 Тогда
			...
			ДополнительныеПараметры = Новый Структура;
			ДополнительныеПараметры.Вставить("ЭтоУпаковкаОбъемноСортовогоУчета", Истина);
			ДополнительныеПараметры.Вставить("УточнитьКоличествоИС",             Не РежимПодбораСуществующихУпаковок);
			ДополнительныеПараметры.Вставить("ЭтоОбработчикУстановкиСтатуса",    Ложь);
		...
		Если КоличествоУпаковокКИзменению > 0 Тогда
			...
			ДополнительныеПараметры.Вставить("ЭтоУпаковкаОбъемноСортовогоУчета", Истина);
			ДополнительныеПараметры.Вставить("УточнитьКоличествоИС",             Ложь);
			ДополнительныеПараметры.Вставить("ЭтоОбработчикУстановкиСтатуса",    Истина);
	...
	Если Не СтрокаДерева.НедопустимыйКодМаркировки Тогда
		...
		Если СтрокаДерева.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.ОбъемноСортовойУчет") Тогда
			...
			Если СтрокаДерева.КоличествоПодчиненныхНеПроверялось > 0 Тогда
				...
				КоличествоПотребительскихУпаковок = СтрокаДерева.КоличествоПодчиненныхНеПроверялось;
				...
				Если НовыйСтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.Отсутствует") Тогда
					...
					Иначе
						СтрокаДерева.КоличествоПодчиненныхОтсутствует = КоличествоПотребительскихУпаковок;
				...
				СписокПолей = "GTIN,Номенклатура,Характеристика,Серия,
					|ВидУпаковки,ТипУпаковки,ТипШтрихкода,ТребуетВзвешивания,ПроизвольнаяЕдиницаУчета";
				
				ДанныеШтрихкода = Новый Структура(СписокПолей);
				ЗаполнитьЗначенияСвойств(ДанныеШтрихкода, СтрокаДерева);
				ДанныеШтрихкода.Вставить("КоличествоПотребительскихУпаковок", КоличествоПотребительскихУпаковок);
				ДанныеШтрихкода.Вставить("Количество",                        Количество);
				ДобавитьОбъемноСортовойКод(ДанныеШтрихкода, СтрокаДерева);
...
Процедура ДобавитьОбъемноСортовойКод(ДанныеШтрихкода, СтрокаОбъемноСортовогоКода = Неопределено, УстанавливатьСтатусПроверки = Истина)
	...
	Если ЭтоДобавлениеНовойСтроки Тогда
		...
		СтрокаОбъемноСортовогоКода.ТипШтрихкода                                     = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.Code128");
	...
	КоличествоПодчиненныхВНаличии = СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВНаличии;
	...
	Если РежимПодбораСуществующихУпаковок
		...
		СтрокаИзменений = Новый Структура;
		СтрокаИзменений.Вставить("GTIN");
		СтрокаИзменений.Вставить("Номенклатура");
		СтрокаИзменений.Вставить("Характеристика");
		СтрокаИзменений.Вставить("Серия");
		СтрокаИзменений.Вставить("СтатусПроверки");
		СтрокаИзменений.Вставить("Количество");
		СтрокаИзменений.Вставить("ПроизвольнаяЕдиницаУчета");
		СтрокаИзменений.Вставить("ТребуетВзвешивания");
		Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидМаркируемойПродукции) Тогда
			СтрокаИзменений.Вставить("ГоденДо");
			СтрокаИзменений.Вставить("СкоропортящаясяПродукция");
			Если ВидМаркируемойПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС") Тогда
				СтрокаИзменений.Вставить("ИдентификаторПроисхожденияВЕТИС");
			КонецЕсли;
		КонецЕсли;
		Если ПараметрыСканирования.ТребуетсяЧастичноеВыбытие
			И ИнтеграцияИСМПКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(
			ВидМаркируемойПродукции, ПараметрыСканирования.ВидОперацииИСМП) Тогда
			СтрокаИзменений.Вставить("ЧастичноеВыбытие");
		КонецЕсли;
		...
		Иначе
			...
			Если КоличествоУпаковокКИзменению > 0 Тогда
				...
				ДополнительныеПараметры = Новый Структура;
				ДополнительныеПараметры.Вставить("ЭтоУпаковкаОбъемноСортовогоУчета", Истина);
				ДополнительныеПараметры.Вставить("УточнитьКоличествоИС",             Истина);
	...
	Если РежимПодбораСуществующихУпаковок
		...
		Если Не РежимПодбораСуществующихУпаковок Тогда
			...
			Если КоличествоУпаковокКИзменению > 0 Тогда
				...
				Если СтрокаОбъемноСортовогоКода.ПроизвольнаяЕдиницаУчета Тогда
					Если СтрокаОбъемноСортовогоКода.ТребуетВзвешивания Тогда
						КоличествоПоУпаковкамКИзменению = СтрокаОбъемноСортовогоКода.Количество - ВесПроверенныхУпаковок;
					Иначе
						КоличествоПоУпаковкамКИзменению = КоличествоУпаковокКИзменению * КоличествоВПотребительскойУпаковке;
...
Процедура УдалитьОбъемноСортовойКод(ДанныеШтрихкода, СтрокаОбъемноСортовогоКода = Неопределено, ИзменятьПодобраннуюПродукцию = Истина)
	...
	ПроверкаИПодборПродукцииИСМПКлиентСервер.ПересчитатьИтогиПоПроверкеСодержимогоУпаковки(СтрокаОбъемноСортовогоКода, Ложь, ЗагрузкаДанныхТСД);
	НормализироватьШтрихкодУпаковкиПоСоставуВложенныхУпаковок(ЭтотОбъект, СтрокаОбъемноСортовогоКода, СоответствиеШтрихкодовСтрокДерева);
	ПроверкаИПодборПродукцииИСМПКлиентСервер.СформироватьПредставлениеДляСтрокиДереваМаркированнойПродукции(СтрокаОбъемноСортовогоКода, ВидМаркируемойПродукции);
	ПроверкаИПодборПродукцииИСМПКлиентСервер.УстановитьДоступностьУпаковкиДляПроверки(СтрокаОбъемноСортовогоКода, ДоступныеДляПроверкиУпаковки);
	...
	ЗаполнитьЗначенияСвойств(СтрокаИзменений, СтрокаОбъемноСортовогоКода,, "Количество");
	СтрокаИзменений.Количество = ДанныеШтрихкода.Количество;
	
	КоличествоПотребительскихУпаковокКИзменению = ДанныеШтрихкода.КоличествоПотребительскихУпаковок;
	
	Если Не РежимПодбораСуществующихУпаковок Тогда
		
		КоличествоУпаковокКИзменению = КоличествоПодчиненныхВсего - СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВсего;
		...
		Если КоличествоУпаковокКИзменению > 0 Тогда
			...
			ДополнительныеПараметры.Вставить("ЭтоУпаковкаОбъемноСортовогоУчета", Истина);
			ДополнительныеПараметры.Вставить("УточнитьКоличествоИС",             Истина);
			ДополнительныеПараметры.Вставить("ЭтоОбработчикУстановкиСтатуса",    ИзменятьПодобраннуюПродукцию);
			
			СтрокаИзменений.Количество = КоличествоУпаковокКИзменению;
	...
	Если СтрокаОбъемноСортовогоКода.КоличествоПодчиненныхВсего = 0 Тогда
		...
		ЭлементыИзГруппировкиПоGTIN.Удалить(СтрокаОбъемноСортовогоКода);
		
		Если ЭлементыИзГруппировкиПоGTIN.Количество() = 0 Тогда
			СтрокаДляПересчета = ЭлементГруппировкиПоGTIN.ПолучитьРодителя();
			СтрокаДляПересчета.ПолучитьЭлементы().Удалить(ЭлементГруппировкиПоGTIN);
		Иначе
			СтрокаДляПересчета = ЭлементГруппировкиПоGTIN;
		КонецЕсли;
	...
	//Если ИзменятьПодобраннуюПродукцию Или Не РежимПодбораСуществующихУпаковок Тогда
	//	
	//	ДополнительныеПараметры = Новый Структура;
	//	ДополнительныеПараметры.Вставить("ЭтоУпаковкаОбъемноСортовогоУчета", ИзменятьПодобраннуюПродукцию);
	//	ДополнительныеПараметры.Вставить("УточнитьКоличествоИС",             ИзменятьПодобраннуюПродукцию И РежимПодбораСуществующихУпаковок);
	//	ДополнительныеПараметры.Вставить("ЭтоОбработчикУстановкиСтатуса",    ИзменятьПодобраннуюПродукцию);
	//	
	//	ПроверкаИПодборПродукцииИСМПКлиентСервер.ПодобраннаяПродукцияПриУдаленииСтроки(
	//		ЭтотОбъект, СтрокаИзменений, ДанныеШтрихкода.КоличествоПотребительскихУпаковок, ПроверкаИПодборПродукцииИСМПКлиент, ДополнительныеПараметры, КэшированныеЗначения);
	//	
	//	КоличествоСтрокПодобраннойПродукции = ПодобраннаяМаркируемаяПродукция.Количество();
	//	
	//КонецЕсли;
	...
	Если РодительНовойСтроки <> Неопределено
		И (РодительНовойСтроки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
			Или (РодительНовойСтроки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор
				И РодительНовойСтроки.Номенклатура = НоваяСтрока.Номенклатура
				И РодительНовойСтроки.Характеристика = НоваяСтрока.Характеристика)) Тогда
...
Процедура ПодобраннаяПродукцияПриУдаленииСтроки(Форма, СтрокаИзменений, КэшированныеЗначения, ДополнительныеПараметры = Неопределено)
	...
	ПроверкаИПодборПродукцииИСМПКлиентСервер.ПодобраннаяПродукцияПриУдаленииСтроки(
		Форма, СтрокаИзменений, СтрокаИзменений.Изменение, ОбщийМодульКонтекстаПиП(), ДополнительныеПараметры, КэшированныеЗначения);
...
Процедура УточнитьДанныеПодобраннойПродукцииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЭтоИзменениеКоличестваПотребительскихУпаковокОСУ =
		ПоддерживаетсяОбъемноСортовойУчет И ИзменяемыеСвойства.Найти("Количество") <> Неопределено;
	
	Индекс = ИзменяемыеСвойства.ВГраница();
	Пока Индекс >= 0 Цикл
		Если ИзменяемыеСвойства[Индекс] = "КоличествоПотребительскихУпаковокОСУ"
			Или ИзменяемыеСвойства[Индекс] = "КоличествоПотребительскихУпаковок" Тогда
			ИзменяемыеСвойства.Удалить(Индекс);
			ЭтоИзменениеКоличестваПотребительскихУпаковокОСУ = Истина;
		КонецЕсли;
		Индекс = Индекс - 1;
	КонецЦикла;
	
	Если ЭтоИзменениеКоличестваПотребительскихУпаковокОСУ Тогда
		...
		КоличествоВложенныхЕдиниц = ДанныеВыбора.КоличествоПотребительскихУпаковокОСУ - ТекущиеДанные.КоличествоПотребительскихУпаковокОСУ;
		...
		ДанныеШтрихкода.Вставить("ТипУпаковки",                        ПредопределенноеЗначение("Перечисление.ТипыУпаковок.МонотоварнаяУпаковка"));
		ДанныеШтрихкода.Вставить("ТипШтрихкода",                       ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.Code128"));
		ДанныеШтрихкода.Вставить("ТребуетВзвешивания",                 ТекущиеДанные.ТребуетВзвешивания);
		ДанныеШтрихкода.Вставить("ПроизвольнаяЕдиницаУчета",           ТекущиеДанные.ПроизвольнаяЕдиницаУчета);
		ДанныеШтрихкода.Вставить("КоличествоВПотребительскойУпаковке", КоличествоВПотребительскойУпаковке);
		
		Если ТекущиеДанные.ТребуетВзвешивания Тогда
			
			Если ИзменяемыеСвойства.Найти("Количество") = Неопределено Тогда
				Количество = 0;
			Иначе
				Количество = ДанныеВыбора.Количество - ТекущиеДанные.КоличествоПодобрано;
			КонецЕсли;
			
			Если КоличествоВложенныхЕдиниц >= 0 И Количество >= 0 Тогда
				
				КоличествоПотребительскихУпаковок = КоличествоВложенныхЕдиниц;
				ВесПотребительскихУпаковок        = Количество;
				ВыполнятьСторнирование            = Ложь;
				
				Если КоличествоПотребительскихУпаковок = 0 Или ВесПотребительскихУпаковок = 0 Тогда
					КоличествоПотребительскихУпаковок = КоличествоПотребительскихУпаковок + 1;
					ВесПотребительскихУпаковок        = ВесПотребительскихУпаковок        + 1;
					ВыполнятьСторнирование            = Истина;
				КонецЕсли;
				
				ДанныеШтрихкода.КоличествоПотребительскихУпаковок = КоличествоПотребительскихУпаковок;
				ДанныеШтрихкода.Количество                        = ВесПотребительскихУпаковок;
				ДанныеШтрихкода.Штрихкод                          = СтрШаблон("(02)%1(37)%2", ДанныеШтрихкода.GTIN, Формат(ДанныеШтрихкода.КоличествоПотребительскихУпаковок, "ЧН=0; ЧГ=0"));
				ДобавитьОбъемноСортовойКод(ДанныеШтрихкода);
				
				Если ВыполнятьСторнирование Тогда
			...
			ИначеЕсли КоличествоВложенныхЕдиниц <= 0 И Количество <= 0 Тогда
			...
			ИначеЕсли Количество > 0 Тогда
				...
				КоличествоПотребительскихУпаковок = (-КоличествоВложенныхЕдиниц) + 1;
				ВесПотребительскихУпаковок        = Количество + 1;
		...
		Иначе
			...
			ДанныеШтрихкодаДо = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ДанныеШтрихкода);
			ЗаполнитьЗначенияСвойств(ДанныеШтрихкодаДо, ТекущиеДанные);
			
			Если ДопустимаПроизвольнаяЕдиницаУчета Тогда
				ЕстьПотребительскиеУпаковкиВСтруктуреУпаковок = (ТекущиеДанные.КоличествоПотребительскихУпаковокВСтруктуреУпаковок > 0);
				ЕстьПодобраннаяПродукция                      = (ТекущиеДанные.КоличествоПотребительскихУпаковок > 0);
			Иначе
				ЕстьПотребительскиеУпаковкиВСтруктуреУпаковок = (ТекущиеДанные.КоличествоИС > 0);
				ЕстьПодобраннаяПродукция                      = (ТекущиеДанные.КоличествоПодобрано > 0);
			КонецЕсли;
			
			Если КоличествоВложенныхЕдиниц > 0 Тогда
				
				Если ЕстьПотребительскиеУпаковкиВСтруктуреУпаковок Тогда
					
					Если Не ЕстьПодобраннаяПродукция И ИзменяемыеСвойства.Количество() > 0 Тогда
						
						ИзменениеКоличестваПотребительскихУпаковок = Мин(ТекущиеДанные.КоличествоПотребительскихУпаковокВСтруктуреУпаковок, КоличествоВложенныхЕдиниц);
						
						КодМаркировки = СтрШаблон("(02)%1(37)%2", ДанныеШтрихкода.GTIN, Формат(ИзменениеКоличестваПотребительскихУпаковок, "ЧН=0; ЧГ=0"));
						
						ДанныеШтрихкодаДо.КоличествоПотребительскихУпаковок = ИзменениеКоличестваПотребительскихУпаковок;
						ДанныеШтрихкодаДо.Количество                        = ИзменениеКоличестваПотребительскихУпаковок * КоличествоВПотребительскойУпаковке;
						ДанныеШтрихкодаДо.Штрихкод                          = КодМаркировки;
						
						УдалитьОбъемноСортовойКод(ДанныеШтрихкодаДо, Неопределено, Ложь);
						
					КонецЕсли;
					
				КонецЕсли;
				
				КодМаркировки = СтрШаблон("(02)%1(37)%2", ДанныеШтрихкода.GTIN, Формат(КоличествоВложенныхЕдиниц, "ЧН=0; ЧГ=0"));
				
				ДанныеШтрихкода.КоличествоПотребительскихУпаковок = КоличествоВложенныхЕдиниц;
				ДанныеШтрихкода.Количество                        = КоличествоВложенныхЕдиниц * КоличествоВПотребительскойУпаковке;
				ДанныеШтрихкода.Штрихкод                          = КодМаркировки;
...
Процедура ОбработатьСтруктуруУпаковокПослеУточненияДанныхПодобраннойПродукции(СтрокиДерева, ПараметрыОбработки)
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Иначе
			...
			АвтопересчетКоличества = СтрокаДерева.ПроизвольнаяЕдиницаУчета <> СтрокаИзмененийПосле.ПроизвольнаяЕдиницаУчета
				Или СтрокаДерева.ТребуетВзвешивания <> СтрокаИзмененийПосле.ТребуетВзвешивания
				Или СтрокаДерева.ПроизвольнаяЕдиницаУчета И Не СтрокаДерева.ТребуетВзвешивания;
...
Процедура УточнитьДанныеСтрокиПодобраннойПродукции()
	...
	УточнениеКоличестваОСУДопустимо    = ПоддерживаетсяОбъемноСортовойУчет
		И (ТекущиеДанные.КоличествоПотребительскихУпаковокОСУ > 0
			Или ТекущиеДанные.Количество > 0
			Или ТекущиеДанные.КоличествоИС > 0
			Или ТекущиеДанные.КоличествоПодобрано > 0
			Или ТекущиеДанные.КоличествоПотребительскихУпаковок > 0
			Или ТекущиеДанные.КоличествоПотребительскихУпаковокВСтруктуреУпаковок > 0);
...
Процедура ДобавитьОСУЗавершение(РезультатВыбора, ДополнительныеПараметры)
	...
	Если ДанныеВыбора.ТребуетВзвешивания Тогда
		...
		Если ДанныеВыбора.Количество = 0 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Количество не задано'"));
			Возврат;
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ВесПоGTIN(СписокGTIN, ПараметрыСканирования)
	...
	Для Каждого СтрокаТЧ Из ДанныеПоШтрихкодамEAN Цикл
		Если СтрокаТЧ.Количество > 0 Тогда
			Результат.Вставить(EANGTIN[СтрокаТЧ.ШтрихкодEAN], СтрокаТЧ.Количество);
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ПолучитьСоответствиеСтрокБезGTIN(СтрокиБезGTIN)
