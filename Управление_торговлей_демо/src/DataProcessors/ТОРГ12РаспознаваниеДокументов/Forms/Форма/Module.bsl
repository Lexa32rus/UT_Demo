...
Процедура ТаблицаДокументаПриИзменении(Элемент)
	...
	Если Элемент.ТекущийЭлемент.Имя = "Номенклатура" Тогда  
...
//	ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности");
//	ОткрытиеФормыДокумента("РеализацияТоваровУслуг", ВидОперации);
//КонецПроцедуры

//&НаКлиенте
//Процедура СоздатьРеализацияПродажаКомиссия(Команда)
//	ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияКомиссионногоТовара");
//	ОткрытиеФормыДокумента("РеализацияТоваровУслуг", ВидОперации);
//КонецПроцедуры
...
//	ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтДавальца");
//	ОткрытиеФормыДокумента("ПоступлениеТоваровУслуг", ВидОперации);
//КонецПроцедуры

//&НаКлиенте
//Процедура СоздатьПоступлениеОсновныеСредства(Команда)
//	ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОС");
//	ОткрытиеФормыДокумента("ПоступлениеТоваровУслуг", ВидОперации);
//КонецПроцедуры

////&НаКлиенте
//Процедура СоздатьПоступлениеПокупкаКомиссия(Команда)
//	ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПриемНаКомиссию");
//	ОткрытиеФормыДокумента("ПоступлениеТоваровУслуг", ВидОперации);
//КонецПроцедуры
...
//	ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеПрочихУслуг");
//	ОткрытиеФормыДокумента("ПоступлениеТоваровУслуг", ВидОперации);
//КонецПроцедуры
...
Процедура ИзменитьОтображениеПоВидуДокумента()
	...
	Измененно = Ложь;
	//Проверка на изменившиеся направление 
	Если Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда	
...
Процедура ИзменитьИсточникиДляВыбораСоглашения(ИмяПоля = "")
	...
	Если ИмяПоля = "ПродавецОрганизация" Или ИмяПоля = "ПокупательОрганизация" Тогда
	 ...
	 Для Каждого ЭтотСвязанный Из ВсеСвязанные Цикл
		...
		Для Каждого ЭтотТип Из ПодходящиеТипы Цикл
			...
			Если КомплектныеСтроки.Количество() <> 0 Тогда
				...
				ИначеЕсли ТипСвязанного = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
					Или ТипСвязанного = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов") Тогда
				...
				ИначеЕсли ТипСвязанного = Тип("ДокументСсылка.РеализацияТоваровУслуг") 
					Или ТипСвязанного = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") Тогда
...
Функция ПолучитьПараметрыОткрытияФормы(Знач ВидОперации, Знач ТипДокументаСтрокой)
	...
	//Записать();
	//ПараметрыЗаполнения = РаспознаваниеДокументовКомплектыВызовСервера.ПолучитьПараметрыОткрытияФормыТОРГ12(
	//	Объект,
	//	ВидОперации,
	//	ТипДокументаСтрокой
	//);
...
Процедура СохранитьИтоговыеЗначения()
	...
	Объект.КоличествоОшибокВТаблицах = ОсталосьОшибок - ПроблемныеЭлементы.Выгрузить(
			Новый Структура("НомерСтроки", 0), "ИмяРеквизита"
		).Количество();
...
Процедура ЗаполнитьЭлементыИтогов()
	...
	//Объект.РеквизитыДокумента[10].ИмяРеквизита = "ИтогоСумма";
	//Объект.РеквизитыДокумента[11].ИмяРеквизита = "ИтогоСуммаНДС";
	//Объект.РеквизитыДокумента[12].ИмяРеквизита = "ИтогоВсего";
...
Процедура УстановитьУсловноеОформление()
	...
	// Снятие отметки незаполненного подразделения, если Товар
	Элемент = УсловноеОформление.Элементы.Добавить();
...
//		Объект, 
//		РаспознаваниеДокументовБП.ОсновныеРеквизиты(Объект)["Организация"], 
