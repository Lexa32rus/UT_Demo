...
Процедура ОстаткиЗапросМарок(ПоТекущейСтроке)
	...
	Если ЗапроситьОстаткиПоСправкам.Количество() Тогда
		Если ТаймерЗапросов <= 0 Тогда
			ТаймерЗапросов = КоличествоМинутДоОчередногоЗапросаОстатковСправок();
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция КоличествоМинутДоОчередногоЗапросаОстатковСправок()
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ПрошлоМинут = Цел((ТекущаяУниверсальнаяДата() - Выборка.ДатаМодификацииУниверсальная)/60);
		Возврат Макс(0, 11-ПрошлоМинут);
	КонецЕсли;
