...
Процедура Загрузить(ПараметрыЗагрузки, АдресХранилища) Экспорт
	...
	Если ДанныеВыписки <> Неопределено Тогда
		...
		ПодготовитьСтрокиВыписки(ДокументыКЗагрузке, БанковскийСчет, РеквизитыСчета);
...
//-- Локализация
	...
	|		)
	|	И НЕ (ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику)
...
// 
// Параметры:
//  ТаблицаДокументов - ТаблицаЗначений - описание см. Обработки.КлиентБанк.ТабличнаяЧасть.ДокументыКВыгрузке.
...
Функция ЗаполнитьОбъектXDTO(Фабрика, Тип, ДанныеДокумента, СтруктураОбъекта, ИмяОбъекта = "")
	...
	Для Каждого ТекущееСвойство Из ОбъектДанныхXDTO.Свойства() Цикл
		...
		Исключение
			...
			ЗаписатьВЖурналРегистрации(ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), , ПараметрыОшибки);
	...
	Попытка
		...
		Для СтрокаМассиваВыписки = 0 По МассивСтраницВыписки.Количество() - 1 Цикл
			...
			Если СтрНайти(СтрокаСТегом25, ":25:") Тогда
				Если Найти(СтрокаСТегом25, "/") Тогда
					ДанныеПоРасчетномуСчету.РасчСчет = СокрЛП(Сред(СтрокаСТегом25, Найти(СтрокаСТегом25, "/") + 1));
				Иначе
...
Процедура ПрочитатьОперациюМТ940(МассивПоТегу86, ДокументыВыписки, РеквизитыВладельцаСчета)
	...
	КоличествоСимволовМеждуNИЗапятой = Найти(Данные61тега, ",") - СимволНачалаОтсчетаСуммы;
	
	Если Найти(Данные61тега,"//") > 0 Тогда
		Номер = СокрЛП(Сред(Данные61тега,Найти(Данные61тега,"//")+2));
	Иначе
		Номер = "";
	КонецЕсли;
	...
	Попытка
		ДанныеОперации.Сумма		  = Число(Сред(Данные61тега, СимволНачалаОтсчетаСуммы, КоличествоСимволовМеждуNИЗапятой + 2));
	Исключение
	...
	Если Найти(СтрокаПоТегу86, "~") > 0 Тогда
		СимволПоиска = "~";
	Иначе
		СимволПоиска = "?";
	КонецЕсли;
	...
	СтруктураВыписки = Новый Структура("Наименование, ИНН, Страна, НомерСчета, БанкSWIFT, Банк");
...
//  * РасчетныеСчета - Массив - расчетные счета по прочитанному файлу.
//  * ДокументыВыписки - Массив из Структура - данные по загружаемым операциям.
//  * ОшибкиРазбора - Массив - ошибки загрузки.
...
Процедура ПодготовитьСтрокиВыписки(ДокументыКЗагрузке, БанковскийСчет, РеквизитыСчета)
...
Процедура ЗаполнитьСведенияОбОстаткахТОФК(ОстаткиДенежныхСредствПоРазделам, НомерСчета, ДатаНачала, ДатаКонца, РасчетныеСчета)
	...
	Для Каждого Раздел Из ОстаткиДенежныхСредствПоРазделам Цикл
		...
		ДанныеПоСчету.РасчСчет = ПолучитьКазначейскийСчетОрганизации(НомерСчета, Раздел.АналитическийКодРаздела);
...
Функция ПолучитьКазначейскийСчетОрганизации(НомерСчета, РазделЛицевогоСчета)
	...
	Запрос.Текст = 
		...
		|	Справочник.БанковскиеСчетаОрганизаций КАК БанковскиеСчетаОрганизаций
		|ГДЕ
		|	БанковскиеСчетаОрганизаций.НомерЛицевогоСчета = &НомерСчета
		|	И БанковскиеСчетаОрганизаций.РазделЛицевогоСчета = &РазделЛицевогоСчета
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет";
	
	Запрос.УстановитьПараметр("НомерСчета", НомерСчета);
	Запрос.УстановитьПараметр("РазделЛицевогоСчета", РазделЛицевогоСчета);
	
	РезультатЗапроса = Запрос.Выполнить().Выбрать();
	
	Если РезультатЗапроса.Следующий() Тогда
		Возврат РезультатЗапроса.Счет;
	Иначе
		Возврат "";
	КонецЕсли;
