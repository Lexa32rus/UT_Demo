...
Процедура ВывестиОтчет(Команда)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.БанковскиеСчета.ВыделенныеСтроки Цикл
		...
		Если Не ДанныеСтроки.Загружен И ТипЗнч(ДанныеСтроки.НастройкаОбмена) = Тип("Строка") Тогда
			...
			Если ДанныеСтроки.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ФорматОбменаСБанком.ClientBankExchange1c") Тогда
				ТекстСообщения = "";
...
Процедура УстановитьВидимость()
	...
	Элементы.ЗаголовокЗачисления.Видимость = Ложь;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьВидимостьЗаголовкаЗачисления()

	Счет = Элементы.БанковскиеСчета.ТекущиеДанные;
	
	ПараметрыОтбора = Новый Структура("БанковскийСчет, ФайлПодтвержденияЗачисления", Счет.Ссылка, Истина);
	Файлы = Объект.Файлы.НайтиСтроки(ПараметрыОтбора);
	
	Элементы.ЗаголовокЗачисления.Видимость = Файлы.Количество() > 0;
...
Процедура ПрочитатьДанныеВыпискиПоФормату(Счет)
	...
	Для каждого ИмяФайла Из СписокФайлов Цикл
		...
		Исключение
			...
			НоваяСтрока.АдресХранилищаФайла = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
			Продолжить;
		КонецПопытки;
		
		Результат = ПрочитатьДанныеВыпискиПоФорматуНаСервере(Счет.ФорматОбмена, ДвоичныеДанныеФайла, Счет.Кодировка);
		
		Если ЗначениеЗаполнено(Результат.РасчетныеСчета) Тогда
			НоваяСтрока.ДатаНачалаЗагрузки = Результат.РасчетныеСчета[0].ДатаНачала;
			НоваяСтрока.ДатаКонцаЗагрузки = Результат.РасчетныеСчета[0].ДатаКонца;
		КонецЕсли;

		НоваяСтрока.Выписка = Новый Структура;
		НоваяСтрока.Выписка.Вставить("ДанныеВыписки", Результат);
		НоваяСтрока.АдресХранилищаФайла = ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла, УникальныйИдентификатор);
		НоваяСтрока.ФайлПодтвержденияЗачисления = Результат.Заголовок.Свойство("НомерРеестра");
...
Процедура ВывестиЗаголовокВыписки()
	...
	Иначе
		...
		Если ФайлыБанковскогоСчета.Количество() <> 0 Тогда
			...
			УстановитьВидимостьЗаголовкаЗачисления();
			Возврат;
		...
		Если ДанныеВыписки.Свойство("РасчетныеСчета", РасчетныеСчетаВыписки) Тогда
			Для каждого РасчетныйСчет Из РасчетныеСчетаВыписки Цикл
				Если РасчетныйСчет.РасчСчет = Счет.НомерСчета
					//++ Локализация
					И РасчетныйСчет.РазделЛицевогоСчета = Счет.РазделЛицевогоСчета
					И РасчетныйСчет.НомерЛицевогоСчета = Счет.НомерЛицевогоСчета 
					//-- Локализация
					Тогда
...
Процедура СформироватьЗаголовкиФайлов(ДанныеСчета, СписокФайлов)
	...
	Для каждого ТекущиеДанныеФайла Из СписокФайлов Цикл
		...
		Если ТекущиеДанныеФайла.ФайлПодтвержденияЗачисления Тогда
			...
			ЗаголовокЗачисления.ДобавитьСтроку(Символы.Таб + ТекущиеДанныеФайла.ИмяФайла);
		...
		Иначе
			...
			ЗаголовокВыписки.ДобавитьСтроку(Символы.Таб + ТекущиеДанныеФайла.ИмяФайла);
...
Процедура ЗагрузитьФайлНаКлиентеПроверкаНаКаталог(ЭтоКаталог, ДополнительныеПараметры) Экспорт
	...
	Если ЭтоКаталог Тогда
		ПрочитатьДанныеВыпискиЗагрузитьНачало(ДополнительныеПараметры.Счет);
	Иначе
...
&НаКлиенте
Процедура ПрочитатьДанныеВыпискиЗагрузитьНачало(Счет)

	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Счет", Счет);
	ПроцедураПослеЗавершения = Новый ОписаниеОповещения("ПослеПолученияСпискаФайловВКаталоге", ЭтотОбъект, ДополнительныеПараметры);
	НачатьПоискФайлов(ПроцедураПослеЗавершения, Счет.НастройкаОбмена, "*");
...
Процедура ЗагрузитьФайлНаКлиентеЗавершениеПомещения(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПомещенныеФайлы <> Неопределено И ПомещенныеФайлы.Количество() > 0 Тогда
		...
		Если ОписаниеФайлов.Количество() <> 0 Тогда
			...
			ПрочитатьДанныеВыпискиИЗагрузитьНаСервере(ДополнительныеПараметры.Счет.ПолучитьИдентификатор(), ОписаниеФайлов);
			ПодключитьОбработчикОжидания("ОбработатьЗавершениеЗагрузки", 1, Истина);
	...
	Для каждого ТекущееОписание Из ОписаниеФайлов Цикл
		...
		Если Счет.ФорматОбмена = ПредопределенноеЗначение("Перечисление.ФорматОбменаСБанком.ClientBankExchange1c") Тогда
			...
			Если ТекущийФайл.Расширение = ".txt" Тогда
				...
				НоваяСтрока = Объект.Файлы.Добавить();
				НоваяСтрока.ИмяФайла = ТекущееОписание.Имя;
				НоваяСтрока.БанковскийСчет = Счет.Ссылка;
...
// * ФайлПодтвержденияЗачисления - Булево - призак файла подтверждения зачисления ДС.
//
&НаКлиентеНаСервереБезКонтекста
