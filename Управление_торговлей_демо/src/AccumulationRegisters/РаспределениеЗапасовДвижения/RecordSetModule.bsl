...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если Не ДополнительныеСвойства.Свойство("ОтключитьРасчетИтогов") Тогда
		...
		КонецЕсли;
		Запрос.Текст = "УНИЧТОЖИТЬ ДвиженияПриЗаписи; УНИЧТОЖИТЬ РаспределениеЗапасовДвиженияИзменение; УНИЧТОЖИТЬ ИзменениеОстаткаПоТоварамДляКонтроляОстатков";
...
Процедура ВременнаяТаблицаОстатокНаСкладе(Запрос)
	...
	КонтрольОстатковВДокументеВключен = ПроведениеДокументов.ЭтоКонтрольныйРегистрИзменений(ДополнительныеСвойства);
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА СУММА(СведенияДоИзменения.РезервироватьПоМереПоступления)
		|				< СУММА(СведенияДоИзменения.Зарезервировано)
		|			ТОГДА СУММА(СведенияДоИзменения.РезервироватьПоМереПоступления)
		|		ИНАЧЕ СУММА(СведенияДоИзменения.Зарезервировано)
		...
		|		СведенияДоИзменения.Назначение          КАК Назначение,
		|		СведенияДоИзменения.ЗаказНаОтгрузку     КАК ЗаказНаОтгрузку,
		|		СведенияДоИзменения.Зарезервировано КАК Зарезервировано,
		|		СведенияДоИзменения.РезервироватьПоМереПоступления КАК РезервироватьПоМереПоступления
		|	ИЗ
		...
		|		ЗаказыТоваровДляКонтроляОстатков КАК Товары
		|			
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаданияКРаспределениюЗапасов КАК ЗаданияПоЗаказам
		|			ПО ЗаданияПоЗаказам.Номенклатура = Товары.Номенклатура
		|			 И ЗаданияПоЗаказам.Характеристика = Товары.Характеристика
		|			 И ЗаданияПоЗаказам.Склад = Товары.Склад
		|			 И ЗаданияПоЗаказам.Назначение = Товары.Назначение
		|			 И ЗаданияПоЗаказам.ЗаказНаОтгрузку = Товары.ЗаказНаОтгрузку
		|	ГДЕ
		|		ЗаданияПоЗаказам.КСнятиюРезерва > 0) КАК СведенияДоИзменения
		|	СГРУППИРОВАТЬ ПО
		...
		|		СведенияДоИзменения.ЗаказНаОтгрузку
		|ИМЕЮЩИЕ
		|	ВЫБОР
		|		КОГДА СУММА(СведенияДоИзменения.РезервироватьПоМереПоступления)
		|				< СУММА(СведенияДоИзменения.Зарезервировано)
		...
		|	ВЫБОР
		|		КОГДА СУММА(ИзмененияДвижений.Отгрузить + ИзмененияДвижений.Резервировать) > 0
		|			ТОГДА СУММА(ИзмененияДвижений.Отгрузить + ИзмененияДвижений.Резервировать)
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Отгрузка
		...
		|ИЗ
		|	ТоварыДляКонтроляОстатков КАК ТоварыДляКонтроляОстатков
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИзменениеОстаткаПоТоварамДляКонтроляОстатков КАК ИзмененияДвижений
		|		ПО ИзмененияДвижений.Номенклатура = ТоварыДляКонтроляОстатков.Номенклатура
		|		 И ИзмененияДвижений.Характеристика = ТоварыДляКонтроляОстатков.Характеристика
		|		 И ИзмененияДвижений.Склад = ТоварыДляКонтроляОстатков.Склад
		|		
		...
		|ГДЕ
		|	НЕ ИзмененияДвижений.Номенклатура ЕСТЬ NULL
		|	
		|СГРУППИРОВАТЬ ПО
		|	ИзмененияДвижений.Номенклатура,
		|	ИзмененияДвижений.Характеристика,
		...
		|		КОНЕЦ КАК Свободно,
		|	ЕСТЬNULL(ДанныеИБ.Излишек, 0) КАК Излишек
...
Процедура ЗаписатьЗадания(Запрос)
	...
	Текст =
		...
		|	ИзмененияДвижений.Назначение      КАК Назначение,
		|	ИзмененияДвижений.ЗаказНаОтгрузку КАК ЗаказНаОтгрузку,
		|	ЕСТЬNULL(ОтгрузкаИзСобственногоРезерва.Отгрузка, 0) КАК КСнятиюРезерва
		|ИЗ
		|	РаспределениеЗапасовДвиженияИзменение КАК ИзмененияДвижений
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИзменениеОстаткаПоТоварамДляКонтроляОстатков КАК ОтгрузкаИзСобственногоРезерва
		|		ПО ОтгрузкаИзСобственногоРезерва.Номенклатура = ИзмененияДвижений.Номенклатура
		|		 И ОтгрузкаИзСобственногоРезерва.Характеристика = ИзмененияДвижений.Характеристика
		|		 И ОтгрузкаИзСобственногоРезерва.Склад = ИзмененияДвижений.Склад
		|		 И ОтгрузкаИзСобственногоРезерва.Назначение = ИзмененияДвижений.Назначение
		|		 И ОтгрузкаИзСобственногоРезерва.ЗаказНаОтгрузку = ИзмененияДвижений.ЗаказНаОтгрузку";
...
Процедура ЗаписатьРаспределениеЗапасов(Запрос)
	...
	Запрос.Текст = РаспределениеЗапасов.ТекстЗапросаРаспределенияЗапасов(Истина, "ТоварыДляРаспределенияЗапасов");
	Запрос.УстановитьПараметр("НачалоТекущегоДня", НачалоДня(ТекущаяДатаСеанса()));
	Пакет = Запрос.ВыполнитьПакет();
